...
from direct.showbase.PythonUtil import makeTuple, getSetter
from pirates.piratesbase.PythonUtil import POD, ParamObj
...
class QuestDNA(ParamObj):
    ...
    def getDescriptionText(self, taskStates = [], bonus = False):
        ...
        if self._getString('customTask'):
            ...
            str = PLocalizer.QuestDescTaskSingle % {
                'task': self._getString('customTask') }
        elif len(taskDNAs) == 1:
            taskState = None
            if taskStates:
    ...
    def getSCSummaryText(self, taskNum, taskState = None):
        ...
        if len(taskDNAs) == 1:
            taskStr = PLocalizer.QuestDescTaskSingleNoPeriod % {
                'task': taskDNAs[0].getSCSummaryText(taskState) }
            str = PLocalizer.QuestStrOneTask % {
                'task': taskStr }
        elif len(taskDNAs) >= taskNum:
            taskStr = PLocalizer.QuestDescTaskSingleNoPeriod % {
                'task': taskDNAs[taskNum].getSCSummaryText(taskState) }
            str = PLocalizer.QuestStrOneTask % {
                'task': taskStr }
        else:
            str = ''
