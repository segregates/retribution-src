...
class DistributedPokerTable(DistributedGameTable.DistributedGameTable, PokerBase.PokerBase):
    ...
    def _DistributedPokerTable__playBetSequence(self, actor, i):
        self.endActionSequence(i)
        sfx = None
        audio = self.getAudio()
        if audio:
    ...
    def _DistributedPokerTable__playCheckSequence(self, actor, i):
        self.endActionSequence(i)
        sfx = None
        audio = self.getAudio()
        if audio:
    ...
    def _DistributedPokerTable__playFoldSequence(self, actor, i):
        self.endActionSequence(i)
        sfx = None
        audio = self.getAudio()
        if audio:
            ...
            if actor and newAction != oldAction:
                ...
                if mostRecentAction == PlayingCardGlobals.CheckCall or mostRecentAction == PlayingCardGlobals.Check:
                    if amount and mostRecentAction != PlayingCardGlobals.Check:
                        self._DistributedPokerTable__playBetSequence(actor, i)
                    else:
                        self._DistributedPokerTable__playCheckSequence(actor, i)
                elif mostRecentAction == PlayingCardGlobals.BetRaise:
                    self._DistributedPokerTable__playBetSequence(actor, i)
                elif mostRecentAction == PlayingCardGlobals.Fold:
                    self._DistributedPokerTable__playFoldSequence(actor, i)
                    self.playerHands[i] = []
    ...
    def dealCommunityCards(self, numCards):
        ...
        tell_action = 0
        enable_tells = False
        for player in xrange(len(self.playerHands)):
    ...
    def pickupCards(self, player, cards):
        ...
        def _DistributedPokerTable__reparentCards():
            for card in cards:
        ...
        pickupCardsIval = Sequence(Wait(random.random() * 1.0), actor.actorInterval('cards_pick_up', endFrame = 17, mixingWanted = False), Func(_DistributedPokerTable__reparentCards), actor.actorInterval('cards_pick_up', startFrame = 17, mixingWanted = False))
        tell_action = 0
        enable_tells = False
        if player <= len(self.totalWinningsArray):
            tell_action = self.totalWinningsArray[player]
