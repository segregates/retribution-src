from panda3d.core import Camera, TextNode, Vec4
...
class InventoryBagPage(InventoryPage.InventoryPage):
    ...
    def __init__(self, inventoryUIManager):
        ...
        weaponKeySlotList = []
        for weaponIndex in xrange(Locations.RANGE_EQUIP_WEAPONS[0], Locations.RANGE_EQUIP_WEAPONS[1] + 1):
    ...
    def askInventory(self, args = None):
        DistributedInventoryBase.DistributedInventoryBase.getInventory(base.localAvatar.inventoryId, self.inventoryArrived)
    ...
    def prepInventroy(self):
        if self.notLoadedWarning:
            self.notLoadedWarning.remove_node()
    ...
    def createContainerTab(self, container):
        self.createTabExtended(container.titleName, container.titleImageClosed, container.titleImageOpen, self.openContainer, [
            container])
    ...
    def showRedeemCodeGUI(self):
        if localAvatar.getTutorialState() < PiratesGlobals.TUT_MET_JOLLY_ROGER:
            if config.GetBool("want-early-coderedemption", False) or config.GetBool("skip-tutorial", False):
                pass
            else:
                localAvatar.guiMgr.createWarning(PLocalizer.CannotRedeemYet, PiratesGuiGlobals.TextFG6, duration = 8.0)
                return None

        if not config.GetBool("want-coderedemption", True):
            localAvatar.guiMgr.createWarning(PLocalizer.CodeRedemptionDisabled, PiratesGuiGlobals.TextFG6, duration = 8.0)
            return None
