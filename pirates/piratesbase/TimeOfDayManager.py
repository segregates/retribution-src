from panda3d.core import Fog, GraphicsStateGuardian, Light, VBase3, VBase4, Vec3, Vec4
...
class TimeOfDayManager(FSM.FSM, TimeOfDayManagerBase.TimeOfDayManagerBase):
    ...
    def toggleDebugMode(self):
        if self.debugTOD:
            self.debugTOD = False
            base.cr.magicWordManager.sendMagicWordResponse(response = 'Turning off TOD info')
        else:
            self.debugTOD = True
            base.cr.magicWordManager.sendMagicWordResponse(response = 'Turning on TOD info')
            base.talkAssistant.receiveSystemMessage('TODMgr: Server time is %s %s %s' % (str(self.getCurrentIngameTime())[:5], TODGlobals.StateDict[self.currentState], TODGlobals.ENV_DEBUG_NAMES[self.environment]))
    ...
    def setRelativeRotation(self, h = 0):
        if hasattr(base.localAvatar, 'gridParent'):
            if base.localAvatar.gridParent and base.localAvatar.gridParent.grid:
                self.skyGroup.setRelativeCompassH(base.localAvatar.gridParent.grid.getH())
            else:
                self.skyGroup.setRelativeCompassH(0.0)

    def disable(self):
        if self.envEffect != None:
    ...
    def syncTimeOfDay(self, soft = 1):
        ...
        if hasattr(base, 'pe'):
            ...
            self.request('EnvironmentTOD')
        if self.debugTOD:
            base.talkAssistant.receiveSystemMessage('syncTimeOfDay')
    ...
    def setFrontLightColor(self, newColor):
        ...
        if not self.flutterFront:
            lightValueForColor = TODGlobals.computeLightColor(self.frontLightColor, ambientColor, self.lightSwitch)
    ...
    def setBackLightColor(self, newColor):
        ...
        if not self.flutterBack:
            lightValueForColor = TODGlobals.computeLightColor(self.backLightColor, ambientColor, self.lightSwitch)
    ...
    def startFlutter(self):
        ...
        taskMgr.add(self._TimeOfDayManager__flutterLights, 'todManager-flutterLights')
    ...
    def insertTODSettings(self, environment, todState, ambientColor, frontColor, backColor, fogColor, fogExp, fogRange, lightSwitch, sunDir):
        ...
        if envGroup:
            ...
            TODGlobals.ENV_SETTINGS_DICT[environment][todState]['FogLinearRange'] = (fogRange[0], fogRange[1])
            TODGlobals.ENV_SETTINGS_DICT[environment][todState]['LightSwitch'] = (lightSwitch[0], lightSwitch[1], lightSwitch[2])
            TODGlobals.ENV_SETTINGS_DICT[environment][todState]['Direction'] = Vec3(sunDir[0], sunDir[1], sunDir[2])
    ...
    def insertTODSettingDict(self, env, tod, settingDict):
        for settingName in settingDict:
            self.insertTODSetting(env, tod, settingName, settingDict[settingName])
    ...
    def insertTODSetting(self, env, todId, settingName, settingValue):
        ...
        if settingName == 'FogLinearRange':
            TODGlobals.ENV_SETTINGS_DICT[env][tod][settingName] = (settingValue[0], settingValue[1])
        else:
            TODGlobals.ENV_SETTINGS_DICT[env][tod][settingName] = settingValue
    ...
    def _transitionTimeOfDay(self, fromState, toState, t, startEnv = None, destEnv = None):
        ...
        timeLightSwitch = TODGlobals.getTodEnvSetting(toState, environment, 'LightSwitch')
        fromAmbient = TODGlobals.getTodEnvSetting(fromState, startEnv, 'AmbientColor')
        fromFogColor = TODGlobals.getTodEnvSetting(fromState, startEnv, 'FogColor')
        fromFogExp = TODGlobals.getTodEnvSetting(fromState, startEnv, 'FogExp')
        fromFogOnset = TODGlobals.getTodEnvSetting(fromState, startEnv, 'FogLinearRange')[0]
        fromFogPeak = TODGlobals.getTodEnvSetting(fromState, startEnv, 'FogLinearRange')[1]
        ...
        if not self.fixedSky:
            ...
            toSkyType = TODGlobals.getTodEnvSetting(toState, destEnv, 'SkyType')
            toSkyColor = TODGlobals.SKY_CLEARCOLORS[toSkyType]
            ival.append(LerpFunctionInterval(self.setSkyClearColor, duration = t, fromData = fromSkyColor, toData = toSkyColor))
            if hasattr(base, 'pe'):
                ival.append(LerpFunctionInterval(base.setBackgroundColor, duration = t, fromData = fromSkyColor, toData = toSkyColor))

            toSkySettings = TODGlobals.getTodEnvSetting(toState, destEnv, 'SkyType')
            fromSkySettings = TODGlobals.getTodEnvSetting(fromState, startEnv, 'SkyType')
            if t > 0.0:
                ival.append(self.skyGroup.transitionSky(fromSkySettings, toSkySettings, duration = t))
        ...
        fromDirectionalColor = TODGlobals.computeLightColor(TODGlobals.getTodEnvSetting(fromState, startEnv, 'FrontColor'), TODGlobals.getTodEnvSetting(fromState, startEnv, 'AmbientColor'), timeLightSwitch)
        toDirectionalColor = TODGlobals.computeLightColor(TODGlobals.getTodEnvSetting(toState, destEnv, 'FrontColor'), TODGlobals.getTodEnvSetting(toState, destEnv, 'AmbientColor'), timeLightSwitch)
        ival.append(LerpFunctionInterval(self.setFrontLightColor, duration = t, toData = TODGlobals.getTodEnvSetting(toState, destEnv, 'FrontColor'), fromData = fromFrontLight, name = 'TOD_dLightColor-%d'))
        self.currShadowLight = self.skyGroup.getShadowLight(fromState)
        self.shadowLight = self.skyGroup.getShadowLight(toState)
        fromShadowLight = TODGlobals.computeLightColor(TODGlobals.getTodEnvSetting(fromState, startEnv, 'BackColor'), TODGlobals.getTodEnvSetting(fromState, startEnv, 'AmbientColor'), timeLightSwitch)
        toShadowLight = TODGlobals.computeLightColor(TODGlobals.getTodEnvSetting(toState, destEnv, 'BackColor'), TODGlobals.getTodEnvSetting(toState, destEnv, 'AmbientColor'), timeLightSwitch)
        ival.append(LerpFunctionInterval(self.setBackLightColor, duration = t, toData = TODGlobals.getTodEnvSetting(toState, destEnv, 'BackColor'), fromData = fromBackLight, name = 'TOD_dLightColor-%d'))
        fromMoonOverlay = TODGlobals.getTodEnvSetting(fromState, startEnv, 'MoonOverlay')
        fromMoonSize = TODGlobals.getTodEnvSetting(fromState, startEnv, 'MoonSize')
        fromMoonState = self.skyGroup.moonState
        toMoonOverlay = TODGlobals.getTodEnvSetting(toState, destEnv, 'MoonOverlay')
        toMoonSize = TODGlobals.getTodEnvSetting(toState, destEnv, 'MoonSize')
        toMoonState = TODGlobals.getTodEnvSetting(toState, destEnv, 'MoonPhase')
        if self.moonJolly:
            fromMoonOverlay = 0.5
            toMoonOverlay = 0.5
    ...
    def animateMoon(self, fromCurrent, startPhase, phase, duration):
        ...
        self.moonPhaseIval.append(LerpFunctionInterval(self.setMoonStateAnim, duration = duration, fromData = startPhase, toData = phase, name = 'animateMoon-Trans-MoonPhase'))
    ...
    def switchJollyMoon(self, jolly):
        if not (self.moonJolly) and jolly:
            ...
            self.moonJollyIval.append(LerpFunctionInterval(self.skyGroup.setMoonOverlayAlpha, duration = 3.0, fromData = 0.0, toData = 0.5, name = 'Switch-MoonJolly'))
            self.moonJollyIval.start()
        elif self.moonJolly and not jolly:
            self.moonJollyIval = Sequence()
            self.moonJollyIval.append(LerpFunctionInterval(self.skyGroup.setMoonOverlayAlpha, duration = 3.0, fromData = 0.5, toData = 0.0, name = 'Switch-MoonJolly'))
            self.moonJollyIval.start()
    ...
    def _prepareState(self, stateId, environment = None):
        ...
        if TODGlobals.getTodEnvSetting(stateId, environment, 'SkyType') == TODGlobals.SKY_OFF:
            self.skyGroup.stash()
            self.showSky = 0
        else:
            ...
            self.showSky = 1
        if not self.fixedSky:
            skyType = TODGlobals.getTodEnvSetting(stateId, environment, 'SkyType')
            ...
            skySettings = TODGlobals.getTodEnvSetting(stateId, environment, 'SkyType')
        ...
        if not render.getFog():
            fogColor = TODGlobals.getTodEnvSetting(stateId, environment, 'FogColor')
            self.fog.setColor(fogColor)
            fogExp = TODGlobals.getTodEnvSetting(stateId, environment, 'FogExp')
            self.setFogExpDensity(fogExp)
            linearFogRanges = TODGlobals.getTodEnvSetting(stateId, environment, 'FogLinearRange')
        ...
        lightSwitch = TODGlobals.getTodEnvSetting(stateId, environment, 'LightSwitch')
        ...
        if self.dlight:
            highLightColor = TODGlobals.computeLightColor(TODGlobals.getTodEnvSetting(stateId, environment, 'FrontColor'), TODGlobals.getTodEnvSetting(stateId, environment, 'AmbientColor'), lightSwitch = lightSwitch)
            self.setFrontLightColor(TODGlobals.getTodEnvSetting(stateId, environment, 'FrontColor'))
    ...
    def getCurrentIngameTime(self, time = None):
        ...
        if time == None:
            currentTime = globalClockDelta.networkToLocalTime(globalClockDelta.getFrameNetworkTime(bits = 32))
        else:
            currentTime = time
    ...
    def getEnvAtTime(self, baseEnv, timeForEnv):
        ...
        if not subEntry:
            ...
            timeStamp = globalClockDelta.networkToLocalTime(subEntry[3], bits = 32)
    ...
    def addEnvSub(self, baseEnv, envReplacement, netTime = None):
        ...
        localNetTime = globalClockDelta.networkToLocalTime(globalClockDelta.getFrameNetworkTime(bits = 32))
        REALSECONDS_PER_GAMEDAY = PiratesGlobals.TOD_REALSECONDS_PER_GAMEDAY / cycleSpeed
        REALSECONDS_PER_GAMEHOUR = float(REALSECONDS_PER_GAMEDAY / PiratesGlobals.TOD_GAMEHOURS_IN_GAMEDAY)
        cycleDuration = REALSECONDS_PER_GAMEHOUR * PiratesGlobals.TOD_GAMEHOURS_IN_GAMEDAY
        (fieldStartTime, absoluteTODCycle) = self.getSubTimeList(self.cycleType, self.environment)
        (previousIndex, currentIndex, nextIndex, stateStartTime) = self.getTODStateDatatAtTime(fieldStartTime, absoluteTODCycle, globalClock.getFrameTime(), REALSECONDS_PER_GAMEHOUR)
        ...
        if needStartRetro:
            if self.debugTOD:
                base.talkAssistant.receiveSystemMessage('Starting time of day for retroActive TimeStamp')
    ...
    def getSubTimeList(self, cycleType, env):
        ...
        REALSECONDS_PER_GAMEHOUR = float(REALSECONDS_PER_GAMEDAY / PiratesGlobals.TOD_GAMEHOURS_IN_GAMEDAY)
        cycleDuration = REALSECONDS_PER_GAMEHOUR * PiratesGlobals.TOD_GAMEHOURS_IN_GAMEDAY
        cycleList = TODGlobals.CycleStateTimeList.get(cycleType) + TODGlobals.CycleStateTimeList.get(cycleType) + TODGlobals.CycleStateTimeList.get(cycleType)
        modCycleList = []
        ...
        cycleEndFrameTime = currentFrameTime + (cycleDuration - gameTimeToSeconds)
        ...
        absoluteTimeList = self.getAbsoluteTimeList(cycleList, -(PiratesGlobals.TOD_GAMEHOURS_IN_GAMEDAY))
        if not sub:
            ...
            for timeEntry in absoluteTimeList:
                ...
                if subStart < transEndTime:
                    regEntry = (vId, startTime, transEndTime, transEndTime, envPre)
                    index += 1
                    subListAbsolute.append(regEntry)
    ...
    def getTODStateDatatAtTime(self, fieldStartTimeInSeconds, absoluteCycleList, currentTimeSeconds, secondsPerHour):
        fieldStartHour = absoluteCycleList[0][1]
        timeInHours = fieldStartHour + (currentTimeSeconds - fieldStartTimeInSeconds) / secondsPerHour
        currentIndex = 0
        nextIndex = None
        previousIndex = None
        stateStartTime = None
    ...
    def getAbsoluteTimeList(self, cycleList, startTime = 0):
        ...
        for timeEntry in cycleList:
            absoluteTimeList.append((timeEntry[0], runningTimeTotal, runningTimeTotal + timeEntry[2], runningTimeTotal + timeEntry[1], None))
    ...
    def setDesiredTime(self, desiredTime):
        ...
        REALSECONDS_PER_GAMEHOUR = float(REALSECONDS_PER_GAMEDAY / PiratesGlobals.TOD_GAMEHOURS_IN_GAMEDAY)
        cycleDuration = REALSECONDS_PER_GAMEHOUR * PiratesGlobals.TOD_GAMEHOURS_IN_GAMEDAY
        currentTime = self.getCurrentIngameTime() - self.timeOffset / REALSECONDS_PER_GAMEHOUR
        offsetInHours = (desiredTime - currentTime) % PiratesGlobals.TOD_GAMEHOURS_IN_GAMEDAY
    ...
    def doStartTimeOfDay(self, initial = 0):
        ...
        totalElapsedTime = (currentTimeSeconds - startServerTime) + self.timeOffset
        timeIntoCycle = totalElapsedTime % cycleDuration
        hoursIntoCycle = timeIntoCycle / REALSECONDS_PER_GAMEHOUR
        currentGameTimeInHours = hoursIntoCycle
        (previousIndex, currentIndex, nextIndex, stateStartTime) = self.getTODStateDatatAtTime(fieldStartTime, absoluteTODCycle, currentTimeSeconds + 0.5, REALSECONDS_PER_GAMEHOUR)
        ...
        secondsIntoCycle = float(currentGameTimeInHours - stateStartTime) * REALSECONDS_PER_GAMEHOUR
        secondsLeftInCycle = float(totalTransTimeHours) * REALSECONDS_PER_GAMEHOUR - secondsIntoCycle
        if secondsIntoCycle < 0 and 0:
            taskMgr.doMethodLater(1.0, self.doStartTimeOfDayTask, self.waitTaskName)
            return None

        self.lastState = absoluteTODCycle[previousIndex][0]
        self.currentStateDuration = float(totalStateTimeHours) * REALSECONDS_PER_GAMEHOUR
        stateDuration = self.currentStateDuration
        transitionTimeSeconds = float(totalTransTimeHours) * REALSECONDS_PER_GAMEHOUR
        stateStartTime = currentTimeSeconds - secondsIntoCycle
        previousStateStartTime = absoluteTODCycle[previousIndex][1]
        previoudStateEndTime = absoluteTODCycle[previousIndex][3]
        previousLength = (previoudStateEndTime - previousStateStartTime) * REALSECONDS_PER_GAMEHOUR
        ...
        if (initial or self.lastState == self.currentState) and startEnv == destEnv:
        ...
        if self.debugTOD:
            base.talkAssistant.receiveSystemMessage('TOD:%s T Sec:%s D Sec:%s In Sec:%s\nfrom %s-%s to %s-%s\nNext %s ' % (str(currentGameTimeInHours)[:5], str(transitionTimeSeconds)[:5], str(stateDuration)[:5], str(secondsIntoCycle)[:6], TODGlobals.ENV_DEBUG_NAMES[startEnv], TODGlobals.StateDict[absoluteTODCycle[previousIndex][0]], TODGlobals.ENV_DEBUG_NAMES[destEnv], TODGlobals.StateDict[absoluteTODCycle[currentIndex][0]], TODGlobals.StateDict[absoluteTODCycle[nextIndex][0]]))

        if TODGlobals.SunRotationStates[self.lastState] != TODGlobals.SunRotationStates[self.currentState]:
            fade = 1
            transitionTime = min(transitionTimeSeconds, TODGlobals.NONGROUP_MAX_TRANSITION_TIME)
        ...
        if initial:
            sunDirLast = TODGlobals.getTodEnvSetting(self.lastState, destEnv, 'Direction')
            self.skyGroup.setSunTrueAngle(sunDirLast)
            sunCorrection = 0

        sunDir = TODGlobals.getTodEnvSetting(self.currentState, destEnv, 'Direction')
        if self.transitionIval:
            self.transitionIval.finish()
            self.transitionIval = None
        ...
        if self.lastState != self.currentState or startEnv != destEnv:
            ...
            timeofDayTrans = self._transitionTimeOfDay(self.lastState, self.currentState, transitionTimeSeconds, startEnv = startEnv, destEnv = destEnv)
        ...
        self.sunMoveIval = self.skyGroup.transitionSunAngle(sunDir, sunMoveTime - sunCorrection, fade, sunDirLast = None)
        ...
        if not pauseTOD:
            ...
            taskMgr.doMethodLater(timeToNextState, self.doStartTimeOfDayTask, self.waitTaskName)
        else:
            self.transitionIval.finish()
    ...
    def enterIndoors(self, todSettings = None):
        ...
        if hasattr(base, 'pe'):
            ...
            base.setBackgroundColor(base.backgroundDrawable.getClearColor())
            self.linearFog.setColor(TODGlobals.getTodEnvSetting(PiratesGlobals.TOD_CUSTOM, self.environment, 'FogColor'))

        if todSettings:
            if todSettings.get('AmbientColors') and todSettings['AmbientColors'].get(self.currentState):
                self.setFillLightColor(todSettings['AmbientColors'][self.currentState])

            if todSettings.get('DirectionalColors') and todSettings['DirectionalColors'].get(self.currentState):
                self.setFrontLightColor(todSettings['DirectionalColors'][self.currentState])

            if todSettings.get('BacklightColors') and todSettings['BacklightColors'].get(self.currentState):
                self.setBackLightColor(todSettings['BacklightColors'][self.currentState])

            if todSettings.get('LightSwitches') and todSettings['LightSwitches'].get(self.currentState):
                self.setLightSwitch(todSettings['LightSwitches'][self.currentState])

            if todSettings.get('SunDirections') and todSettings['SunDirections'].get(self.currentState):
                self.skyGroup.setSunTrueAngle(todSettings['SunDirections'][self.currentState])

            if todSettings.get('FogColors') and todSettings['FogColors'].get(self.currentState):
                fogColor = todSettings['FogColors'][self.currentState]
                self.linearFog.setColor(fogColor)
                self.setSkyClearColor(fogColor, 1)

            if todSettings.get('LinearFogRanges') and todSettings['LinearFogRanges'].get(self.currentState):
            ...
            if todSettings.get('FogRanges') and todSettings['FogRanges'].get(self.currentState):
                fogDensity = todSettings['FogRanges'][self.currentState]
                self.setFogExpDensity(fogDensity)

            if todSettings.get('FogTypes') and todSettings['FogTypes'].get(self.currentState):
    ...
    def getTimeUntil(self, stateId):
        ...
        while currStateId != stateId:
            currStateId = TODGlobals.getNextStateId(self.cycleType, currStateId)
            stateDuration = TODGlobals.getStateDuration(self.cycleType, currStateId) * self.cycleDuration
            remTime += stateDuration
    ...
    def setEnvironment(self, envId, envData = None):
        ...
        if self.environment == envId and envData == None:
            return None
    ...
    def recalcClosestAmbNode(self, task = None):
        ...
        taskMgr.doMethodLater(2.5, self.recalcClosestAmbNode, 'ambientNodeSFXTask')
        timeStart = (5.5, 6.0)
        timeEnd = (20.0, 20.5)
        self.timeVol = 1.0
    ...
    def modifyWaterColor(self, color):
        self.setWColor = Vec4(color[0], color[1], color[2], 1)
        if not self.seapatch:
            return None

        if self.use_shader:
            self.seapatch.setWaterColor(self.setWColor)

        elif not self.seapatch.isEmpty():
            self.seapatch.setColorScale(self.setWColor)


    def setGreenSea(self):
        pass
        #self.seapatch.setWaterColor(VBase3(0.22, 0.560000, 0.149))
    ...
    def setFogExpDensity(self, newValue):
        ...
        if self.fog:
            if hasattr(base, 'localAvatar') and base.localAvatar.ship:
                self.fog.setExpDensity(self.fogExpDensity * TODGlobals.SHIP_FOG_MULT)
            else:
                self.fog.setExpDensity(self.fogExpDensity)
            ...
            if timeDict and timeDict != { }:
                ...
                for timeSettingName in TODGlobals.TOD_ATTRIBUTES:
                    ...
                    if timeSetting != None:
                        ...
                        outputString += tab * tabs
                        outputString += '        "%s" : %s,' % (timeSettingName, timeSetting)
                        outputString += '\n'
                        continue
