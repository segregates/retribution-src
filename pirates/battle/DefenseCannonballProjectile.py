from panda3d.core import BitMask32, CollideMask, CollisionHandler, CollisionHandlerEvent, CollisionHandlerQueue, CollisionNode, CollisionSphere, ColorBlendAttrib, NodePath, VBase4, Vec3
...
class DefenseCannonballProjectile(CannonballProjectile):
    ...
    def dispatchHitByAmmoEvent(self, playEffect = True):
        if self.hitByAmmoEvent:
            ...
            if shotNum:
                messenger.send(self.hitByAmmoEvent, [
                    shotNum,
                    playEffect])
    ...
    def destroyIfNecessary(self, cr, attackerId, hitObject, objType, pos, skillId, ammoSkillId, normal = None):
        if ammoSkillId == InventoryType.DefenseCannonMine:
            ...
            if objType in [
                PiratesGlobals.COLL_SEA,
                PiratesGlobals.COLL_DEFENSE_AMMO]:
                return None
    ...
    def findTarget(self, task = None):
        ...
        for i in xrange(len(base.cr.activeWorld.flamingBarrels)):
        ...
        for i in xrange(self.areaCollQueue.getNumEntries()):
            ...
            if self.target == None and ship and ship.queryGameState() not in [
                'FadeOut',
                'Sunk',
                'Sinking']:
                self.target = ship
                self.getWorldPos = True
                continue
            if ammo and int(ammo.getTag('ammoSkillId')) == InventoryType.DefenseCannonPowderKeg:
                self.target = ammo
                self.getWorldPos = False
                break
                continue
