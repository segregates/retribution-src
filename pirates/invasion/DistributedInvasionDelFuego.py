...
class DistributedInvasionDelFuego(DistributedInvasionObject.DistributedInvasionObject):
    ...
    def announceGenerate(self):
        ...
        self.waveMessages = [
            ...
            loadSfx(SoundGlobals.SFX_MONSTER_JR_RUBBLE),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_CRABS)]
    ...
    def updateBrigadeText(self):
        if self.brigadeText and self.parentObj.minimapArea:
            ...
            self.brigadeText = DirectLabel(parent = self.parentObj.minimap.getOverlayNode(), relief = None, text = PLocalizer.InvasionJollyRogerBrigadeUpdate % (self.currentPhase, self.totalPhases - self.currentPhase), text_align = TextNode.ALeft, text_fg = PiratesGuiGlobals.TextFG2, text_font = PiratesGlobals.getPirateOutlineFont(), text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, scale = 60, pos = (-800, -200, 0), hpr = (0, -90, 0))
