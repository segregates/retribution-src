...
class PotionStatusEffectManager:
    ...
    def addStatusEffect(self, effectId, attackerId, duration, timeLeft, timestamp, buffData):
        ...
        if effectId == WeaponGlobals.C_VOMIT:
            ...
            self._headFireSeq = self.avatar.getTransformSequence(None, self._addHeadFireEffect, [
                attackerId,
                timeLeft], timeSince)
            self._headFireSeq.start()
        elif effectId in [
            WeaponGlobals.C_INVISIBILITY_LVL1,
            WeaponGlobals.C_INVISIBILITY_LVL2]:
            if self._turnInvisibleSeq:
                self._turnInvisibleSeq.clearToInitial()
                self._turnInvisibleSeq = None

            self._turnInvisibleSeq = self.avatar.getTransformSequence(None, self.avatar.setIsInvisible, [
                True], timeSince)
            self._turnInvisibleSeq.start()
        elif effectId in [
            WeaponGlobals.C_SCORPION_TRANSFORM,
            WeaponGlobals.C_ALLIGATOR_TRANSFORM,
            WeaponGlobals.C_CRAB_TRANSFORM]:
            self.avatar.setCreatureTransformation(True, effectId, timeSince)
        elif effectId in [
            WeaponGlobals.C_SIZE_INCREASE,
            WeaponGlobals.C_SIZE_REDUCE]:
    ...
    def _playIntervalIfLandRoam(self, interval):
        if self.avatar.gameFSM != None:
            if self.avatar.getGameState() in [
                'LandRoam',
                'Battle']:
                interval.start()
