...
from panda3d.core import ColorBlendAttrib, ModelNode, Point3, Vec3
# File: F (Python 2.4)
...
class FishBitingBubbleEffect(PooledEffect, EffectController):
    cardScale = 128.0
    ...
    def __init__(self):
        ...
        if not FishBitingBubbleEffect.particleDummy:
            FishBitingBubbleEffect.particleDummy = render.attachNewNode(ModelNode('FishBitingBubbleEffectParticleDummy'))
            FishBitingBubbleEffect.particleDummy.node().setAttrib(ColorBlendAttrib.make(ColorBlendAttrib.MAdd, ColorBlendAttrib.OIncomingAlpha, ColorBlendAttrib.OOne))
            FishBitingBubbleEffect.particleDummy.setDepthWrite(0)
            FishBitingBubbleEffect.particleDummy.setFogOff()
            FishBitingBubbleEffect.particleDummy.setLightOff()
            FishBitingBubbleEffect.particleDummy.setColorScaleOff()
            FishBitingBubbleEffect.particleDummy.setTwoSided(1)

        self.f = ParticleEffect.ParticleEffect('FishBitingBubbleEffect')
