...
class BlockShield(PooledEffect, EffectController):
    ...
    def __init__(self):
        ...
        self.effectColor = Vec4(0.4, 0.598, 1, 1)
    ...
    def createTrack(self):
        ...
        fadeOut = LerpColorScaleInterval(self.effectModel, 0.200, Vec4(0, 0, 0, 0), startColorScale = self.effectColor, blendType = 'easeIn')
        scaleIval = LerpScaleInterval(self.effectModel, 0.200, Vec3(1, 1, 1.2), startScale = Vec3(1, 1, 0.5))
        uvScroll = LerpFunctionInterval(self.setNewUVs, 0.4, toData = 0.5, fromData = 0.0, extraArgs = [
            self.effectModel,
            textureStage])
        self.startEffect = Parallel(Sequence(Wait(0.200), fadeOut), scaleIval, uvScroll)
