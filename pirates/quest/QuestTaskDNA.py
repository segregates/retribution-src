...
class QuestTaskDNA(POD):
    ...
    def getLocationStr(self):
        ...
        if location != LocationIds.ANY_LOCATION:
            ...
            locationStr = PLocalizer.QuestTaskLocation % {
                'locationName': locationName }
            if islandName and locationName != islandName:
                locationStr += PLocalizer.QuestTaskIsland % {
...
class VisitTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        npcNameStr = PLocalizer.QuestTaskNpc % {
...
class GoToTaskDNA(VisitTaskDNA):
    ...
    def getGoalUid(self, taskState = None):
        ...
        elif not self.getGoalLocation():
            pass
        return QuestGoal(self.npcId)
    ...
    def getDescriptionText(self, state):
        ...
        if self.containerId:
            ...
            return PLocalizer.VisitPropTaskDesc % {
                'propName': containerName,
                'location': locationStr }
        else:
            npcNameStr = PLocalizer.QuestTaskNpc % {
                'npcName': self.getNPCName(self.npcId) }
...
class RecoverAvatarItemTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        numStr = ''
        if self.num > 1:
            numStr = PLocalizer.QuestTaskNum % {
                'num': self.num }
            itemName = PLocalizer.QuestItemNames[self.item][1]
            ...
            itemName = PLocalizer.QuestItemNames[self.item][0]
            enemyName = self.enemyType.getStrings()[0]
        itemStr = PLocalizer.QuestTaskItem % {
            'itemName': itemName }
        enemyStr = PLocalizer.QuestTaskEnemy % {
            'enemyName': enemyName }
        levelStr = ''
        if self.level > 0:
            levelStr = PLocalizer.QuestTaskLevel % {
                'level': self.level }

        locationStr = self.getLocationStr()
        return PLocalizer.RecoverAvatarItemTaskDesc % {
            'num': numStr,
            'itemName': itemStr,
            'level': levelStr,
            'enemyName': enemyStr,
    ...
    def getGoalUid(self, taskState = None):
        ...
        typeInfo = {
            QuestGoal.LEVEL_IDX: level,
            QuestGoal.TYPE_IDX: self.getEnemyType(),
            QuestGoal.LOCATION_IDX: self.getLocation() }
...
class RecoverNPCItemTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        itemStr = PLocalizer.QuestTaskItem % {
            'itemName': itemName }
        npcName = PLocalizer.QuestTaskEnemy % {
            'enemyName': self.getNPCName(self.getNpcId()) }
        locationStr = self.getLocationStr()
        return PLocalizer.RecoverAvatarItemTaskDesc % {
            'num': '',
            'itemName': itemStr,
...
class RecoverShipItemTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        if self.faction and self.faction != AvatarTypes.AnyShip:
            ...
            factionStr = PLocalizer.QuestTaskFaction % {
                'factionName': faction }
        ...
        if self.hull is not None:
            shipType = PLocalizer.ShipClassNames.get(self.hull)
            shipStr = PLocalizer.QuestTaskEnemy % {
                'enemyName': shipType }
        ...
        if PLocalizer.requiresAnIndefiniteArticle(stringList = [
            levelStr,
            faction,
            shipType]):
    ...
    def getGoalUid(self, taskState = None):
        ...
        typeInfo = {
            QuestGoal.LEVEL_IDX: level,
            QuestGoal.TYPE_IDX: 'ship',
            QuestGoal.FACTION_IDX: self.getFaction(),
            QuestGoal.HULL_IDX: self.getHull(),
            QuestGoal.FLAGSHIP_IDX: self.getIsFlagship(),
            QuestGoal.LOCATION_IDX: self.getLocation() }
        goal = QuestGoal(typeInfo)
        return goal
...
class RecoverContainerItemTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        if self.num > 1:
            ...
            itemName = PLocalizer.QuestItemNames[self.item][0]
            containerName = PLocalizer.PropTypeNames[containerType][0]
        itemNameStr = PLocalizer.QuestTaskItem % {
            'itemName': itemName }
        containerStr = PLocalizer.QuestTaskContainer % {
            'containerName': containerName }
        locationStr = self.getLocationStr()
        return PLocalizer.RecoverContainerItemTaskDesc % {
            'num': numStr,
...
class PoisonContainerTaskDNA(RecoverContainerItemTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        containerStr = PLocalizer.QuestTaskContainer % {
            'containerName': containerName }
        locationStr = self.getLocationStr()
        return PLocalizer.PoisonContainerTaskDesc % {
            'num': numStr,
            'container': containerStr,
            'location': locationStr }
...
class DeliverItemTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        numStr = ''
        if self.num > 1:
            numStr = PLocalizer.QuestTaskNumReg % {
                'num': self.num }
            itemName = PLocalizer.QuestItemNames[self.item][1]
        else:
            itemName = PLocalizer.QuestItemNames[self.item][0]
        return PLocalizer.DeliverItemTaskDesc % {
            'num': numStr,
            'itemName': itemName,
            'npcName': npcNameStr,
            'location': locationStr }
...
class SailToTaskDNA(QuestTaskDNA):
    ...
    def getSCSummaryText(self, state):
        ...
        return PLocalizer.QuestSCSailTo % {
            'location': locationName }
    ...
    def getDescriptionText(self, state):
        ...
        return PLocalizer.SailToTaskDesc % {
            'location': locationName }
    ...
    def getProgressMessage(self, taskState):
        ...
        return (progressMsg, PiratesGuiGlobals.TextFG10)
...
class SkeletonPokerTaskDNA(QuestTaskDNA):
    DataSet = {
        'gold': 1,
        'goldBonus': None,
        'containerId': PropIds.UNDEAD_POKER_TABLE }
    ...
    def getSCSummaryText(self, state):
        return PLocalizer.QuestSCSkeletonPokerWinGold % {
            'gold': self.gold }
    ...
    def getDescriptionText(self, state):
        return PLocalizer.PokerSkeletonTaskDescP % {
            'gold': self.gold }
    ...
    def getDescriptionTextBonus(self, state):
        ...
        return PLocalizer.PokerSkeletonTaskDescB % {
            'gold': self.goldBonus }
...
class PokerTaskDNA(QuestTaskDNA):
    ...
    def getSCSummaryText(self, state):
        if self.gold == 1:
            return PLocalizer.QuestSCPokerWinGold % {
                'gold': self.gold }
        else:
            return PLocalizer.QuestSCPokerWinGold % {
                'gold': self.gold }
...
class BlackjackTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        if self.gold == 1:
            ...
            return PLocalizer.BlackjackTaskDescP % {
                'gold': self.gold }
...
class RecoverTreasureItemTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        itemStr = PLocalizer.QuestTaskItem % {
            'itemName': itemName }
        locationStr = self.getLocationStr()
        return PLocalizer.RecoverTreasureItemTaskDesc % {
            'num': numStr,
            'itemName': itemStr,
            'location': locationStr }
...
class DefeatTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        if num > 1:
            numStr = PLocalizer.QuestTaskNum % {
                'num': num }
            enemyNameStr = PLocalizer.QuestTaskEnemy % {
                'enemyName': strings[1] }
        else:
            enemyNameStr = PLocalizer.QuestTaskEnemy % {
                'enemyName': strings[0] }
        ...
        if weaponName is not None:
            weaponStr = PLocalizer.QuestTaskWeapon % {
                'weaponName': weaponName }

        return PLocalizer.DefeatTaskDesc % {
            'num': numStr,
            'level': levelStr,
    ...
    def getDescriptionText(self, state):
        ...
        if self.num is not None:
            if self.num == 1:
                ...
                elif self.killType == PiratesGlobals.ShipPVPShip:
...
class DefeatShipTaskDNA(QuestTaskDNA):
    ...
    def getSCSummaryText(self, state):
        ...
        if shipsLeft == 1:
            if self.faction is None or self.faction == AvatarTypes.AnyShip:
                if shipType:
                    return PLocalizer.QuestSCSinkShip % {
                        'shipType': shipType }
                else:
                ...
                if shipType:
                    ...
                    return PLocalizer.QuestSCSinkFaction % {
                        'faction': faction }
        elif self.faction is None or self.faction == AvatarTypes.AnyShip:
            if shipType:
                ...
                return PLocalizer.QuestSCSinkNum % {
                    'num': shipsLeft }
        else:
            faction = PLocalizer.FactionShipTypeNames[self.faction.getFaction()][1][1]
            if shipType:
...
class RandomizedDefeatTaskDNA(DefeatTaskDNA):
    ...
    def getGoalUid(self, taskState = None):
        ...
        typeInfo = {
            QuestGoal.LEVEL_IDX: level,
            QuestGoal.TYPE_IDX: self._getEnemyType(taskState),
            QuestGoal.LOCATION_IDX: self.getLocation() }
        goal = QuestGoal(typeInfo)
        return goal
...
class RandomizedDefeatShipTaskDNA(DefeatShipTaskDNA):
    ...
    def getGoalUid(self, taskState = None):
        ...
        typeInfo = {
            QuestGoal.LEVEL_IDX: level,
            QuestGoal.TYPE_IDX: 'ship',
            QuestGoal.FACTION_IDX: self._getFaction(taskState),
            QuestGoal.HULL_IDX: self._getHull(taskState),
            QuestGoal.FLAGSHIP_IDX: self.getIsFlagship(),
            QuestGoal.LOCATION_IDX: self.getLocation() }
        goal = QuestGoal(typeInfo)
        return goal
...
class ViewCutsceneTaskDNA(QuestTaskDNA):
    ...
    def handleNPCVisit(self, questEvent, taskState):
        if questEvent.npcId == self.npcId:
            ...
            npcId = currWorld.uidMgr.getDoId(self.npcId)
            npc = simbase.air.doId2do.get(npcId)
            playerAv.b_setGameState('NPCInteract', localArgs = [
                npc,
                True,
                False])
            if self.dialogId:
    ...
    def getDescriptionText(self, state):
        return PLocalizer.ViewCutsceneTaskDesc % {
            'toNpcName': self.getNPCName(self.npcId) } + ' to view cutscene'
...
class BossBattleTaskDNA(QuestTaskDNA):
    ...
    def getSCSummaryText(self, state):
        ...
        return PLocalizer.QuestSCBossBattleMap % {
...
class BurnPropTaskDNA(QuestTaskDNA):
    DataSet = {
        'propType': PiratesGlobals.QUEST_PROP_AZT_IDOL_A_DESTR,
        'num': 1 }
    ...
    def getDescriptionText(self, state):
        ...
        locationStr = self.getLocationStr()
        return PLocalizer.BurnPropTaskDesc % {
            'num': numStr,
...
class DefeatNearPropTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        return PLocalizer.DefeatNearPropTaskDesc % {
            'num': numStr,
            'level': levelStr,
            'enemyName': enemyNameStr,
            'propName': propNameStr,
...
class DefendNPCTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        enemyNameStr = PLocalizer.QuestTaskEnemy % {
            'enemyName': strings[0] }
        return PLocalizer.DefendNPCTaskDesc % {
            'enemyName': enemyNameStr }


    def getTitle(self):
        strings = self._getEnemyType(state).getStrings()
        return PLocalizer.DefendNPCTaskTitle % PLocalizer.QuestTaskEnemy % {
            'enemyName': strings[0] }
...
class DefeatAroundPropTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        return PLocalizer.DefeatAroundPropTaskDesc % {
            'enemyName': enemyNameStr,
            'propName': propNameStr,
            'itemName': itemName }
...
class FishingTaskDNA(QuestTaskDNA):
    ...
    def handleFishCaught(self, questEvent, taskState):
        if not questEvent.fishType == self.fishType:
            pass
        return questEvent.fishType > self.fishTypeBonus
    ...
    def getSCSummaryText(self, state):
        ...
        return PLocalizer.QuestSCFishing % {
            'fishName': fishName }
    ...
    def getDescriptionText(self, state):
        ...
        if self.fishType == InventoryType.Collection_Set11:
            if self.num == 1:
                return PLocalizer.FishingTaskDescLegendaryS % {
                    'fishName': fishName }
            else:
                return PLocalizer.FishingTaskDescLegendaryP % {
                    'num': self.num,
                    'fishName': fishName }
        elif self.num == 1:
            return PLocalizer.FishingTaskDescS % {
                'fishName': fishName }
        else:
            return PLocalizer.FishingTaskDescP % {
                'num': self.num,
                'fishName': fishName }
...
class PotionsTaskDNA(QuestTaskDNA):
    DataSet = {
        'potionType': None,
        'potionTypeBonus': None,
        'num': 1,
        'numBonus': 1 }

    def handlePotionBrewed(self, questEvent, taskState):
        return questEvent.potionType in [
            self.potionType,
            self.potionTypeBonus]
    ...
    def getSCSummaryText(self, state):
        ...
        return PLocalizer.QuestSCPotions % {
    ...
    def getDescriptionText(self, state):
        ...
        goalNum = self.num
        if goalNum == 1:
            return PLocalizer.PotionsTaskDescS % {
                'potionName': potionName }
        else:
            return PLocalizer.PotionsTaskDescP % {
                'num': goalNum,
                'potionName': potionName }
...
class DowsingRodTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        itemStr = PLocalizer.QuestTaskItem % {
            'itemName': itemName }
        return PLocalizer.DowsingRodTaskDesc % {
            'itemName': itemStr }
...
class LootPropTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        ...
        propNameStr = PLocalizer.PropTypeNames[containerType][0]
        locationStr = self.getLocationStr()
        return PLocalizer.LootPropTaskDesc % {
            'propName': propNameStr,
            'location': locationStr }
...
class ScrimmageTaskDNA(QuestTaskDNA):
    ...
    def getDescriptionText(self, state):
        npcNameStr = PLocalizer.QuestTaskNpc % {
            'npcName': self.getNPCName(self.npcId) }
        locationStr = self.getLocationStr()
        return PLocalizer.ScrimmageTaskDesc % {
            'npcName': npcNameStr,
            'location': locationStr,
            'num': self.num }
...
DBId2Class = {
    ...
    23: RecoverNPCItemTaskDNA,
    24: PoisonContainerTaskDNA,
    25: BurnPropTaskDNA,
    26: DefeatNearPropTaskDNA,
    27: DefendNPCTaskDNA,
    28: DefeatAroundPropTaskDNA,
    25: CompleteQuestContainerTaskDNA,
    26: BurnPropTaskDNA,
    27: DefeatNearPropTaskDNA,
    28: DefendNPCTaskDNA,
    29: DefeatAroundPropTaskDNA,
    30: DowsingRodTaskDNA,
    31: SailToTaskDNA,
    32: PotionsTaskDNA,
    33: FishingTaskDNA,
    34: SkeletonPokerTaskDNA,
    35: GoToTaskDNA,
    36: LootPropTaskDNA,
    37: ScrimmageTaskDNA }
Class2DBId = invertDict(DBId2Class)
RecoverItemClasses = (RecoverAvatarItemTaskDNA, RecoverTreasureItemTaskDNA, RecoverShipItemTaskDNA, RecoverContainerItemTaskDNA, RecoverNPCItemTaskDNA, DowsingRodTaskDNA)
