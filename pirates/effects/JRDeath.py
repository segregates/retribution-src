from panda3d.core import ColorBlendAttrib, NodePath, Texture, TextureStage, VBase3, VBase4, Vec3
# File: J (Python 2.4)
...
class JRDeath(PooledEffect, EffectController):
    ...
    def createTrack(self):
        ...
        fadeInRays = LerpColorScaleInterval(self.rays, 0.200, VBase4(0.800000, 1, 0.100, 1), startColorScale = VBase4(0.800000, 1, 0.100, 0))
        fadeOutRays = LerpColorScaleInterval(self.rays, 0.75, VBase4(0.800000, 1, 0.100, 0), startColorScale = VBase4(0.800000, 1, 0.100, 1))
        scaleUpRays = LerpScaleInterval(self.rays, 0.200, VBase3(1, 1, 1), startScale = VBase3(1, 1, 0), blendType = 'easeOut')
        ...
        uvScrollA = LerpFunctionInterval(self.setNewUVs, 1.0, toData = -2.60, fromData = 0.0, extraArgs = [
            self.jolly,
            textureStage])
        uvScrollB = LerpFunctionInterval(self.setNewUVs, 0.75, toData = -6.5, fromData = -2.60, extraArgs = [
            self.jolly,
            textureStage])
        self.track = Sequence(Func(self.anim.loop), Parallel(fadeInRays, scaleUpRays, scaleJollyIval, Sequence(uvScrollA, uvScrollB), Sequence(Wait(0.200), Parallel(fadeOutRays, scaleDownRays))), Func(self.anim.finish), Func(self.cleanUpEffect))
