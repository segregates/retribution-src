from panda3d.physics import BaseParticleEmitter, BaseParticleRenderer, LinearFrictionForce
...
class MusketFlame(PooledEffect, EffectController):
    ...
    def __init__(self):
        ...
        if not MusketFlame.particleDummy:
            MusketFlame.particleDummy = render.attachNewNode(ModelNode('FireParticleDummy'))
    ...
    def createTrack(self):
        ...
        self.p0.renderer.setInitialYScale(0.0598 * self.cardScale)
        self.p0.renderer.setFinalYScale(0.02 * self.cardScale)
        self.p0.renderer.setNonanimatedTheta(0.0)
        self.p0.renderer.setAlphaBlendMethod(BaseParticleRenderer.PPNOBLEND)
        self.p0.renderer.setAlphaDisable(0)
        self.p0.emitter.setEmissionType(BaseParticleEmitter.ETRADIATE)
        self.p0.emitter.setAmplitude(-0.299)
        self.p0.emitter.setAmplitudeSpread(0.299)
        self.p0.emitter.setOffsetForce(Vec3(0.0, 40.0, 0.0))
        self.p0.emitter.setExplicitLaunchVector(Vec3(0.0, 0.0, 0.0))
        self.p0.emitter.setRadiateOrigin(Point3(0.0, 0.0, 0.0))
        self.p0.emitter.setRadius(0.200)
        fadeBlast = self.flash.colorScaleInterval(0.149, Vec4(0, 0, 0, 0), startColorScale = self.startCol, blendType = 'easeOut')
        scaleBlast = self.flash.scaleInterval(0.200, 10, blendType = 'easeIn')
        ...
        self.playParticles = Sequence(Func(self.p0.setBirthRate, 0.01), Func(self.p0.clearToInitial), Func(self.f.start, self, self.particleDummy), Func(self.f.reparentTo, self), Wait(0.149), Func(self.p0.setBirthRate, 100), Wait(1.5), Func(self.cleanUpEffect))
