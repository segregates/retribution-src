...
class DistributedInvasionTortuga(DistributedInvasionObject.DistributedInvasionObject):
    ...
    def __init__(self, cr):
        ...
        self.setHolidayId(HolidayGlobals.INVASIONTORTUGA)


    def announceGenerate(self):
        DistributedInvasionObject.DistributedInvasionObject.announceGenerate(self)
        self.startMessages = [
            loadSfx(SoundGlobals.SFX_MONSTER_JR_GOGOGO)]
        ...
        self.waveMessages = [
            ...
            loadSfx(SoundGlobals.SFX_MONSTER_JR_SPARROW_ALIVE),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_FAITHFUL_BRIDE),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_RANSACK_TOWN)]
        ...
        self.loseMessages = [
            (PLocalizer.InvasionJollyRogerEndPlayerLose4, loadSfx(SoundGlobals.SFX_MONSTER_JR_SPARROW_ESCAPED))]
    ...
    def updateBrigadeText(self):
        if self.brigadeText and self.parentObj.minimapArea:
            ...
            self.brigadeText = DirectLabel(parent = self.parentObj.minimap.getOverlayNode(), relief = None, text = PLocalizer.InvasionJollyRogerBrigadeUpdate % (self.currentPhase, self.totalPhases - self.currentPhase), text_align = TextNode.ALeft, text_fg = PiratesGuiGlobals.TextFG2, text_font = PiratesGlobals.getPirateOutlineFont(), text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, scale = 40, pos = (-350, 450, 0), hpr = (0, -90, 0))
