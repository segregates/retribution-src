...
class SectionAreaBuilder(AreaBuilderBase):
    ...
    def addSectionObj(self, geometry, visZone, logError = 0):
        ...
        if not visZone in self.visHelper:
        ...
        visible = (visZone == self.visZone) or visZone in self.visHelper.get(self.visZone, set())
        if visible:
            geometry.unstash()
        else:
            geometry.stash()
    ...
    def isVisible(self, data):
        ...
        if not self.visTable:
            return True

        return data in self.visTable[self.visZone][0] or data == self.visZone
