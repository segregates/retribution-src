from panda3d.core import CollideMask, CollisionNode, CollisionTube, GeomNode, Light, NodePath, Point3
...
class Avatar(Actor, ShadowCaster):
    ...
    def deleteNametag3d(self):
        if self.nametagNodePath:
            self.nametagNodePath.remove_node()
            self.nametagNodePath = None
        return
...
def clickNametag(avId):
    ...
    av.clickedNametag()
    return "Simulated %s's Name" % av.getName()
