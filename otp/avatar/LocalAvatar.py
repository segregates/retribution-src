...
class LocalAvatar(DistributedAvatar.DistributedAvatar, DistributedSmoothNode.DistributedSmoothNode):
    ...
    def deleteSmartCameraCollisions(self):
        ...
        del self.ccLine
        del self.ccLineNode
        self.ccLineNodePath.remove_node()
        del self.ccLineNodePath
        del self.camCollisionQueue
        del self.ccRay
        del self.ccRayNode
        self.ccRayNodePath.remove_node()
    ...
    def rocketman():
        if not localAvatar.rocketOn == 0:
            ...
            return "And I think it's gonna be a long long time."
        localAvatar.startRocketJumpMode()
        return 'Zero hour nine a.m. (Bill Shatner Version)'

    @magicWord(CATEGORY_GAME_DEVELOPER)
    def shipUpgrade():
        localAvatar.guiMgr.toggleShipUpgrades()
        return 'Toggled ship upgrades!'

    @magicWord(CATEGORY_STAFF)
    def shipCam():
        if not base.shipLookAhead:
            base.setShipLookAhead(1)
            return 'Ship Look ahead camera enabled!'
        base.setShipLookAhead(0)
        return 'Ship Look ahead camera disabled!'

    @magicWord(CATEGORY_BASIC)
    def time():
        return 'The time is %s, Pirates Standard Time' % str(base.cr.timeOfDayManager.getCurrentIngameTime())[:5]
    ...
    @magicWord(CATEGORY_STAFF)
    def vismask():
        return 'Vis Mask %s' % localAvatar.invisibleMask
    ...
    @magicWord(CATEGORY_STAFF)
    def toggleColl():
        if not base.localAvatar.isCollisions == 0:
            base.localAvatar.collisionsOff()
            base.localAvatar.isCollisions = 0
            return 'Collisions disabled!'
        base.localAvatar.collisionsOn()
        base.localAvatar.isCollisions = 1
        return 'Collisions enabled!'
