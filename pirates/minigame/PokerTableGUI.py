from panda3d.core import CardMaker, NodePath, TextNode, Vec3, Vec4
# File: P (Python 2.4)
...
class PokerTableGUI(DirectFrame, TableGUI):
    ...
    def __init__(self, table, maxCommunityCards, maxHandCards):
        ...
        for i in xrange(self.table.NumSeats + 1):
    ...
    def clearTable(self):
        ...
        for i in xrange(len(self.communityCards)):
    ...
    def setTableState(self, round, buttonSeat, communityCardValues, playerHandValues, totalWinningsArray):
        ...
        for i in xrange(len(playerHandValues)):
    ...
    def destroy(self):
        ...
        self.menu.destroy()
        del self.menu
        self.communityCardNode.remove_node()
        ...
        if self.table.wantMeter == 1:
            self.meter.remove_node()
            del self.meter
            self.meterBorder.remove_node()
            del self.meterBorder

        if self.table.wantMeter == 2:
            self.fulcrum.remove_node()
            del self.fulcrum
            self.balance.remove_node()
            del self.balance
