...
from panda3d.core import Fog, Light, VBase3, Vec3, Vec4
...
def forceLowLODOnAvatars():
    ...
    for item in base.cr.doId2do.items():
        ...
        if do.dclass.getName() == 'DistributedNPCTownfolk':
            do.forceLOD(0)
        if do.dclass.getName() == 'DistributedHoldemTable':
            do.dealer.forceLOD(0)
            for i in xrange(len(do.AIPlayers)):
                if do.AIPlayers[i]:
...
def Cutscene1_2ivals(cutscene):
    ...
    (subs, clearSubs) = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(hideInterceptorFunc, Sequence(Func(av.recordEvent, 'sendToBoat')), subs), Parallel(Sequence(showInterceptorFunc), clearSubs))
...
def Cutscene2_1_bivals(cutscene):
    ...
    cutlass.flattenStrong()
    (subs, clearSubs) = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(Func(av.attachHandheld, cutlass, True), Sequence(WaitFrames(80), Func(av.detachHandheld, cutlass)), Sequence(WaitFrames(206), Func(willTurner.spawnSkeletons, cutscene)), Sequence(WaitFrames(314), Func(cutcam.changeCameraParams, 0, 35)), Sequence(WaitFrames(346), Func(willTurner.attachHandheld, cutlass, True)), Sequence(WaitFrames(393), Func(base.transitions.fadeOut)), subs), Parallel(Func(willTurner.detachHandheld, cutlass, True), Func(forceInteract, TutorialGlobals.PIT_INTERIOR, 1), clearSubs))
...
def Cutscene2_4_bivals(cutscene):
    ...
    (subs, clearSubs) = subtitleSequence(cutscene.cutsceneName)
    return (Parallel(Sequence(WaitFrames(980), Func(base.transitions.fadeOut, 1.0)), subs), clearSubs)
...
def subtitleSequence(cutsceneId):
    ...
    if 'localAvatar' not in __builtins__:
        if not 'subtitler' in __builtins__:
