from panda3d.core import ModelNode, NodePath, Vec3
import math
import random
...
localFireSfxNames = [
    ...
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_FIRE_03),
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_FIRE_04),
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_FIRE_05)]
distFireSfxNames = [
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_DIST_FIRE_01),
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_DIST_FIRE_02),
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_DIST_FIRE_03),
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_DIST_FIRE_04),
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_DIST_FIRE_05),
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_DIST_FIRE_06),
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_DIST_FIRE_07),
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_DIST_FIRE_08),
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_DIST_FIRE_09),
    loadSfx(SoundGlobals.SFX_WEAPON_CANNON_DIST_FIRE_10)]
...
class DistributedShipBroadside(DistributedWeapon, DistributedShippart):
    ...
    def loadModel(self):
        for i in xrange(len(self.leftBroadsideConfig)):
        ...
        for i in xrange(len(self.rightBroadsideConfig)):
    ...
    def doBroadside(self, side, delays, hitPosList, zoneId, flightTime, timestamp = None, clientFire = 0):
        ...
        if side == 0:
            ...
            self.leftPlayShots = []
            for index in xrange(len(self.leftBroadside)):
                if self.leftBroadside[index]:
                    ...
                    playShot = Sequence(Wait(delays[index]), Func(self.setCannonAnim, index, 0, 0), Wait(0.5), Func(self._DistributedShipBroadside__requestAttack, index, side, targetPos, flightTime), Wait(3.0), Func(self.setCannonAnim, index, 0, 1))
                    playShot.start(ts)
            ...
            self.rightPlayShots = []
            for index in xrange(len(self.rightBroadside)):
                if self.rightBroadside[index]:
                    ...
                    playShot = Sequence(Wait(delays[index]), Func(self.setCannonAnim, index, 1, 0), Wait(0.5), Func(self._DistributedShipBroadside__requestAttack, index, side, targetPos, flightTime), Wait(3.0), Func(self.setCannonAnim, index, 1, 1))
                    playShot.start(ts)
