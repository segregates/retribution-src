from panda3d.core import CollideMask, CollisionNode, CollisionSphere, ModelRoot, NodePath, TextNode, Vec4
...
class DistributedCannonDefenseShip(DistributedNPCSimpleShip):
    ...
    def initHealthBar(self):
        self.healthBar = DirectWaitBar(frameSize = (-0.35, 0.35, -0.04, 0.04), relief = DGG.FLAT, frameColor = (0.0, 0.0, 0.0, 0.0), borderWidth = (0.0, 0.0), barColor = (0.0, 1.0, 0.0, 1.0), scale = 100)
    ...
    def initIndicatorIcons(self):
        ...
        self.goldStolenlbl = DirectLabel(parent = self.healthBar, relief = None, pos = (0, 0, 0.2), text_align = TextNode.ACenter, text_scale = 0.5, textMayChange = 0, text = '!', text_fg = PiratesGuiGlobals.TextFG23, text_shadow = PiratesGuiGlobals.TextShadow, text_font = PiratesGlobals.getPirateBoldOutlineFont(), sortOrder = 2)
        self.goldStolenlbl.setTransparency(1)
        self.goldStolenlbl.hide()
        self.hasGoldlbl = DirectLabel(parent = self.healthBar, relief = None, pos = (0, 0, 0.35), image = self.goldIcon, image_scale = 0.5, image_pos = (0, 0, 0), sortOrder = 2)
    ...
    def playSharkAttack(self, pos):
        if self.sharkActor.getCurrentAnim() == None:
            ...
            hprIvalShip = LerpHprInterval(self.getModelRoot(), duration = 3, hpr = (0, 0, -180))
