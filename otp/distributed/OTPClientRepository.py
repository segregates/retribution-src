...
from direct.gui.DirectGui import *
from direct.showbase import PythonUtil, BulletinBoardWatcher
...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def __init__(self, serverVersion, launcher = None, playGame = None):
        ...
        self._proactiveLeakChecks = config.GetBool('proactive-leak-checks', 1)
    ...
    def handleSystemMessage(self, di):
        ...
        if hasattr(base, 'talkAssistant'):
            ...
            if not self.systemMessageSfx:
                self.systemMessageSfx = base.loadSfx('audio/sfx/clock03.ogg')
    ...
    def enterPlayGame(self, hoodId, zoneId, avId):
        ...
        base.loadingScreen.tick(tickNumber = 100)
    ...
    def handleGenerateWithRequiredOwner(self, di):
        doId = di.getUint32()
        parentId = di.getUint32()
        zoneId = di.getUint32()
        classId = di.getUint16()
        dclass = self.dclassesByNumber[classId]
        dclass.startGenerate()
        distObj = self.generateWithRequiredOwner(dclass, doId, di)
        dclass.stopGenerate()

    def generateWithRequiredOwner(self, dclass, doId, di):
        ...
        distObj.generate()
        dclass.receiveUpdateBroadcastRequiredOwner(distObj, di)
