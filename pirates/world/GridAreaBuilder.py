from panda3d.core import AnimControl, AnimControlCollection, CollideMask, InternalName, LODNode, Light, NodePath, Plane, SceneGraphReducer, Vec3, autoBind
import random
import re
import types
...
class GridAreaBuilder(AreaBuilderBase.AreaBuilderBase):
    ...
    def addChildObj(self, levelObj):
        ...
        if objectCat not in ('PROP_OBJ', 'BUILDING_OBJ', 'Cell Portal Area'):
            loadableType = objectType in ('Dinghy', 'Holiday Object', 'PotionTable')
          #  if not (loadableType and 'Objects' in objData):
          #      return
        ...
        if 'Visual' in objData and 'Model' in objData['Visual']:
            ...
            if objData['Type'] == 'Collision Barrier':
                ...
                if newBitmasks and newBitmasks != [
                    'None']:
            ...
            if objectType not in self.LARGE_OBJECTS:
                ...
                if numGridLODs > numLODs:
                    ...
                    if lowestModel:
                        for i in xrange(numGridLODs - numLODs):
        ...
        if objModel is None:
            self.notify.warning("objModel is None!")
            return
