...
class DistributedNPCTownfolk(DistributedBattleNPC.DistributedBattleNPC, DistributedShopKeeper.DistributedShopKeeper, Townfolk.Townfolk):
    ...
    def __init__(self, cr):
        ...
        DistributedShopKeeper.DistributedShopKeeper.__init__(self, cr)
    ...
    def announceGenerate(self):
        ...
        if self.getHelpId():
            ...
            self.nametagIconGlow.reparentTo(NodePath(self.nametag.getNameIcon()))
    ...
    def firstNoticeLocalAvatar(self):
        ...
        if self.hasQuestOffersForLocalAvatar() and self.animSet not in [
            'tatoo_receive',
            'sit',
            'sit_sleep']:
    ...
    def hasOpenGUI(self):
        if self.interactGUI and self.questMenuGUI or self.shipNamePanel:
            return True
    ...
    def offerOptions(self, dialogFlag):
        ...
        if hasattr(self, 'currentDialogMovie') and self.purgeInteractGui or self.playingQuestString == True:
        ...
        if self.avatarType.isA(AvatarTypes.Musician):
            self.acceptOnce('stoppedShopping', self.cancelInteraction, [
                base.localAvatar])
    ...
    def confirmHealHp(self):
        ...
        self.confirmDialog = PDialog.PDialog(text = PLocalizer.HealHpConfirmDialog % {
            'gold': gold }, style = OTPDialog.YesNo, command = self._DistributedNPCTownfolk__handleHealHpConfirmation)


    def _DistributedNPCTownfolk__handleHealHpConfirmation(self, value):
        if self.confirmDialog:
            self.confirmDialog.destroy()
            self.confirmDialog = None
    ...
    def confirmHealMojo(self):
        ...
        self.confirmDialog = PDialog.PDialog(text = PLocalizer.HealMojoConfirmDialog % {
            'gold': gold }, style = OTPDialog.YesNo, command = self._DistributedNPCTownfolk__handleHealMojoConfirmation)


    def _DistributedNPCTownfolk__handleHealMojoConfirmation(self, value):
        if self.confirmDialog:
            self.confirmDialog.destroy()
            self.confirmDialog = None
    ...
    def confirmBribe(self):
        ...
        if avGold >= gold:
            self.confirmDialog = PDialog.PDialog(text = PLocalizer.BribeConfirmDialog % {
                'name': self.getName(),
                'gold': gold }, style = OTPDialog.YesNo, command = self._DistributedNPCTownfolk__handleBribeConfirmation)
        else:
            self.confirmDialog = PDialog.PDialog(text = PLocalizer.BribeNotEnoughGold % {
    ...
    def computeOptions(self):
        ...
        if avFishingLevel < 10 or localAvatar.onWelcomeWorld:
        ...
        buttonStateDict = {
            ...
            InteractGlobals.TALK: InteractGlobals.DISABLED,
            InteractGlobals.DUEL: InteractGlobals.DISABLED,
            InteractGlobals.STORE: storeButtonState,
            InteractGlobals.SELL_ITEMS: sellItemButtonState,
            InteractGlobals.SHIPS: shipButtonState,
            InteractGlobals.SELL_SHIPS: sellShipButtonState,
            InteractGlobals.TRAIN: InteractGlobals.DISABLED,
            InteractGlobals.REPAIR: repairButtonState,
            InteractGlobals.UPGRADE: InteractGlobals.DISABLED,
            InteractGlobals.TRADE: InteractGlobals.DISABLED,
            InteractGlobals.HEAL_HP: healButtonState,
            InteractGlobals.HEAL_MOJO: healMojoButtonState,
            InteractGlobals.CANCEL: InteractGlobals.NORMAL,
            InteractGlobals.SAIL: sailButtonState,
            InteractGlobals.SAILTM: sailTMButtonState,
            InteractGlobals.BRIBE: bribeButtonState,
            InteractGlobals.ACCESSORIES_STORE: InteractGlobals.NORMAL,
            InteractGlobals.TATTOO_STORE: InteractGlobals.NORMAL,
            InteractGlobals.JEWELRY_STORE: InteractGlobals.NORMAL,
            InteractGlobals.BARBER_STORE: InteractGlobals.NORMAL,
            InteractGlobals.RESPEC: respecButtonState,
            InteractGlobals.MUSICIAN: InteractGlobals.NORMAL,
            InteractGlobals.PVP_REWARDS_TATTOO: InteractGlobals.NORMAL,
            InteractGlobals.PVP_REWARDS_COATS: InteractGlobals.NORMAL,
            InteractGlobals.PVP_REWARDS_HATS: InteractGlobals.NORMAL,
            InteractGlobals.STOWAWAY: stowawayButtonState,
            InteractGlobals.PLAY_CANNON_DEFENSE: cannonDefenseButtonState,
            InteractGlobals.POTION_TUTORIAL: potionTutorialButtonState,
            InteractGlobals.LAUNCH_FISHING_BOAT: launchFishingButtonState,
            InteractGlobals.LEGENDARY_FISH_STORY: tellLegendaryFishStoryButtonState,
            InteractGlobals.UPGRADE_ROD: upgradeRodButtonState,
            InteractGlobals.CATALOG_STORE: InteractGlobals.NORMAL,
            InteractGlobals.PLAY_SCRIMMAGE: InteractGlobals.NORMAL }
        stateCodes = []
    ...
    def selectOption(self, optionId):
        if optionId in [
            InteractGlobals.STORE,
            InteractGlobals.TRAIN,
            InteractGlobals.SHIPS,
            InteractGlobals.UPGRADE,
            ...
            InteractGlobals.PVP_REWARDS_TATTOO,
            InteractGlobals.PVP_REWARDS_HATS,
            InteractGlobals.PVP_REWARDS_COATS,
            InteractGlobals.STOWAWAY]:
            if self.interactGUI:
                self.interactGUI.hide()
    ...
    def cancelInteraction(self, av, dialogStr = ''):
        if av == localAvatar:
            ...
            if av.playRewardAnimation:
                localAvatar.b_setGameState('OOBEmote', localArgs = [
                    av.playRewardAnimation[0],
                    av.playRewardAnimation[1]])
    ...
    def swordTutorialPt1(self):
        self.sendUpdate('swordTutorialPt1', [
            localAvatar.getDoId()])
        localAvatar.cameraFSM.request('FPS')


    def pistolTutorialPt1(self):
        self.sendUpdate('pistolTutorialPt1', [
            localAvatar.getDoId()])
    ...
    def drawSwordTutorial(self):
        self.drawSwordPanel = NewTutorialPanel([
            'drawSword'])
    ...
    def attackSwordTutorial(self):
        ...
        self.attackSwordPanel = NewTutorialPanel([
            'attackSword'])
    ...
    def handleShipNameChosen(self, shipName):
        ...
        self.sendUpdate('shipTutorialPt1', [
            avId,
            shipName])
    ...
    def levelUpCutlass(self):
        self.sendUpdate('levelUpCutlass', [
            localAvatar.getDoId()])
    ...
    def initializeNametag3d(self):
        ...
        if not self.classNameText:
            self.classNameText = OnscreenText(parent = self.iconNodePath, pos = (0, -2.5), fg = (1, 1, 1, 1), bg = (0, 0, 0, 0), scale = 0.9, mayChange = 1, font = PiratesGlobals.getPirateBoldOutlineFont())
    ...
    def showUpgradeRodDialog(self):
        ...
        if avGold < upgradeCost:
            rodFailText = PLocalizer.UpgradeRodFail % {
                'gold': str(upgradeCost),
                'rod': PLocalizer.FishingRodNames[rodLvl + 1] }
            self.confirmDialog = PDialog.PDialog(text = rodFailText, style = OTPDialog.Acknowledge, command = self.notEnoughMoney)
        else:
            upgradeRodText = PLocalizer.UpgradeRodConfirmDialog % {
                'gold': str(upgradeCost),
                'rod': PLocalizer.FishingRodNames[rodLvl + 1] }
            self.confirmDialog = PDialog.PDialog(text = upgradeRodText, style = OTPDialog.YesNo, command = self._DistributedNPCTownfolk__handleUpgradeRodConfirmation)
    ...
    def showLaunchFishingBoatDialog(self):
        ...
        if avGold < launchCost:
            ...
            launchBoatText = PLocalizer.LaunchFishingBoatConfirmDialog % {
                'gold': str(launchCost) }
            self.confirmDialog = PDialog.PDialog(text = launchBoatText, style = OTPDialog.YesNo, command = self._DistributedNPCTownfolk__handleLaunchFishingBoatConfirmation)
    ...
    def showRespecMenu(self):
        ...
        optionIds = [
            InteractGlobals.RESPEC_CUTLASS,
            InteractGlobals.RESPEC_PISTOL,
            InteractGlobals.RESPEC_DAGGER,
            InteractGlobals.RESPEC_DOLL,
            InteractGlobals.RESPEC_GRENADE,
            InteractGlobals.RESPEC_STAFF,
            InteractGlobals.RESPEC_SAILING,
            InteractGlobals.RESPEC_CANNON,
            InteractGlobals.BACK]
        stateCodes = []
    ...
    def isRespecAvailable(self, igOption):
        if igOption == InteractGlobals.CANCEL or igOption == InteractGlobals.BACK:
            ...
            for skillId in xrange(begin, end):
    ...
    def selectRespecOptionConfirm(self, optionId):
        ...
        respecText = PLocalizer.RespecConfirmDialog % {
            'gold': str(goldCost),
            'weapon': PLocalizer.InventoryTypeNames[weaponRep] }
        if numRespecs < 2:
            respecText += PLocalizer.RespecPriceIncreaseDialog

        self.confirmDialog = PDialog.PDialog(text = respecText, style = OTPDialog.YesNo, command = self._DistributedNPCTownfolk__handleRespecConfirmation)


    def _DistributedNPCTownfolk__handleRespecConfirmation(self, value):
        if self.confirmDialog:
            self.confirmDialog.destroy()
            self.confirmDialog = None
    ...
    def _DistributedNPCTownfolk__handleUpgradeRodConfirmation(self, value):
        if self.confirmDialog:
            self.confirmDialog.destroy()
            self.confirmDialog = None
    ...
    def _DistributedNPCTownfolk__handleLaunchFishingBoatConfirmation(self, value):
        if self.confirmDialog:
            self.confirmDialog.destroy()
            self.confirmDialog = None
    ...
    def canIdleSplash(self):
        if self.getCurrentAnim() == 'idle':
            pass
        return self.animProp == None
