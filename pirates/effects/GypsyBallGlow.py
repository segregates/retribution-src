...
class GypsyBallGlow(PooledEffect, EffectController):
    ...
    def __init__(self, effectParent = None, billboardOffset = 1.0):
        ...
        self.glowHalo.setColorScale(0.25, 0.200, 0.299, 0.75)
        self.fadeIval = None
        self.scaleIval = None


    def createTrack(self, lod = Options.SpecialEffectsHigh):
        self.glow.setColor(0.9, 0.848, 0.946, 0.946)
        if lod >= Options.SpecialEffectsHigh:
            fadeIn = self.glow.colorInterval(1.0, Vec4(0.848, 0.800000, 0.9, 0.9), startColor = Vec4(0.946, 0.9, 1, 1), blendType = 'easeInOut')
            fadeOut = self.glow.colorInterval(1.0, Vec4(0.946, 0.9, 1, 1), startColor = Vec4(0.848, 0.800000, 0.9, 0.9), blendType = 'easeInOut')
            self.fadeIval = Sequence(fadeIn, fadeOut)

        scaleUpHalo = self.glowHalo.scaleInterval(1.0, 1.0, startScale = 0.800000, blendType = 'easeInOut')
        scaleDownHalo = self.glowHalo.scaleInterval(1.0, 0.800000, startScale = 1.0, blendType = 'easeInOut')
