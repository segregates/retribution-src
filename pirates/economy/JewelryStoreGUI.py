...
class JewelryStoreCartList(DirectScrolledFrame):
    ...
    def addPanel(self, data, repack = 1):
        ...
        isDisabled = 0
        panel = DirectButton(parent = self, relief = None, text = itemText[:maxLength], text_fg = self.itemColor, text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleMed, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.06, 0.0), command = self.removePanel, extraArgs = [
            data])
        panel.costLabel = DirectLabel(parent = panel, relief = None, text = str(itemCost), text_fg = self.itemColor, text_align = TextNode.ARight, text_scale = PiratesGuiGlobals.TextScaleMed, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.45, 0.0), image = self.parent.CoinImage, image_scale = 0.15, image_pos = (0.48, 0.0, 0.014))
...
class JewelryStoreGUI(DirectFrame):
    ...
    def __init__(self, npc, shopId, **kw):
        ...
        self.balanceValue = DirectFrame(parent = self.cartFrame, relief = None, text = str(self.balance), text_fg = PiratesGuiGlobals.TextFG2, text_align = TextNode.ARight, text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (-0.055, 0.0), text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, image = self.CoinImage, image_scale = 0.15, image_pos = (-0.025, 0, 0.015), pos = (self.cartWidth, 0, 0.225))
        self.myGoldTitle = DirectFrame(parent = self.cartFrame, relief = None, text = PLocalizer.YourMoney, text_fg = PiratesGuiGlobals.TextFG2, text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (0.0, 0.0), text_shadow = PiratesGuiGlobals.TextShadow, pos = (0.09, 0, 0.155))
        self.myGold = DirectFrame(parent = self.cartFrame, relief = None, text = str(localAvatar.getMoney()), text_fg = PiratesGuiGlobals.TextFG2, text_align = TextNode.ARight, text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (-0.055, 0.0), text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, image = self.CoinImage, image_scale = 0.15, image_pos = (-0.025, 0, 0.015), pos = (self.cartWidth, 0, 0.155))
        ...
        self.accept(localAvatar.uniqueName('jewelryUpdate'), self.reloadPirateDNA, extraArgs = [
            self.pirate])
        ...
        self.equipRequests = {
            ...
            JewelryGlobals.LEAR: None,
            JewelryGlobals.REAR: None,
            JewelryGlobals.NOSE: None,
            JewelryGlobals.MOUTH: None,
            JewelryGlobals.LHAND: None,
            JewelryGlobals.RHAND: None }
        self.initTabs()
        self.updateBalance()
    ...
    def aspectRatioChange(self):
        ...
        height = 0.074 * aspect2d.getScale()[2]
    ...
    def destroy(self):
        ...
        if self.jewelerIconsA:
            self.jewelerIconsA.remove_node()
            self.jewelerIconsA = None

        if self.jewelerIconsB:
            self.jewelerIconsB.remove_node()
    ...
    def updateBalance(self, extraArgs = None):
        ...
        if self.balance > localAvatar.getMoney() or transactions == 0:
            ...
            self.balanceValue['text_fg'] = PiratesGuiGlobals.TextFG2
            self.commitButton['state'] = DGG.NORMAL
        if base.localAvatar.getGoldInPocket() < self.balance or self.purchaseInventory.panels == []:
    ...
    def setPage(self, pageName, startIndex = 0, refreshWardrobe = True):
        self.tabBar.unstash()
        self.titleLabel['text'] = '\x01smallCaps\x01' + self.rootTitle + ' - ' + PLocalizer.JewelryNames.get(pageName)
            ...
            if self.currentWardrobe:
                for itemInfo in self.currentWardrobe:
                    ...
                    if type == jewelryType:
                        ...
                        if location in xrange(Locations.RANGE_EQUIP_JEWELRY[0], Locations.RANGE_EQUIP_JEWELRY[1]):
                            equipped = True
        ...
        for jewel in jewelry:
            if self.jewelryAmount - startIndex < self.buttonsPerPage and self.jewelryAmount >= startIndex:
                ...
                elif questDrop is True and self.mode == SELLING and not equipped:
                    jewelryButton.itemText['text'] = PLocalizer.ShopQuestItem
                else:
                    jewelryButton.itemText['text'] = ''
                jewelryButton.addToCart = GuiButton.GuiButton(command = self.addToCart, parent = jewelryButton, text_fg = PiratesGuiGlobals.TextFG2, text_pos = (0.0, -0.01), text_scale = PiratesGuiGlobals.TextScaleLarge, text_align = TextNode.ACenter, text_shadow = PiratesGuiGlobals.TextShadow, image_color = buttonColorB, pos = (0.16, 0.0, 0.055))
                jewelryButton.addToCart['extraArgs'] = [
                    jewelryButton,
                    type,
                    uid,
                    location]
                jewelryButton.bind(DGG.ENTER, self.highlightJewelryStart, extraArgs = [
                    self.jewelryAmount])
                jewelryButton.bind(DGG.EXIT, self.highlightJewelryStop, extraArgs = [
                    self.jewelryAmount])
                if self.mode == BUYING and not owned:
    ...
    def setupDisplayRegions(self, regionData, pageName):
        ...
        if x < self.buttonsPerPage:
            for y in xrange(self.buttonsPerPage - x):
