...
class InventoryUICardContainer(InventoryUIStackContainer.InventoryUIStackContainer):
    ...
    def takeOut(self):
        ...
        if not self.isReady:
            ...
            for itemId in self.itemList:
                self.accept('inventoryQuantity-%s-%s' % (localAvatar.getInventory().doId, itemId), self.handleUpdateCardItem, extraArgs = [
                    itemId])
    ...
    def clearOut(self):
        ...
        self.cardGroup.getChildren().detach()
    ...
    def placeCardItem(self, cardId):
        ...
        if cardCell.inventoryItem:
            cardCell.inventoryItem.remove_node()
