...
class QuestReward(POD):
    ...
    makeFromStruct = staticmethod(exceptionLogged()(makeFromStruct))
    ...
    getDescriptionText = staticmethod(getDescriptionText)

    def getItemId():
        pass

    getItemId = staticmethod(getItemId)
...
class PlayingCardReward(QuestReward):
    ...
    def applyTo(self, trade, av):
        ...
        trade.setSuccessCallback(av.giveCardMessage, extraArgs = [
            InventoryType.begin_Cards + cardId])
...
class CollectReward(QuestReward):
    ...
    def applyTo(self, trade, av):
        if self.amount >= InventoryType.begin_Collections and self.amount < InventoryType.end_Collections:
            ...
            for i in xrange(CollectionMap.Collection_Set_Sizes[self.amount]):
...
class PistolUpgradeReward(QuestReward):
    ...
    def applyTo(self, trade, av):
        ...
        if itemId:
            ...
            trade.setSuccessCallback(av.giveWeaponMessage, extraArgs = [
                self.amount])
            trade.setFailureCallback(av.giveFreeInventoryMessage, extraArgs = [
...
class JewelryQuestReward(QuestReward):
    ...
    def applyTo(self, trade, av):
        ...
        trade.setSuccessCallback(av.giveJewelryMessage, extraArgs = [
            uid])
        trade.setFailureCallback(av.giveFreeInventoryMessage, extraArgs = [
...
class TattooQuestReward(QuestReward):
    ...
    def applyTo(self, trade, av):
        ...
        if tattooUIDs:
            trade.setSuccessCallback(av.giveTattooMessages, extraArgs = [
                tattooUIDs])
            trade.setFailureCallback(av.giveFreeInventoryMessage, extraArgs = [
...
class ClothingQuestReward(QuestReward):
    ...
    def applyTo(self, trade, av):
        ...
        trade.setSuccessCallback(av.giveClothingMessage, extraArgs = [
            dropId,
            colorId])
        trade.setFailureCallback(av.giveFreeInventoryMessage, extraArgs = [
