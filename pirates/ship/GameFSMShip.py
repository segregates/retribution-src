from panda3d.physics import ActorNode, PhysicsObject
from panda3d.core import CollideMask, CollisionNode, CollisionSphere, ModelRoot, NodePath, Point3, Vec3, Vec4
import random
...
class GameFSMShip(FSM.FSM):
    ...
    def exitOff(self):
        messenger.send('shipStateOn-%s' % self.ship.doId, [
            self.ship])
    ...
    def initAudio(self):
        ...
        if base.config.GetBool('want-sailing-music-d', False):
            self.currentMusic = random.choice((SoundGlobals.MUSIC_SAILING_A, SoundGlobals.MUSIC_SAILING_B, SoundGlobals.MUSIC_SAILING_C, SoundGlobals.MUSIC_SAILING_D, SoundGlobals.MUSIC_SAILING_E))
        else:
