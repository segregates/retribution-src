from panda3d.core import Light, TextNode, Texture
...
class LookoutRequestLVL1(InventoryPage, InventoryRequestGameType):
    ...
    def submitRequest(self):
        ...
        def finishSubmitRequest(options, shipIds):
            self.searchParams = {
                'cat': GameTypeGlobals.getGameTypeString(gameCat, 'type'),
                'type': GameTypeGlobals.getGameTypeString(gameType, 'style', category = gameCat),
                'catId': gameCat,
                'typeId': gameType,
                'opts': options }
            selectedVal = gameCat
    ...
    def getItemList(self):
        ...
        for currGameType in allGameTypes:
            ...
            itemList.append({
                'Text': GameTypeGlobals.getGameTypeString(currGameType, 'type'),
                'Icon': GameTypeGlobals.getGameTypeString(currGameType, 'icon'),
                'Value': currGameType })
    ...
    def createNewItem(self, item, parent, itemType = None, columnWidths = [], color = None):
        ...
        elif item['Value'] == PiratesGlobals.GAME_TYPE_CREW and item['Value'] == PiratesGlobals.GAME_TYPE_PRIV or item['Value'] == PiratesGlobals.GAME_TYPE_PVP:
        ...
        def gotStylz(gameStylz):
            ...
            if gameStylz:
                for currStyle in gameStylz:
                    styleName = (GameTypeGlobals.getGameTypeString(currStyle, 'style', category),)
                    desc = (GameTypeGlobals.getGameTypeString(currStyle, 'descriptionStyle', category),)
                    availItems.append({
                        'Text': styleName,
                        'Desc': desc,
                        'Icon': GameTypeGlobals.getGameTypeString(currStyle, 'iconStyle'),
                        'Value': currStyle })
    ...
    def _determineTitle(self, mode):
        ...
        if mode == PiratesGuiGlobals.SEARCH_MODE:
            ...
            return (PLocalizer.LookoutPanelStatus, PLocalizer.LookoutPanelStatusDesc, iconName)
        elif mode == PiratesGuiGlobals.INVITE_MODE:
            iconName = GameTypeGlobals.GameTypeStrings['icon'][self.searchParams['catId']]
            return (PLocalizer.LookoutPanelInvite, PLocalizer.LookoutPanelInviteDesc, iconName)
        elif mode == PiratesGuiGlobals.REQUEST_OPT_MODE:
            iconName = GameTypeGlobals.GameTypeStrings['icon'][selectedVal]
            return (PLocalizer.LookoutOptionsTitle, PLocalizer.LookoutOptionsDesc, iconName)
        elif mode == PiratesGuiGlobals.REQUEST_FOUND_MODE:
            iconName = GameTypeGlobals.GameTypeStrings['icon'][selectedVal]
            return (PLocalizer.LookoutPanelJoin, PLocalizer.LookoutPanelJoinDesc, iconName)
        elif mode == PiratesGuiGlobals.REQUEST_JOIN_MODE:
            iconName = GameTypeGlobals.GameTypeStrings['icon'][selectedVal]
            return (PLocalizer.LookoutPanelJoined, PLocalizer.LookoutPanelJoinedDesc, iconName)
        elif mode == PiratesGuiGlobals.REQUEST_TYPE_DIRECT_MODE:
            iconName = GameTypeGlobals.GameTypeStrings['icon'][selectedVal]
            return (PLocalizer.LookoutChallengeTitle, PLocalizer.LookoutChallengeDesc, iconName)
        elif mode == PiratesGuiGlobals.CHALLENGE_MODE:
            iconName = GameTypeGlobals.GameTypeStrings['icon'][selectedVal]
            return (PLocalizer.LookoutSkirmishTitle, PLocalizer.LookoutSkirmishDesc, iconName)
    ...
    def matchFound(self, matchId, timeToJoin = PiratesGlobals.LOOKOUT_JOIN_TIMEOUT):
        if self.currMode == PiratesGuiGlobals.SEARCH_MODE and self.currMode == PiratesGuiGlobals.INVITE_MODE and self.currMode == PiratesGuiGlobals.CHALLENGE_MODE or self.currMode == PiratesGuiGlobals.INVITE_ACCEPTED_MODE:
            if not self.currMode == PiratesGuiGlobals.INVITE_MODE and self.currMode == PiratesGuiGlobals.CHALLENGE_MODE:
                pass
            inviting = self.currMode == PiratesGuiGlobals.INVITE_ACCEPTED_MODE
            if inviting == False and self.guiMgr.isSeaChestAllowed():
                ...
                self.confirmMsg = self.guiMgr.messageStack.addModalTextMessage(description, buttonStyle = OTPDialog.YesNo, yesCallback = lambda param1 = True, param2 = self.searchParams['type']: self.quickConfirm(param1, param2), noCallback = lambda param1 = False: self.quickConfirm(param1), icon = ('lookout', None))
    ...
    def requestInvite(self, inviterName, activityCategory, activityType, options):
        ...
        elif activityCategory == PiratesGlobals.GAME_TYPE_TM and self.guiMgr.crewHUD.crew:
        ...
        if self.guiMgr.isSeaChestAllowed():
            ...
            self.confirmMsg = self.guiMgr.messageStack.addModalTextMessage(description, buttonStyle = OTPDialog.YesNo, yesCallback = lambda param1 = True, param2 = activityType: self.quickConfirm(param1, param2), noCallback = lambda param1 = False: self.quickConfirm(param1), icon = ('lookout', None))
