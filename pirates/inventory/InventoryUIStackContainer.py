...
class InventoryUIStackContainer(InventoryUIContainer.InventoryUIContainer):
    ...
    def figureOutStackTypes(self):
        ...
        for itemId in self.itemList:
            ...
            if self.isSkillValid(itemId):
                if not skillRepId in repCategoryDict:
    ...
    def setupCells(self):
        ...
        for Z in xrange(self.gridZ):
            for X in xrange(len(self.listOfItemLists[Z])):
    ...
    def doSetup(self):
        self.setupCells()
        self.titleLabel = DirectLabel(relief = None, parent = self, textMayChange = 1, text = self.titleText, text_fg = (1, 1, 1, 1), text_font = PiratesGlobals.getPirateBoldOutlineFont(), text_scale = self.textScale, text_align = TextNode.ACenter, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.0, 0.0))
