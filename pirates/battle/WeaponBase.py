...
class WeaponBase(WeaponBaseBase.WeaponBaseBase):
    ...
    def projectileHitObject(self, entry):
        ...
        if objType == PiratesGlobals.COLL_AV or objType == PiratesGlobals.COLL_MONSTER:
            ...
            if avId:
                avId = int(avId)
                self._WeaponBase__avatarHit(avId, hitObject, entry, skillId, ammoSkillId)
            ...
            if self.localAvatarUsingWeapon:
                self._WeaponBase__shippartHit(hitObject, entry, skillId, ammoSkillId)

        elif objType == PiratesGlobals.COLL_SEA:
            self._WeaponBase__waterHit(hitObject, entry, skillId, ammoSkillId)
        elif objType == PiratesGlobals.COLL_LAND:
            groundId = hitObject.getNetTag('groundId')
            if groundId:
                groundId = int(groundId)
                self._WeaponBase__groundHit(groundId, hitObject, entry, skillId, ammoSkillId)

        elif objType == PiratesGlobals.COLL_BLDG:
            self._WeaponBase__buildingHit(hitObject, entry, skillId, ammoSkillId)
        elif objType == PiratesGlobals.COLL_SHIP_WRECK:
            propId = int(hitObject.getNetTag('propId'))
            self._WeaponBase__shipWreckHit(propId, hitObject, entry)
        elif objType == PiratesGlobals.COLL_GRAPPLE_TARGET:
            shipId = hitObject.getNetTag('shipId')
            if shipId:
                shipId = int(shipId)
                self._WeaponBase__grappleTargetHit(shipId, hitObject, entry, skillId, ammoSkillId)

        elif objType == PiratesGlobals.COLL_CANNON:
            cannonId = hitObject.getNetTag('cannonId')
            if cannonId:
                cannonId = int(cannonId)
                self._WeaponBase__cannonHit(cannonId, hitObject, entry, skillId, ammoSkillId)

            broadsideId = hitObject.getNetTag('broadsideId')
            if broadsideId:
                self._WeaponBase__cannonPortHit(hitObject, broadsideId)

        elif objType == PiratesGlobals.COLL_FORT:
            fortId = hitObject.getNetTag('fortId')
            if fortId:
                fortId = int(fortId)
                self._WeaponBase__fortHit(fortId, hitObject, entry, skillId, ammoSkillId)
            ...
            self._WeaponBase__avatarHit(obj.doId, hitObject, entry, skillId, ammoSkillId)
    ...
    def _WeaponBase__shipHit(self, shipId, hitObject, entry, skillId, ammoSkillId):
    ...
    def _WeaponBase__buildingHit(self, hitObject, entry, skillId, ammoSkillId):
        pass


    def _WeaponBase__shipWreckHit(self, propId, hitObject, entry):
    ...
    def _WeaponBase__waterHit(self, hitObject, entry, skillId, ammoSkillId):
        pass
    ...
    def _WeaponBase__cannonHit(self, cannonId, hitObject, entry, skillId, ammoSkillId):
        pos = entry.getFromNodePath().getParent().getPos()
        cannon = self.cr.doId2do.get(cannonId)
        cannon.sendHitByProjectile()


    def _WeaponBase__fortHit(self, fortId, hitObject, entry, skillId, ammoSkillId):
        if self.localAvatarUsingWeapon:
    ...
    def _WeaponBase__cannonPortHit(self, hitObject, broadsideId):
        if broadsideId:
    ...
    def sendShipAttack(self, skillId, ammoSkillId, attackerId, pos, normal, hullCode, mastCode, timestamp32):
        self.sendUpdate('processShipAttack', [
            skillId,
            ammoSkillId,
            attackerId,
            pos,
