from panda3d.core import Buffer, Camera, CardMaker, DisplayRegion, Lens, Mat4, NodePath, PerspectiveLens, TextNode, Texture, TransformState, Vec3, Vec4
...
class InventoryUIClothingItem(InventoryUIItem.InventoryUIItem):
    ...
    def showDetails(self, cell, detailsPos, detailsHeight, event = None):
        ...
        subtitleLabel = DirectLabel(parent = self, relief = None, text = '\x01slant\x01%s\x02 %s' % (rarityText, typeText), text_scale = subtitleScale, text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, text_align = TextNode.ACenter, pos = (0.0, 0.0, runningVertPosition), text_pos = (0.0, -textScale))
        subtHeight = 0.05
        ...
        goldLabel = DirectLabel(parent = self, relief = None, image = coinImage, image_scale = 0.12, image_pos = Vec3(0.025, 0, -0.02), text = str(value), text_scale = subtitleScale, text_align = TextNode.ARight, text_fg = PiratesGuiGlobals.TextFG1, text_shadow = PiratesGuiGlobals.TextShadow, pos = (halfWidth - 0.05, 0.0, runningVertPosition + 0.08), text_pos = (0.0, -textScale))
