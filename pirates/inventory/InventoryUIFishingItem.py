from panda3d.core import Buffer, Camera, CardMaker, DisplayRegion, Lens, NodePath, PerspectiveLens, TextNode, Texture, Vec4
...
class InventoryUIFishingItem(InventoryUIStackItem.InventoryUIStackItem):
    ...
    def __init__(self, manager, skillId, itemTuple, imageScaleFactor = 1.0, showMax = 1, update = False):
        ...
        self.initialiseoptions(InventoryUIFishingItem)
        self.iconName = self.FishingItemTable.get(skillId)
        iconAsset = self.Icons.find('**/%s' % self.iconName)
        self['image'] = iconAsset
        self.helpFrame = None
    ...
    def destroy(self):
        ...
        if self.portraitSceneGraph:
            self.portraitSceneGraph.remove_node()
    ...
    def getName(self):
        return PLocalizer.InventoryTypeNames.get(self.skillId)
    ...
    def updateAmount(self, caller = None):
        ...
        skillQuantity = inventory.getStackQuantity(self.getSkillId())
        if skillQuantity >= 2:
    ...
    def showDetails(self, cell, detailsPos, detailsHeight, event = None):
        ...
        self.iconLabel = DirectLabel(parent = self.portraitSceneGraph, relief = None, image = self.Icons.find('**/%s' % self.iconName), pos = (0.0, 2.5, 0.0))
        iHeight = 0.18
        self.createBuffer()
