from panda3d.core import AudioSound, NodePath, Point3
import random
import math
...
class RepairPitchingGame(RepairMincroGame):
    ...
    def _initAudio(self):
        ...
        self.pitchSounds = (loadSfx(SoundGlobals.SFX_MINIGAME_REPAIR_PITCH_PLUG01), loadSfx(SoundGlobals.SFX_MINIGAME_REPAIR_PITCH_PLUG02), loadSfx(SoundGlobals.SFX_MINIGAME_REPAIR_PITCH_PLUG03))
        self.leakSounds = (loadSfx(SoundGlobals.SFX_MINIGAME_REPAIR_PITCH_LEAK01), loadSfx(SoundGlobals.SFX_MINIGAME_REPAIR_PITCH_LEAK02), loadSfx(SoundGlobals.SFX_MINIGAME_REPAIR_PITCH_LEAK03), loadSfx(SoundGlobals.SFX_MINIGAME_REPAIR_PITCH_LEAK04))
    ...
    def reset(self):
        ...
        self.clearLeaks()
        for i in xrange(self.leakCount):
    ...
    def destroy(self):
        ...
        del self.locators
        self.board.remove_node()
        del self.board
        self.crossHair.remove_node()
        del self.crossHair
        self.bucket.remove_node()
        del self.bucket
        self.bucketIdle.remove_node()
        del self.bucketIdle
        if self.missSeq is not None:
            self.missSeq.finish()

        del self.missSeq
        self.missPatch.remove_node()
        del self.missPatch
