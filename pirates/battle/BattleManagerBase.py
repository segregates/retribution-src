...
class BattleManagerBase:
    ...
    def willWeaponHit(self, attacker, target, skillId, ammoSkillId, charge):
        ...
        for targetEffect in skillEffects:
            ...
            if not attacker.isNpc:
                if targetEffect in [
                    WeaponGlobals.C_ACCURACY_BONUS_LVL1,
                    WeaponGlobals.C_ACCURACY_BONUS_LVL2,
                    WeaponGlobals.C_ACCURACY_BONUS_LVL3]:
    ...
    def getModifiedSkillEffects(self, attacker, target, skillId, ammoSkillId, charge = 0, distance = 0.0):
        ...
        randVal = attacker.battleRandom.getRandom('getModifiedSkillEffect %s %s' % (attacker, target), range = [
            50,
            100])
        if attacker.isNpc:
            minValue = -1
        ...
        if not (attacker.isNpc) and not WeaponGlobals.isSelfUseSkill(skillId):
        ...
        if not WeaponGlobals.isSelfUseSkill(skillId):
            ...
            if hasattr(attacker, 'currentWeaponId'):
                ...
                if distanceModifiers and WeaponGlobals.getAttackClass(skillId) != WeaponGlobals.AC_COMBAT:
            ...
            if not attacker.isNpc:
                ...
                for buff in skillEffects:
                    if not buff in [
                        ...
                        WeaponGlobals.C_CANNON_DAMAGE_LVL2,
                        WeaponGlobals.C_CANNON_DAMAGE_LVL3] or repId == InventoryType.CannonRep:
                        if not buff in [
                            WeaponGlobals.C_PISTOL_DAMAGE_LVL1,
                            WeaponGlobals.C_PISTOL_DAMAGE_LVL2,
                            WeaponGlobals.C_PISTOL_DAMAGE_LVL3] or repId == InventoryType.PistolRep:
                            if (buff in [
                                WeaponGlobals.C_CUTLASS_DAMAGE_LVL1,
                                WeaponGlobals.C_CUTLASS_DAMAGE_LVL2,
                                WeaponGlobals.C_CUTLASS_DAMAGE_LVL3] or repId == InventoryType.CutlassRep or buff in [
                                WeaponGlobals.C_DOLL_DAMAGE_LVL1,
                                WeaponGlobals.C_DOLL_DAMAGE_LVL2,
                                WeaponGlobals.C_DOLL_DAMAGE_LVL3]) and repId == InventoryType.DollRep:
        ...
        if target and not WeaponGlobals.isSelfUseSkill(skillId):
            if hasattr(target, 'getSkillEffects'):
                ...
                if WeaponGlobals.C_WARDING in skillEffects and not WeaponGlobals.isHealingSkill(skillId):
                    if attacker.isNpc or inPVPMode:
        ...
        if target and not WeaponGlobals.isSelfUseSkill(skillId) and not inPVPMode:
            if hasattr(target, 'avatarType'):
                ...
                if avClass == EnemyGlobals.MONSTER:
                    ...
                    if buff == WeaponGlobals.C_MONSTER_KILLER and ammoBuff == WeaponGlobals.C_MONSTER_KILLER and buff == WeaponGlobals.C_UNDEAD_KILLER or ammoBuff == WeaponGlobals.C_UNDEAD_KILLER:
                        oldTHealth = tHealth
                        tHealth = min(-1, tHealth * WeaponGlobals.RESIST_DAMAGE_PENALTY)
                        if self.wantOutput:
        ...
        if target and hasattr(attacker, 'currentWeaponId'):
            if tHealth < 0:
                ...
                if ItemGlobals.getWeaponAttributes(attacker.currentWeaponId, ItemGlobals.LEECH_VOODOO) and hasattr(target, 'currentWeaponId'):
                    if target.avatarType.isA(AvatarTypes.Pirate) and target.avatarType.isA(AvatarTypes.Muck) and target.avatarType.isA(AvatarTypes.Cadaver) and target.avatarType.isA(AvatarTypes.Splatter) and target.avatarType.isA(AvatarTypes.Drench) or target.avatarType.isA(AvatarTypes.JollyRoger):
                        mojo = target.getMojo()
    ...
    def getModifiedSkillEffectsSword(self, attacker, target, skillId, ammoSkillId, charge = 0, distance = 0.0):
        ...
        itemEffects = []
        inPVPMode = self.isPVP(attacker, target)
        randVal = attacker.battleRandom.getRandom('getModifiedSkillEffectSword %s' % tHealth, range = [
            50,
            100])
            ...
            if not attacker.isNpc:
                ...
                for buff in skillEffects:
                    if buff in [
                        WeaponGlobals.C_CUTLASS_DAMAGE_LVL1,
                        WeaponGlobals.C_CUTLASS_DAMAGE_LVL2,
                        WeaponGlobals.C_CUTLASS_DAMAGE_LVL3] and repId == InventoryType.CutlassRep:
                        potionMultiplier += PotionGlobals.getPotionPotency(buff)
                        continue
        ...
        if hasattr(target, 'getSkillEffects'):
            ...
            if WeaponGlobals.C_WARDING in skillEffects:
                if attacker.isNpc or inPVPMode:
                    tHealth = min(-1, tHealth - tHealth * WeaponGlobals.WARDING_PENALTY)
        ...
        if not WeaponGlobals.isSelfUseSkill(skillId) and target and not WeaponGlobals.isFriendlyFire(skillId, ammoSkillId):
            if not hasattr(attacker, 'inInvasion') or not attacker.isInInvasion():
                if (not hasattr(target, 'inInvasion') or not target.isInInvasion()) and not isKraken(target):
                    damageMod = WeaponGlobals.getComparativeLevelDamageModifier(attacker, target)
                    if inPVPMode:
                        damageMod = 1
    ...
    def addStats(self, attackerStats, targetStats, attackerBonusStats, targetBonusStats):
        for i in xrange(len(attackerStats)):
            if attackerStats[i] != 0:
                attackerStats[i] += attackerBonusStats[i]

        for i in xrange(len(targetStats)):
            if targetStats[i] != 0:
    ...
    def getModifiedHullDamage(self, attacker, target, skillId, ammoSkillId):
        ...
        return (finalDamage, itemEffects)
    ...
    def getModifiedShipDamage(self, attacker, target, skillId, ammoSkillId, damage):
        ...
        if WeaponGlobals.getAttackClass(skillId) == WeaponGlobals.AC_CANNON or (WeaponGlobals.getAttackClass(skillId) == WeaponGlobals.AC_GRENADE and skillId == EnemySkills.LEFT_BROADSIDE or skillId == EnemySkills.RIGHT_BROADSIDE):
            amt = attacker.getSkillRankBonus(InventoryType.CannonBarrage)
            amt += 1.0
            damage *= amt
        ...
        if hasattr(attacker, 'getSkillEffects'):
            ...
            for buff in skillEffects:
                ...
                if buff in [
                    WeaponGlobals.C_CANNON_DAMAGE_LVL1,
                    WeaponGlobals.C_CANNON_DAMAGE_LVL2,
                    WeaponGlobals.C_CANNON_DAMAGE_LVL3]:
        ...
        if hasattr(attacker, 'ship') and attacker.ship:
            ...
            for buff in skillEffects:
                if buff == WeaponGlobals.C_INCOMING:
                    if skillId == EnemySkills.LEFT_BROADSIDE or skillId == EnemySkills.RIGHT_BROADSIDE:
        ...
        if target:
            if (hasattr(attacker, 'getNPCship') or attacker.getNPCship() or hasattr(target, 'getNPCship')) and target.getNPCship():
    ...
    def getModifiedExperienceGrade(self, attacker, target, skillId = 0, ammoSkillId = 0, currentWeaponId = 0, repId = 0, forColor = False):
        ...
        if not (attackerLevel and targetLevel) or not inv:
            return 0
