    ...
    def generateFireworkIval(self):
        if not self.fireworkIval:
            ...
            if self.typeId == FireworkType.BasicPeony:
                firework = FireworkEffect(FireworkBurstType.PeonyShell, FireworkTrailType.Polygonal, self.velocity, self.scale, self.primaryColor, self.secondaryColor, self.burstDelay)
                ...
                firework = FireworkEffect(FireworkBurstType.PeonyParticleShell, FireworkTrailType.Polygonal, self.velocity, self.scale, self.primaryColor, self.secondaryColor, self.burstDelay)
                firework.reparentTo(self)
                self.fireworkEffects.append(firework)
                self.fireworkIval.append(firework.getFireworkMainIval())
            elif self.typeId == FireworkType.DiademPeony:
                firework = FireworkEffect(FireworkBurstType.PeonyDiademShell, FireworkTrailType.Polygonal, self.velocity, self.scale, self.primaryColor, self.secondaryColor, self.burstDelay)
                firework.reparentTo(self)
                self.fireworkEffects.append(firework)
                self.fireworkIval.append(firework.getFireworkMainIval())
                ...
                firework = FireworkEffect(FireworkBurstType.ChrysanthemumDiademShell, FireworkTrailType.Polygonal, self.velocity, self.scale, self.primaryColor, self.secondaryColor, self.burstDelay)
                firework.reparentTo(self)
                self.fireworkEffects.append(firework)
                self.fireworkIval.append(firework.getFireworkMainIval())
                ...
                firework.reparentTo(self)
                firework.gravityMult = 4.0
                self.fireworkEffects.append(firework)
                self.fireworkIval.append(firework.getFireworkMainIval())
            elif self.typeId == FireworkType.Mickey:
                head = FireworkEffect(FireworkBurstType.PeonyShell, FireworkTrailType.Glow, velocity = Vec3(0, 0, 425 * self.scale), scale = self.scale / 1.2, primaryColor = self.primaryColor, secondaryColor = self.secondaryColor, burstDelay = 1.5)
                leftEar = FireworkEffect(FireworkBurstType.PeonyShell, FireworkTrailType.Glow, velocity = Vec3(-175 * self.scale, 0, 650 * self.scale), scale = self.scale / 2.0, primaryColor = self.primaryColor, secondaryColor = self.secondaryColor, burstDelay = 1.7)
                rightEar = FireworkEffect(FireworkBurstType.PeonyShell, FireworkTrailType.Glow, velocity = Vec3(175 * self.scale, 0, 650 * self.scale), scale = self.scale / 2.0, primaryColor = self.primaryColor, secondaryColor = self.secondaryColor, burstDelay = 1.7)
                ...
                colors = [
                    ...
                    Vec4(1, 1, 1, 1)]
                for i in xrange(4):
                    firework = FireworkEffect(None, FireworkTrailType.LongGlowSparkle, velocity = Vec3(-150 * self.scale, 0, 350 * self.scale - 20 * i), scale = self.scale * 2.0, primaryColor = colors[i % 2], burstDelay = 2.5)
                    firework.reparentTo(self)
                    firework.setX(-200.0 * self.scale + 100.0 * i * self.scale)
                    self.fireworkEffects.append(firework)
                    fireworkParallel.append(Sequence(Wait(0.25 * i), firework.getFireworkMainIval()))

                firework = FireworkEffect(FireworkBurstType.Sparkles, FireworkTrailType.Default, velocity = Vec3(-150, 0, 450), scale = self.scale * 1.5)
                firework.reparentTo(self)
                self.fireworkEffects.append(firework)
