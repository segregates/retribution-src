...
class InventoryUIBeltGrid(InventoryUIContainer.InventoryUIContainer):
    ...
    def __init__(self, manager, sizeX = 1.0, sizeZ = 1.0, countX = 1, countZ = 1, hotkeyList = []):
        ...
        self.freeWeaponInfo = DirectLabel(parent = self, relief = None, textMayChange = 0, text = PLocalizer.FreeWeaponFromBlackSmith, text_font = PiratesGlobals.getPirateBoldOutlineFont(), text_fg = PiratesGuiGlobals.TextFG2, text_scale = 0.04, text_align = TextNode.ACenter, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0, 0))
        self.freeWeaponInfo.setPos(0.35, 0, -0.1)
        self.freeWeaponInfo.show()
        self.overflowInfo = DirectLabel(parent = self, relief = None, textMayChange = 0, text = PLocalizer.OverflowHint, text_font = PiratesGlobals.getPirateBoldOutlineFont(), text_fg = PiratesGuiGlobals.TextFG2, text_scale = 0.04, text_align = TextNode.ACenter, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0, 0))
    ...
    def manageCells(self, hotkeyList):
        ...
        for index in xrange(len(self.hotkeyList)):
            ...
            self.accept(cell.boundEvent, self.useHotkey, extraArgs = [
                cell])
            cell.label.show()
    ...
    def useHotkey(self, cell):
        if cell.inventoryItem:
            messenger.send('Use_Weapon_Request', [
                cell.inventoryItem.getId(),
                cell.slotId])
    ...
    def postUpdate(self, cell):
        if cell and cell.slotId:
            ...
            elif cell.slotId in Locations.RANGE_EQUIP_ITEMS:
            ...
            if cell.slotId in xrange(Locations.RANGE_EQUIP_WEAPONS[0], Locations.RANGE_EQUIP_WEAPONS[1] + 1):
