from panda3d.core import Buffer, Camera, CardMaker, DisplayRegion, Lens, NodePath, PerspectiveLens, Point3, TextNode, Texture, VBase4, Vec3, Vec4, lookAt
...
class SimpleStoreColorPicker(BorderFrame):
    ...
    def __init__(self, store, parentFrame, item, **kw):
        optiondefs = (('state', DGG.DISABLED, None), ('frameSize', (-0.5 * self.sizeX, 0.5 * self.sizeX, -0.5 * self.sizeZ, 0.5 * self.sizeZ), None), ('modelName', 'pir_m_gui_frm_subframe', None), ('imageColorScale', VBase4(0.75, 0.75, 0.9, 0.75), None))
        self.defineoptions(kw, optiondefs)
...
class SimpleStoreGUI(DirectFrame):
    ...
    holidayIdList = []
    itemGlobalsClasses = {
        InventoryType.ItemTypeClothing: SimpleClothingItem,
        InventoryType.ItemTypeJewelry: SimpleJewelryItem,
        InventoryType.ItemTypeTattoo: SimpleTattooItem,
        InventoryType.ItemTypeWeapon: SimpleWeaponItem,
        InventoryType.ItemTypeConsumable: SimpleConsumableItem,
        ItemType.DAGGERAMMO: SimpleAmmoItem,
        ItemType.PISTOLAMMO: SimpleAmmoItem,
        ItemType.CANNONAMMO: SimpleAmmoItem,
        ItemType.GRENADEAMMO: SimpleAmmoItem,
        ItemType.PISTOL_POUCH: SimplePouchItem,
        ItemType.DAGGER_POUCH: SimplePouchItem,
        ItemType.GRENADE_POUCH: SimplePouchItem,
    ...
    def handleBuyItem(self, value = DGG.DIALOG_YES):
        ...
        if self.buyPanelItem.quantity > 0:
            if not stackable:
                for i in xrange(self.buyPanelItem.quantity):
    ...
    def setBuyPanelItem(self, itemId):
        ...
        if not self.colorPanel:
            self.colorPanel = SimpleStoreColorPicker(self, self.cartFrame, self.buyPanelItem)
...
class JewelryStoreGUI(SimpleStoreGUI):
    ...
    tabInfos = {
        JewelryGlobals.RBROW: [
            iconsB.find('**/icon_shop_tailor_brow'),
            (-0.5, 0.5, 0.5)],
        JewelryGlobals.LBROW: [
            iconsB.find('**/icon_shop_tailor_brow'),
            (0.5, 0.5, 0.5)],
        JewelryGlobals.LEAR: [
            iconsA.find('**/chargui_ears'),
            (1.7, 1.7, 1.7)],
        JewelryGlobals.REAR: [
            iconsA.find('**/chargui_ears'),
            (-1.7, 1.7, 1.7)],
        JewelryGlobals.NOSE: [
            iconsA.find('**/chargui_nose'),
            1.7],
        JewelryGlobals.MOUTH: [
            iconsA.find('**/chargui_mouth'),
            1.7],
        JewelryGlobals.LHAND: [
...
class MerchantStoreGUI(SimpleStoreGUI):
    TabIdFromItemType = {
        ItemConstants.SWORD: 'SWORD',
        ItemConstants.GUN: 'GUN',
        ItemConstants.DOLL: 'DOLL',
        ItemConstants.DAGGER: 'DAGGER',
        ItemConstants.GRENADE: 'GRENADE',
        ItemConstants.STAFF: 'STAFF',
        ItemConstants.CANNON: 'CANNON',
        ItemConstants.SAILING: 'CHARMS',
        ItemConstants.AXE: 'AXE',
        ItemConstants.FENCING: 'FENCING',
        ItemConstants.POTION: 'POTION',
        ItemConstants.ODDS_AND_ENDS: 'ODDS_AND_ENDS',
        ItemConstants.MONSTER: 'MONSTER',
        ItemConstants.FISHING: 'FISHING',
        ItemConstants.QUEST_PROP: 'QUEST_PROP' }
    TabIdFromItemTypeGroup = {
        EconomyGlobals.ItemTypeGroup.CUTLASS: 'SWORD',
        EconomyGlobals.ItemTypeGroup.PISTOL: 'GUN',
        EconomyGlobals.ItemTypeGroup.DOLL: 'DOLL',
        EconomyGlobals.ItemTypeGroup.DAGGER: 'DAGGER',
        EconomyGlobals.ItemTypeGroup.GRENADE: 'GRENADE',
        EconomyGlobals.ItemTypeGroup.WAND: 'STAFF',
        EconomyGlobals.ItemTypeGroup.CANNON: 'CANNON',
        EconomyGlobals.ItemTypeGroup.POTION: 'POTION',
        EconomyGlobals.ItemTypeGroup.FISHING_GEAR: 'FISHING' }
    TabOrder = [
        'DAGGER',
        'SWORD',
        'AXE',
    ...
    TabDisplayNames = {
        ...
        'UNKNOWN': 'UNKNOWN' }
    TabDisplayIcons = {
        'AXE': ('**/pir_t_ico_swd_rapier_a', 'weapon'),
        'CHARMS': ('**/pir_t_ico_swd_rapier_a', 'weapon'),
        'DAGGER': ('**/pir_t_ico_knf_dagger_a', 'weapon'),
        'DOLL': ('**/pir_t_ico_dol_spirit_b', 'weapon'),
        'FENCING': ('**/pir_t_ico_swd_rapier_a', 'weapon'),
        'FISHING': ('**/pir_t_gui_fsh_mdRodIcon', 'fishing'),
        'GRENADE': ('**/pir_t_ico_bom_grenade', 'weapon'),
        'GUN': ('**/pir_t_ico_gun_pistol_a', 'weapon'),
        'MONSTER': ('**/pir_t_ico_swd_rapier_a', 'weapon'),
        'ODDS_AND_ENDS': ('**/pir_t_ico_swd_rapier_a', 'weapon'),
        'POTION': ('**/pir_t_ico_pot_elixir', 'skill'),
        'QUEST_PROP': ('**/pir_t_ico_swd_rapier_a', 'weapon'),
        'STAFF': ('**/pir_t_ico_stf_ward_a', 'weapon'),
    ...
    def cancelCellItemDetails(self, cell, detailsDelay = 0, event = None):
        ...
        if item:
            ...
            if not detailsDelay:
                ...
                tsk = taskMgr.doMethodLater(detailsDelay, self.hideDetails, 'inventoryUIHideDetailsTask', extraArgs = [
                    item])
                tsk.cell = cell
                tsk.item = item
