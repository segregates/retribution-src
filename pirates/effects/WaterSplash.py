...
class WaterSplash(PooledEffect, EffectController):
    ...
    def createTrack(self, lod = Options.SpecialEffectsHigh):
        self.startEffect = Sequence(Func(self.p0.setBirthRate, 0.0149), Func(self.p0.clearToInitial), Func(self.f.start, self, self), Func(self.f.reparentTo, self))
    ...
    def setEffectScale(self, effectScale):
        ...
        self.p0.renderer.setInitialXScale(0.0299 * self.cardScale * effectScale)
        ...
        self.p0.renderer.setFinalYScale(0.050000 * self.cardScale * effectScale)
