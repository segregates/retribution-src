from panda3d.core import CollideMask, CollisionNode, CollisionSphere, NodePath, Point3, TextNode, TransparencyAttrib, Vec3
import types
...
class InteractiveBase(FSM.FSM):
    ...
    def destroyProximitySphere(self):
        if self.hasProximityCollision:
            ...
            self.proximityCollisionNodePath.remove_node()
            self.proximityCollisionNodePath = None
            self.hasProximityCollision = 0
    ...
    def loadUseLabel(self):
        self.useLabel = DirectLabel(parent = aspect2d, frameColor = (0.1, 0.1, 0.25, 0.2), text = '', text_align = TextNode.ACenter, text_scale = 0.06, text_pos = (0.02, 0.02), text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1), textMayChange = 1, text_font = PiratesGlobals.getPirateOutlineFont())
