from panda3d.core import Light, TextNode, Vec4
# File: R (Python 2.4)
...
class RepairGameButton(GuiButton):
    ...
    def _initGUI(self):
        ...
        self.skillRing = DialMeter(self, wantCover = False, dangerRatio = 0.0, meterColor = Vec4(0.9, 0.9, 0.100, 1.0), baseColor = Vec4(0.149, 0.070, 0.0299, 1.0), completeColor = Vec4(0.100, 0.9, 0.100, 1.0))
        self.skillRing.reparentTo(self)
        self.skillRing.setScale(0.28498, 0.299, 0.265)
        self.skillRing.setBin('fixed', 32)
        self.skillGlow = OnscreenImage(parent = self, image = mainGui.find('**/icon_glow'), scale = (1.0, 1.0, 1.0), color = (1.0, 1.0, 0.598, 1.0))
        self.glow = OnscreenImage(parent = self, image = mainGui.find('**/icon_glow'), scale = (1.0, 1.0, 1.0), color = (1.0, 1.0, 0.598, 1.0))
    ...
    def createPirateNameBox(self, pirateName):
        ...
        width = self.pirateNameLabel.getWidth() + 0.050000
        pos = [
            0.0,
            0.0,
            height / 2 - 0.035000]
        ...
        self.pirateNameBox.setPos(aspect2d, x, 0, z - 0.174)
