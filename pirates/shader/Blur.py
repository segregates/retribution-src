from panda3d.core import Buffer, Camera, ColorBlendAttrib, DisplayRegion, InternalName, Lens, NodePath, Shader, ShaderInput, TextNode, Texture, Vec4
import math
import random
...
class RenderToTexture(DirectObject):
    ...
    def __destroyBuffer(self):
        if self.camera_node_path:
            self.camera_node_path.remove_node()
            self.camera_node_path = None
...
class Glow(DirectObject):
    ...
    def delete(self):
        ...
        if self.glow_camera_node_path:
            self.glow_camera_node_path.remove_node()
            self.glow_camera_node_path = None
...
class Blur(DirectObject):
    ...
    def __init__(self, width, height, source_rtt, luminance = 1, add = 0, order = -1, format = 0, hdr = 1, hdr_output = 1, add_glow = 0, glow_rtt = 0, average = 0):
        ...
        self.enable(0)
        self.enable(1)
        if self.checkArray(self.shader_array, self.checkShader):
            pass
        self.success = self.checkArray(self.rtt_array, self.checkRtt)
    ...
    def loadShader(self, filepath):
        ...
        # These are commented out since they seemed Disney-specific, and HDR wouldn't
        # work otherwise.
        '''
