...
class WorldCreator(WorldCreatorBase.WorldCreatorBase, DirectObject.DirectObject):
    ...
    def loadObjectsFromFile(self, filename, parent, parentUid = None, dynamic = 0, merge = False):
        ...
        # I don't know if Python 2.4 had superpowers but the sub class method should be executed too.
        WorldCreatorBase.WorldCreatorBase.loadObjectsFromFile(self, filename, parent)
    ...
    def loadWaypointData(self, filename, fileData):
        ...
        for link in links:
            if not link[0] in self.waypoints[filename]:
                continue

            if not link[1] in self.waypoints[filename]:
                continue
        ...
        for key in fileDict.iterkeys():
            if isinstance(fileDict[key], type({ })):
                if fileDict[key].get('Type') in WorldDataGlobals.WAYPOINT_TYPES:
                    self.waypoints[filename][key] = {
                        'Pos': fileDict[key].get('Pos'),
                        'Links': [] }
        ...
        if dynamic:
            ...
            if objType == 'Jack Sparrow Standin' and config.GetBool('want-npcs', 0):
                ...
                if objType == 'Light - Dynamic':
                    ...
                    if 'Color' in object:
                        if not 'Visual' in object:
                            object['Visual'] = { }

                        if not 'Color' in object['Visual']:
    ...
    def registerSpawnNodes(self, parent, uniqueId):
        ...
        for object in spawnNodes:
            ...
            self.cr.activeWorld.addPlayerSpawnPt(parent, posHpr, index)
    ...
    def createObj(self, levelObj, gameArea):
        ...
        data = ObjectList.AVAIL_OBJ_LIST.get(objectCat, {}).get(objType, {})
        if 'Entity' in data:
        ...
        elif objectCat in ('PROP_OBJ', 'BUILDING_OBJ') or objType in ('Dinghy', 'Holiday Object', 'Cave_Pieces'):
            newObj = gameArea.builder.addChildObj(levelObj)
            ...
            uid = levelObj.uniqueId
            newObj = gameArea.builder.areaGeometry.find('npcData')
    ...
    def isPvpIslandByUid(self, islandUid):
        if not base.cr.config.GetBool('want-privateering', 0):
            return False
