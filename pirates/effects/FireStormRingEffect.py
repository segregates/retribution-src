from panda3d.physics import BaseParticleEmitter, BaseParticleRenderer, LinearNoiseForce, LinearVectorForce
...
class FireStormRingEffect(PooledEffect, EffectController):
    ...
    def createTrack(self):
        ...
        self.track = Sequence(Func(self.p0.setBirthRate, 0.149), Func(self.p0.clearToInitial), Func(self.f.start, self, self), Parallel(fadeBlast, scaleBlast), Func(self.p0.setBirthRate, 100.0), Wait(3.0), Func(self.cleanUpEffect))


    def setEffectScale(self, scale):
        self.effectScale = scale
        self.p0.renderer.setInitialXScale(1.39 * self.cardScale * scale)
        self.p0.renderer.setFinalXScale(1.2 * self.cardScale * scale)
        self.p0.renderer.setInitialYScale(1.39 * self.cardScale * scale)
