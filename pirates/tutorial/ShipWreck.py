from panda3d.core import CollideMask, NodePath
...
class ShipWreck(NodePath):
    ...
    def __init__(self, npShipWreck, uid):
        ...
        self._ShipWreck__targetableCollisions = []
    ...
    def makeTargetableCollision(self, doId):
        for i in xrange(0, self.coll.getNumPaths()):
            c = self.coll[i]
    ...
    def addTargetableCollision(self, coll):
        self._ShipWreck__targetableCollisions.append(coll)


    def getTargetableCollisions(self):
        return self._ShipWreck__targetableCollisions
    ...
    def setTargetBitmask(self, on):
        if on:
            for coll in self._ShipWreck__targetableCollisions:
