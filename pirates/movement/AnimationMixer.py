...
class PartMixer:
    ...
    def __init__(self, mixer, channelCount, actor, partNameList):
        ...
        self.channels = [ AnimationChannel(x, x in mixer.LOOP.values(), actor, partNameList, self.distributeWeight) for x in xrange(channelCount) ]

    def __str__(self):
        outStr = '(PartMixer: parts = %s)\n' % `self.partNameList`
        for chanNum in xrange(len(self.channels) - 1, -1, -1):
