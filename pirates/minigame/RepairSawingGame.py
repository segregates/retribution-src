from panda3d.core import CollideMask, NodePath, Point3, TextNode, Vec4
import math
import random
...
class RepairSawingGame(RepairMincroGame):
    ...
    def reset(self):
        ...
        if self.currentBoard:
            self.currentBoard.remove_node()
            self.currentBoard = None

        if self.onDeckBoard:
            self.onDeckBoard.remove_node()
            self.onDeckBoard = None
    ...
    def getHitWaypoints(self):
        ...
        for i in xrange(len(self.sawWaypoints)):
