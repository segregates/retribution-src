...
class CrewIconSelector(GuiPanel.GuiPanel):
    ...
    def __init__(self, title, selectedIconKey = 0):
        GuiPanel.GuiPanel.__init__(self, title, 0.800000, 0.77, 0, '', pos = (0.52, 0, -0.815))
        ...
        self.localHeight = PiratesGuiGlobals.InventoryPanelHeight - 0.200
        ...
        for colSpot in xrange(4):
            for rowSpot in xrange(4):
                if colSpot > 0:
                    ...
                    blip.setScale(0.450)
                    blip.setPos(0.239 + 0.179 * rowSpot, 0, self.localHeight - 0.4 - 0.179 * colSpot)
                    continue


        crewIconParent.flattenStrong()
        self.iconFrame = DirectFrame(parent = self, relief = None, geom = crewIconParent)
        self.iconFrame.setPos(-0.100, 0, -0.12)
        ...
        self.bOk.setPos(0.25, 0, 0.050000)
        self.bCancel.setPos(0.450, 0, 0.050000)
        self.loadIconList()
    ...
    def loadIconList(self):
        ...
        for (loopItr, iconFile) in CREW_ICONS.iteritems():
            ...
            self.setPics[setKey] = DirectButton(parent = self.iconFrame, relief = None, image = tex, image_scale = 0.34, image2_scale = 0.359, image_pos = (0, 0, 0), pos = (0.239 + 0.179 * rowSpot, 0, self.localHeight - 0.4 - 0.179 * colSpot - 0.179), command = self._CrewIconSelector__setIconKey, extraArgs = [
