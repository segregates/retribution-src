from panda3d.core import GeomNode, NodePath, NodePathCollection, TexMatrixAttrib, Texture, TextureAttrib, TextureStage, TransformState, VBase4, Vec2
...
male_hairs = [
    'style 0: bald',
    'style 1: base w/A_top',
    'style 2: base w/A1_top',
    'style 3: base w/A_top + A2_sides',
    'style 4: base w/A1_top + A2_sides',
    'style 5: base w/A_top + B_ponytail',
    'style 6: base w/A1_top + B_ponytail',
    'style 8: base w/A_top + E_spike',
    'style 9: base w/A1_top + E_spike',
    'style 10: balding f0',
    'style 11: balding g0',
    'style 12: base w/A_top + H_mullet',
    'style 13: base w/A1_top + H_mullet',
    'style 14: mohawk i0']
vector_tattoos = [
    [
        0,
...
class PirateMale(DirectObject.DirectObject):
    ...
    def __init__(self, pirate, dna):
        ...
        self.currentClothingModels = {
            ...
            'BEARD': NodePathCollection(),
            'MUSTACHE': NodePathCollection(),
        ...
        if hatIdx[0] > 0:
            ...
            if hatIdx[0] == 7:
                if hairIdx[0] in [
                    ...
                    10,
                    13]:
                    for i in xrange(0, currentHat.getNumPaths()):
    ...
    def setupClothing(self, lodName = '2000'):
        ...
        if config.GetBool('want-gen-pics-buttons'):
            self.clothesByType = {
                ...
                'PANT': self.clothingsLayer1[16:22] + self.clothingsLayer1[29:30],
                'COAT': self.clothingsLayer3,
                #wtf, why this no work???
                'BELT': self.clothingsLayer2[4:],
                'SHOE': self.clothingsLayer1[22:28],
                'HAT': self.clothingsLayer1[31:] }
                    ...
                    if not isFromQuest or ItemGlobals.isFromQuest(hatId):
                        if not isFromPromo or ItemGlobals.isFromPromo(hatId):
                            if (isFromPVP or ItemGlobals.isFromPVP(hatId)) and isFromNPC:
                                pass
            toBeAdded = True#&= ItemGlobals.isFromNPC(hatId)
            if toBeAdded:
                choiceHats[hatId] = [
                    modelId,
                    texId,
                    ...
                    if not isFromQuest or ItemGlobals.isFromQuest(shirtId):
                        if not isFromPromo or ItemGlobals.isFromPromo(shirtId):
                            if (isFromPVP or ItemGlobals.isFromPVP(shirtId)) and isFromNPC:
                                pass
            toBeAdded = True#&= ItemGlobals.isFromNPC(shirtId)
            if toBeAdded:
                    ...
                    if not isFromQuest or ItemGlobals.isFromQuest(vestId):
                        if not isFromPromo or ItemGlobals.isFromPromo(vestId):
                            if (isFromPVP or ItemGlobals.isFromPVP(vestId)) and isFromNPC:
                                pass
            toBeAdded = True#&= ItemGlobals.isFromNPC(vestId)
            if toBeAdded:
                choiceVests[vestId] = [
                    modelId,
                    texId,
                    ...
                    if not isFromQuest or ItemGlobals.isFromQuest(coatId):
                        if not isFromPromo or ItemGlobals.isFromPromo(coatId):
                            if (isFromPVP or ItemGlobals.isFromPVP(coatId)) and isFromNPC:
                                pass
            toBeAdded = True#&= ItemGlobals.isFromNPC(coatId)
            if toBeAdded:
                choiceCoats[coatId] = [
                    modelId,
                    texId,
                    ...
                    if not isFromQuest or ItemGlobals.isFromQuest(pantId):
                        if not isFromPromo or ItemGlobals.isFromPromo(pantId):
                            if (isFromPVP or ItemGlobals.isFromPVP(pantId)) and isFromNPC:
                                pass
            toBeAdded = True#&= ItemGlobals.isFromNPC(pantId)
            if toBeAdded:
                choicePants[pantId] = [
                    modelId,
                    texId,
                    ...
                    if not isFromQuest or ItemGlobals.isFromQuest(beltId):
                        if not isFromPromo or ItemGlobals.isFromPromo(beltId):
                            if (isFromPVP or ItemGlobals.isFromPVP(beltId)) and isFromNPC:
                                pass
            toBeAdded = True#&= ItemGlobals.isFromNPC(beltId)
            if toBeAdded:
                choiceBelts[beltId] = [
                    modelId,
                    texId,
                    ...
                    if not isFromQuest or ItemGlobals.isFromQuest(shoeId):
                        if not isFromPromo or ItemGlobals.isFromPromo(shoeId):
                            if (isFromPVP or ItemGlobals.isFromPVP(shoeId)) and isFromNPC:
                                pass
            toBeAdded = True#&= ItemGlobals.isFromNPC(shoeId)
            if toBeAdded:
                choiceShoes[shoeId] = [
                    modelId,
                    texId,
    ...
    def handleClothesHidingOld(self):
        ...
        for j in xrange(0, len(layerShirtModified[0])):
        ...
        if parts.getNumPaths():
            ...
            if vestIdx[0] != 2 and shirtIdx[0] not in (5, 7, 8, 9):
        ...
        for shirtIdx in xrange(len(self.clothingsShirt)):
            ...
            flattenedSet = {
                'nobelt': {
                    ...
                    'openVest': NodePathCollection(),
                    'closedVest': NodePathCollection(),
                    'coat': NodePathCollection(),
                    'coat+closedVest': NodePathCollection(),
                    'covered': NodePathCollection() },
                'belt': {
                    'full': NodePathCollection(),
                    'openVest': NodePathCollection(),
                    'closedVest': NodePathCollection(),
                ...
                for idx in shirt[0]:
                    if idx == 4 or idx == 5:
                        ...
                        for i in xrange(pieceSet.getNumPaths()):
                            ...
                            piece.getName().find('collar_v_low') < 0
        ...
        for vestIdx in xrange(len(self.clothingsVest)):
            ...
            flattenedSet = {
                'belt': {
                    'full': NodePathCollection(),
                    'coat': NodePathCollection(),
                    'coatSpecial': NodePathCollection() },
                'nobelt': {
                    'full': NodePathCollection(),
                    'coat': NodePathCollection(),
                    'coatSpecial': NodePathCollection() } }
                ...
                for lod in [
                    ...
                    for partIdx in hairParts:
                        ...
                        if partIdx > 0:
                            ...
                            if not cutFound:
                                if (hatIdx == 7 or partIdx == 2) and partIdx == 5 and partIdx == 8 or partIdx == 10:
                                    hairIndices.add(partIdx)
                                    hairData.addPathsFrom(hair)
