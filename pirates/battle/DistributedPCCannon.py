from panda3d.core import ModelNode, NodePath, Point3, Vec3, lookAt
import math
import random
import copy
...
class DistributedPCCannon(DistributedWeapon.DistributedWeapon):
    ...
    def disable(self):
        ...
        if self.grapplingHook:
            self.grapplingHook.remove_node()
            self.grapplingHook = None
    ...
    def _doFireCannon(self):
        ...
        if localAvatar.guiMgr.combatTray.skillTray.traySkillMap:
            for i in xrange(len(localAvatar.guiMgr.combatTray.skillTray.traySkillMap)):
