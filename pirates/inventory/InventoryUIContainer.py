...
class InventoryUIContainer(DirectFrame, InventoryRequestBase.InventoryRequestBase):
    ...
    def makeCell(self, cellImage = None, imageScale = None, imagePos = None, frameScale = (1.0, 1.0), offset = (0.0, 0.0)):
        ...
        if self.testWithIn():
            cell.bind(DGG.ENTER, self.manager.startCellItemDetails, extraArgs = [
                cell,
                self.detailsPos,
                self.detailsHeight,
                self.detailsDelay])
            cell.bind(DGG.EXIT, self.manager.cancelCellItemDetails)
            cell.bind(DGG.WITHIN, self.manager.setWithin, extraArgs = [
                cell,
                0])
            cell.bind(DGG.WITHOUT, self.manager.setWithin, extraArgs = [
                cell,
                1])
        else:
            cell.bind(DGG.ENTER, self.enterCell, extraArgs = [
                cell])
            cell.bind(DGG.EXIT, self.exitCell, extraArgs = [
                cell])
    ...
    def putIntoCell(self, item, cell):
        if cell and item and not (cell.inventoryItem):
            ...
            if cell.slotId:
                messenger.send('inventory_slot_assigned %s' % cell.slotId, [
                    cell.slotId])
    ...
    def canSwap(self, myCell, otherCell):
        if not myCell.inventoryItem and otherCell.inventoryItem:
            return 0
        elif (self.canGive(myCell) and otherCell.container or otherCell.container.canGive(otherCell)) and self.canReceive(myCell, fromSwap = 1, itemInQuestion = otherCell.inventoryItem) and otherCell.container.canReceive(otherCell, fromSwap = 1, itemInQuestion = myCell.inventoryItem):
            return 1
        else:
            return 0
    ...
    def canReceive(self, myCell, fromSwap = 0, itemInQuestion = None):
        if (myCell.inventoryItem or not fromSwap) and self.manager.isSlotPending(myCell.slotId) or MASK_NOTDEST in myCell.statusMask:
