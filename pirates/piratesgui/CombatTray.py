from panda3d.core import NodePath, Point3, SequenceNode, TextNode, Vec3, Vec4
...
class TonicButton(SkillButton):
    ...
    def __init__(self):
        ...
        SkillButton.__init__(self, InventoryType.Potion1, self.callback, 0.0, 0.0,
                             showQuantity = True, showHelp = False, showRing = True,
                             hotkey = 'T', assocAmmo = assocAmmo)
...
class CombatTray(GuiTray):
    ...
    COMBO_ATTACKS = (InventoryType.CutlassHack, InventoryType.CutlassSlash, InventoryType.CutlassCleave, InventoryType.CutlassFlourish, InventoryType.CutlassStab, InventoryType.DaggerCut, InventoryType.DaggerSwipe, InventoryType.DaggerGouge, InventoryType.DaggerEviscerate)
    ...
    def __init__(self, parent, **kw):
        ...
        self.skillMapping = { }
        self.chargeCard.remove_node()
        self.chargeCard = None
        self.accept('drawStarted', self.setDrawWeapon, [
            1])
        self.accept('drawFinished', self.setDrawWeapon, [
            0])
    ...
    def destroy(self):
        ...
        if self.bloodFirePath:
            self.bloodFirePath.remove_node()
    ...
    def toggleWeapon(self, weaponId, slotId, args = None, fromWheel = 0):
        ...
        if weaponVolley and self.volley >= weaponVolley:
        ...
        if inventory:
            ...
            if itemTuple:
                ...
                if not canUse:
                    localAvatar.guiMgr.inventoryUIManager.displayReasonNoUse(popUp = 0)
                    return None
        ...
        if state in ('Spawn', 'Stunned', 'Death', 'WaterRoam', 'WaterTreasureRoam', 'Cutscene', 'MakeAPirate', 'TeleportOut', 'TeleportIn', 'ParlorGame', 'NPCInteract', 'ShipBoarding', 'Ensnared', 'Thrown', 'Knockdown', 'Unconcious', 'Injured', 'Dying', 'ThrownInJail', 'DoorKicking', 'EnterTunnel', 'LeaveTunnel', 'PVPWait', 'ShipPilot', 'Cannon', 'Fishing'):
    ...
    def initCombatTray(self, rep = None):
        ...
        if not rep:
            if self.rep == 0 or self.rep == InventoryType.MeleeRep:
                self.rep = InventoryType.MeleeRep
                leftSkills = [
                    InventoryType.MeleePunch]
                self.weaponMode = WeaponGlobals.COMBAT
            elif ItemGlobals.getType(localAvatar.currentWeaponId) == ItemGlobals.QUEST_PROP:
        ...
        if self.rep not in [
            InventoryType.DefenseCannonRep,
            InventoryType.MeleeRep]:
            allSkills = RadialMenu.getAllSkills(self.rep, 2, wantWeaponSkill = 1)
            for i in xrange(len(allSkills)):
                (skillId, avail) = allSkills[i][:2]
                if avail:
                    self.accept('%d' % (i + 1), self.triggerSkillTraySkill, [
                        skillId])
                    continue
    ...
    def trySkill(self, skillId, ammoSkillId, combo = 0, charge = 0):
        ...
        if localAvatar.getGameState() == 'Fishing':
            ...
            if rechargingTime <= 0:
                if skillId in xrange(InventoryType.begin_WeaponSkillFishingRod, InventoryType.end_WeaponSkillFishingRod):
                    if localAvatar.fishingGameHook:
                        if localAvatar.fishingGameHook.fsm.getCurrentOrNextState() in [
                            'PlayerIdle']:
                        ...
                        if localAvatar.fishingGameHook.fsm.getCurrentOrNextState() in [
                            ...
                            'FishFighting']:
                            if skillId not in [
                                InventoryType.FishingRodPull,
                                InventoryType.FishingRodHeal,
                                InventoryType.FishingRodTug]:
                                localAvatar.guiMgr.createWarning(PLocalizer.FishingAbilityWarning, PiratesGuiGlobals.TextFG6)
                                return 0
                        ...
                        if self.skillTray.traySkillMap:
                            if skillId in self.skillTray.traySkillMap:
                                for i in xrange(len(self.skillTray.traySkillMap)):
                        ...
                        self._CombatTray__startSkillTimers()
        ...
        if ammoItemId in (InventoryType.ShipRepairKit,) and hasattr(localAvatar, 'ship') and localAvatar.ship:
        ...
        if WeaponGlobals.getNeedSight(skillId, ammoSkillId):
            ...
            if self.aimAssistTarget:
                ...
                if target:
                    if not target.hasNetPythonTag('MonstrousObject'):
    ...
    def _CombatTray__startSkillTimers(self):
        ...
        taskMgr.remove('comboReadyTimer')
        taskMgr.remove('skillFinishedTimer')
        taskMgr.doMethodLater(self.WINDOW_LENGTH * duration, self._CombatTray__comboReadyTimer, 'comboReadyTimer')
        taskMgr.doMethodLater(duration, self._CombatTray__skillFinishedTimer, 'skillFinishedTimer', priority = 40)


    def _CombatTray__comboReadyTimer(self, task):
        self.comboReady = 1


    def _CombatTray__skillFinishedTimer(self, task):
        ...
        if skillId > 0:
            ...
            if self.weaponMode in (WeaponGlobals.FIREARM, WeaponGlobals.STAFF, WeaponGlobals.CANNON, WeaponGlobals.GRENADE, WeaponGlobals.DEFENSE_CANNON):
                ...
                if skillId in WeaponGlobals.SpecialSkills or WeaponGlobals.getSkillReputationCategoryId(skillId) not in (InventoryType.PistolRep, InventoryType.WandRep, InventoryType.CannonRep, InventoryType.GrenadeRep, InventoryType.DefenseCannonRep):
                    ...
                    if ammoAmt > 0 and maxQuant == WeaponGlobals.INF_QUANT and WeaponGlobals.canUseInfiniteAmmo(localAvatar.currentWeaponId, ammoChoice) or WeaponGlobals.canUseInfiniteAmmo(localAvatar.getCurrentCharm(), ammoChoice):
    ...
    def buttonCharging(self, task):
        ...
        if self.chargeTime > 0:
            ...
            if not self.thresholdHit:
                ...
                self.isCharging = 1
                if self.ammoSkillId:
                    if not WeaponGlobals.isInfiniteAmmo(self.ammoSkillId) and not WeaponGlobals.canUseInfiniteAmmo(localAvatar.currentWeaponId, self.ammoSkillId) and not WeaponGlobals.canUseInfiniteAmmo(localAvatar.getCurrentCharm(), self.ammoSkillId):
    ...
    def triggerSkillQueue(self):
        if self.actionPressed and self.weaponMode == WeaponGlobals.FIREARM:
            if not self.skillQueue or self.skillQueue[0] != EnemySkills.PISTOL_RELOAD:
                self.isUsingSkill = 0
                self.endButtonCharge()
    ...
    def resetComboLevel(self, args = None, skillId = None):
        ...
        if self.weaponMode == WeaponGlobals.FIREARM or self.weaponMode == WeaponGlobals.GRENADE:
            weaponVolley = WeaponGlobals.getWeaponVolley(localAvatar.currentWeaponId)
            if weaponVolley > 0 and self.volley >= weaponVolley:
                if WeaponGlobals.C_QUICKLOAD in localAvatar.getSkillEffects() and self.usedBayonetSkill or skillId:
                    if skillId in WeaponGlobals.SpecialSkills or WeaponGlobals.getSkillReputationCategoryId(skillId) not in (InventoryType.PistolRep, InventoryType.GrenadeRep):
                        self.usedBayonetSkill = False
                        self.volley = 0
                ...
                skillId == EnemySkills.PISTOL_RELOAD
                self.reloadWeapon()
            else:
                self.acceptInput()
        else:
    ...
    def _CombatTray__skillTrayCallback(self, skillId):
    ...
    def skillDisabled(self, skillId):
        if skillId in (InventoryType.SailPowerRecharge, InventoryType.SailRammingSpeed,
                       InventoryType.SailTakeCover, InventoryType.SailOpenFire) and localAvatar.ship:
            return localAvatar.ship.isFishing
