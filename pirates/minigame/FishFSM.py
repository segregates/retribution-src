from panda3d.core import MouseButton, NodePath, Point3, TextNode
# File: F (Python 2.4)
...
class FishFSM(FSM.FSM):
    ...
    def enterHooked(self):
        ...
        taskMgr.doMethodLater(delay, self.requestNext, name = '%s_StartFighting' % self.fish.getName(), extraArgs = [
            'AboutToFight'])
    ...
    def enterAboutToFight(self):
        ...
        taskMgr.doMethodLater(FishingGlobals.fightWarningDurations[self.fish.myData['size']], self.requestNext, name = '%s_GoFighting' % self.fish.getName(), extraArgs = [
            'HookedFighting'])
    ...
    def enterHookedFighting(self):
        ...
        taskMgr.doMethodLater(delay, self.requestNext, name = '%s_StopFighting' % self.fish.getName(), extraArgs = [
            'Hooked'])
    ...
    def enterFlee(self):
        taskMgr.doMethodLater(FishingGlobals.fleeDuration, self.requestNext, name = '%s_StopFleeing' % self.fish.getName(), extraArgs = [
            'Swimming'])
        self.fish.myZ = self.fish.getZ()
        self.fish.setHpr(0.0, 0.0, 0.0)
        if self.fish.movingRight:
