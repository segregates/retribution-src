...
class MysticSmoke(PooledEffect, EffectController):
    ...
    def __init__(self, parent = None):
        ...
        self.p0.renderer.setInitialYScale(0.0250 * self.cardScale)
        self.p0.renderer.setFinalYScale(0.100 * self.cardScale)
        self.p0.renderer.setAlphaBlendMethod(BaseParticleRenderer.PPBLENDLINEAR)
        self.p0.renderer.setAlphaDisable(0)
        self.p0.renderer.getColorInterpolationManager().addLinear(0.0, 1.0, Vec4(0.5, 0.598, 0.149, 1.0), Vec4(0.598, 0.75, 0.0, 0.0), 1)
    ...
    def createTrack(self, lod = None):
        self.startEffect = Parallel(Func(self.p0.setBirthRate, 0.0149), Func(self.p0.clearToInitial), Func(self.f.start, self, self.particleDummy))
