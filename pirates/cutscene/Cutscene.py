from panda3d.core import Camera, NodePath, TextNode
...
class Cutscene(NodePath, DirectObject):
    ...
    def __init__(self, cr, cutsceneName, doneCallback = None, giverId = None, otherLocalAvatar = None):
        self.otherLocalAvatar = otherLocalAvatar
    ...
    def initialize(self, doneCallback = None, giverId = None, patch = False):
        ...
        if originNode is not None:
            ...
            elif self.otherLocalAvatar:
                self.reparentTo(self.otherLocalAvatar)
                #print self.otherLocalAvatar.getPos()
                camera.setX(15)
                camera.setY(20)
            else:
                self.reparentTo(render)
    ...
    def getEnvEffects(self):
        ...
        if self.cr:
            if 'localAvatar' in __builtins__:
                ...
                ga = None
                #self.localPirate = DynamicHuman.DynamicHuman()
                #ga = self.localPirate.getParentObj()
    ...
    def destroy(self):
        ...
        if hasattr(self, 'timer'):
            self.timer.removeNode()
            self.timer = None
        if hasattr(self, 'localPirate'):
            self.localPirate.removeNode()
            del self.localPirate
        if hasattr(self, 'otherLocalAvatar'):
            # Don't remove the node. This will remove the MAP instance.
            del self.otherLocalAvatar
    ...
    def isPlaying(self):
        if self._ival: return self._ival.isPlaying()
