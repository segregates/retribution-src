from panda3d.core import AnimControl, CompassEffect, Fog, Light, ModelNode, NodePath, Plane, PlaneNode, Point3, RopeNode, Texture, TransformState, VBase3, Vec3, Vec4, lookAt
...
class Ship(DirectObject.DirectObject):
    ...
    WantWake = config.GetBool('want-wake', 0)
    breakSfx1 = None
    breakSfx3 = None
    sinkingSfx1 = None
    sinkingSfx2 = None
    ...
    def __init__(self, shipClass, root, breakAnims, hitAnims, metaAnims, collisions, locators):
        ...
        self.isSplit = False
        self.owner = None
        self.mastCollisions = [(int(x.getTag('Mast Code')), x) for x in self.modelCollisions.findAllMatches('**/collision_masts')]
    ...
    def manufactureCannons(self, detailLevel = 2):
        ...
        cannons = { }
        for i in xrange(len(cannonConfig)):
        ...
        broadsides = [
            [
                ...
                []],
            None]
        for i in xrange(len(leftConfig)):
            if leftConfig[i] > 0:
        ...
        for i in xrange(len(rightConfig)):
            if rightConfig[i] > 0:
    ...
    def setupCannons(self, cannons, broadsides, detailLevel = 2):
        ...
        if broadsides:
            ...
            for (broadsideSet, side) in zip(broadsides[0], ((leftRoot, 'left'), (rightRoot, 'right'))):
                for i in xrange(len(broadsideSet)):
                    port = broadsideSet[i]
                    if not port:
                        continue
