from panda3d.core import BitMask32, CollideMask, CollisionNode, CollisionTube, LODNode, Point3, Vec3
...
class Holder(Creature, TentacleUtils):
    ...
    SfxNames.update({
        'pain': SoundGlobals.SFX_MONSTER_KRAKEN_PAIN,
        'death': SoundGlobals.SFX_MONSTER_KRAKEN_DEATH })
    sfx = { }
    AnimList = (('idle', 'idle'), ('emerge', 'emerge'), ('release', 'release'), ('death', 'death'))
    ...
    def initStatusTableBETA(self):
        ...
        if self.hasLOD():
            ...
            root = self
        jointList = [
            root.find('**/def_tent_03'),
            root.find('**/def_tent_05'),
            root.find('**/def_tent_06'),
            root.find('**/def_tent_08'),
            root.find('**/def_tent_10'),
            root.find('**/def_tent_12')]
