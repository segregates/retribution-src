from panda3d.core import Buffer, Camera, CardMaker, DecalEffect, GeomNode, GeomVertexWriter, Lens, Mat3, NodePath, OrthographicLens, Texture, TextureStage, TransformState, Vec2, Vec4
...
class Flag(NodePath):
    ...
    def _Flag__setBaseModelColor(self, color):
        ...
        for rowNum in xrange(vertexData.getNumRows()):
    ...
    def _Flag__setBgStyle(self):
        ...
        bgTexCol = [ bgTexCol[x] for x in xrange(bgTexCol.getNumTextures()) ]
        sortDict = [](_[1]([ `x` for x in bgTexCol ], bgTexCol))
        keys = sortDict.keys()
        keys.sort()
        bgTexCol = [ sortDict[name] for name in keys ]
        for bgnum in xrange(0, len(bgTexCol) + 1):
