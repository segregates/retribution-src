...
class ClubheartsPortrait(DirectFrame):
    ...
    def __init__(self):
        ...
        self.bg = DirectFrame(parent = self, relief = None, text_scale = PiratesGuiGlobals.TextScaleTitleSmall, text_fg = PiratesGuiGlobals.TextFG26, text_align = TextNode.ACenter, text = titleName, text_pos = (0.0, 0.25), text_shadow = PiratesGuiGlobals.TextShadow, image = portraitGui.find('**/picture'), image_scale = 0.125, image_pos = (0.0, 0.0, -0.08))
        portraitGui.remove_node()
        detailGui.remove_node()
        i = 0
        self.checkBoxes = []
    ...
    def isItemEquipped(self, itemId):
        ...
        for location in xrange(locationRange[0], locationRange[1] + 1):
