...
class Trade(DistributedObject):
    ...
    def announceGenerate(self):
        ...
        messenger.send(PiratesGlobals.TradeIncomingEvent, [
            self])
    ...
    def sendRequestChangeGiving(self, giving):
        self.sendUpdate('requestChangeGiving', [
            giving])


    def rejectChangeGiving(self, reason):
        messenger.send('tradeRejectChangeGiving-%s' % (self.doId,), [
            reason])
    ...
    def sendRequestChangeStatus(self, isTradeApproved):
        self.sendUpdate('requestChangeStatus', [
            isTradeApproved])


    def rejectChangeStatus(self, reason):
        messenger.send('tradeRejectChangeStatus-%s' % (self.doId,), [
            reason])
    ...
    def rejectRemoveTrade(self, reason):
        messenger.send('rejectRemoveTrade-%s' % (self.doId,), [
            reason])
