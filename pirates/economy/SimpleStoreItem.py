...
class SimpleClothingItem(SimpleLootItem):
    ...
    def getCameraLookAtPos(self, pirate):
        return (pirate.headNode.getX(pirate), pirate.headNode.getY(pirate), pirate.headNode.getZ(pirate) * 0.9)
...
class SimpleEconomyItem(SimpleItem):
    ...
    def makeButton(self, parent, pos, cellSizeX, cellSizeZ):
        ...
        button = DirectButton(parent = parent, relief = None, rolloverSound = None, text = '', text_scale = 0.05, textMayChange = 1, geom = geomParams['geom'], geom_pos = (0, 0, 0), geom_scale = geomParams['geom_scale'], pos = pos, extraArgs = [
            self])
        return button
...
class SimplePouchItem(SimpleEconomyItem):
    ...
    def __init__(self, uid):
        ...
        if uid in xrange(InventoryType.begin_PistolPouches, InventoryType.end_PistolPouches):
            self.shortDesc = PLocalizer.makeHeadingString(PLocalizer.InventoryItemClassNames.get(ItemType.PISTOL), 1)
        elif uid in xrange(InventoryType.begin_DaggerPouches, InventoryType.end_DaggerPouches):
            self.shortDesc = PLocalizer.makeHeadingString(PLocalizer.InventoryItemClassNames.get(ItemType.DAGGER), 1)
        elif uid in xrange(InventoryType.begin_GrenadePouches, InventoryType.end_GrenadePouches):
            self.shortDesc = PLocalizer.makeHeadingString(PLocalizer.GrenadeShort, 1)
        elif uid in xrange(InventoryType.begin_CannonPouches, InventoryType.end_CannonPouches):
