...
class CrewPage(SocialPage.SocialPage):
    ...
    def determineOptionsButtonsState(self):
        if self.startACrewButton:
            if self.crewHUD.crew and self.crewHUD.joinACrewStatus and self.crewHUD.joinACrewStatusPVP or self.crewHUD.inTM:
        ...
        if self.changeCrewLookout:
            if (self.crewHUD.crew or DistributedBandMember.DistributedBandMember.IsLocalAvatarHeadOfBand() == 1 or self.crewHUD.startACrewState) and not (self.crewHUD.inTM):
