from panda3d.core import CardMaker, ConfigVariable, ConfigVariableBool, Geom, GeomVertexData, GeomVertexWriter, LineSegs, NodePath, NodePathCollection, TextNode, Texture, VBase3
...
class FancyLoadingScreen(DirectObject.DirectObject):
    ...
    def __init__(self, parent):
        ...
        if self.debugMode:
            ...
            self.enterToContinue = DirectLabel(parent = self.root, text = 'Press Shift To Continue', relief = None, text_scale = 0.1, pos = (0, 0, -0.9), text_align = TextNode.ACenter)
            self.enterToContinue.hide()
            self.stageLabel = DirectLabel(parent = self.root, text = '', relief = None, text_scale = 0.1, pos = (-1.25, 0, 0.75), text_align = TextNode.ALeft, textMayChange = 1)
            self.tickLabel = DirectLabel(parent = self.root, text = '', relief = None, text_scale = 0.1, pos = (0.75, 0, 0.75), textMayChange = 1)
            self.overallLabel = DirectLabel(parent = self.root, text = '', relief = None, text_scale = 0.1, pos = (0, 0, -0.75), textMayChange = 1)
        else:
            ...
            self.percentLabel = DirectLabel(text = '0%', parent = self.root, relief = None, text_font = PiratesGlobals.getPirateFont(), text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, text_scale = 0.031, pos = (0, 0, -0.4445), textMayChange = 1)
        ...
        self.loadingBarColors = [ (((i % 10) / 10.0 + 0.5) / 2.0, ((i % 100) / 10 / 10.0 + 0.5) / 2.0, (i / 100 / 10.0 + 0.5) / 2.0, 1) for i in xrange(1000) ]
        random.shuffle(self.loadingBarColors)
        self.lastUpdateTime = globalClock.getRealTime()
        self.locationLabel = DirectLabel(parent = self.root, relief = None, text = '', text_font = PiratesGlobals.getPirateOutlineFont(), text_fg = PiratesGuiGlobals.TextFG1, text_shadow = PiratesGuiGlobals.TextShadow, text_scale = PiratesGuiGlobals.TextScaleTitleJumbo * 0.7, text_align = TextNode.ACenter, pos = (0.0, 0.0, 0.515), textMayChange = 1)
    ...
    def cleanupLoadingScreen(self):
        if self.debugMode:
            self.loadingBarRoot.getChildren().detach()
    ...
    def cleanupAnalysis(self):
        ...
        self.analysisButtons = []
        self.analysisBarRoot.getChildren().detach()
        self.analysisBar.hide()
        self.analyzeMode = False
