...
class DistributedPlayerPirate(DistributedPirateBase, DistributedPlayer, DistributedBattleAvatar, DistributedQuestAvatar):
        ...
        try:
            ...
            if not self.badgeIconDict:
                ...
                for titleId in TitleGlobals.TitlesDict.keys():
                    ...
                    for rank in xrange(TitleGlobals.getMaxRank(titleId) + 1):
            ...
            if not self.gmNameTag:
                ...
                gmPurpleColor.setTextColor(0.8, 0, 1, 1)
    ...
    def setupInjured(self, timeStamp = None, MessageData = None):
        ...
        self.injuredTimeLeft = PiratesGlobals.REVIVE_TIME_OUT
        ...
        for index in xrange(InventoryType.begin_Consumables, InventoryType.end_Consumables):
    ...
    def injuryCountDownTask(self, task = None):
        ...
        downTime = currTime - self.initTime
        self.injuredTimeLeft = PiratesGlobals.REVIVE_TIME_OUT - downTime
    ...
    def setTutorialHandlerZone(self, zoneId):
        localAvatar.setInterest(base.cr.distributedDistrict.doId, zoneId, [
            'tutorialHandlerInterest'])
    ...
    def putOnBoat(self, doId, location, zone):
        pass
...
class MinimapPlayerPirate(MinimapBattleAvatar):
    ...
    def setAdminAccess(self, access):
        ...
        if self.isLocal():
            self.cr.wantMagicWords = self.adminAccess >= CATEGORY_GAME_MASTER.access
    ...
    def showProfile(avId):
        ...
        if str(avId)[:2] == "40": # This implies AI object, since pirates start with '1'
            return '%s is an NPC!' % av.getName()
        av.guiMgr.handleAvatarDetails(avId)
        return "Shown %s's Profile" % av.getName()

    @magicWord(CATEGORY_STAFF, types=[int])
    def playEmote(emoteId):
        '''this is limited to client side considering the missing d_playEmote'''
        base.localAvatar.setEmote(emoteId)
        base.localAvatar.setGameState("Emote", 25638)
        base.localAvatar.setSCEmote(emoteId)
        base.localAvatar.playEmote(emoteId)
        base.localAvatar.d_setEmote(emoteId)
        base.localAvatar.d_setGameState("Emote", 25638)
        base.localAvatar.d_setSCEmote(emoteId)
      # base.localAvatar.d_playEmote(emoteId)
        return 'Emote %s played' % emoteId

    @magicWord(CATEGORY_STAFF, types=[int])
    def changeBody(val):
        '''this is client side only to prevent abuse - requested by Johnny
        change body types from 0-9'''
