from panda3d.core import AmbientLight, AttribNodeRegistry, ColorBlendAttrib, ColorWriteAttrib, CompassEffect, DirectionalLight, Fog, Light, NodePath, Texture, TextureAttrib, TextureStage, VBase3, VBase4, Vec3, Vec4
...
class SkyGroup(NodePath):
    ...
    def __init__(self):
        ...
        self.skySettings = {
            TODGlobals.SKY_OFF: [
                (self.texTransparent, '', VBase4(0, 0, 0, 0), VBase4(0.0, 0.0, 0.0, 1)),
                (self.texOpaque, '', VBase4(0, 0, 0, 0), VBase4(0.0, 0.0, 0.0, 1)),
    ...
    def transitionSky(self, skyTypeA, skyTypeB, duration = 10.0):
        ...
        ival = Sequence(Parallel(LerpFunctionInterval(self.setStageColor, duration, fromData = 0.0, toData = 1.0, extraArgs = [
            self.tsSides[2],
            settingsA[0][2],
            ...
            self.tsTop[1],
            Vec4(0, 0, 0, 0),
            Vec4(1, 1, 1, 1)]), LerpColorScaleInterval(self.sides, duration, settingsB[0][3]), LerpColorScaleInterval(self.top, duration, settingsB[1][3]), LerpColorScaleInterval(self.clouds, duration, settingsB[2]), LerpColorScaleInterval(self.horizon, duration, settingsB[3])), Func(self.setLastSky, skyTypeB))
        return ival
    ...
    def transitionSkyFromCurrent(self, skyTypeB, duration = 10.0):
        ...
        ival = Parallel(LerpFunctionInterval(self.setStageColor, duration, fromData = 0.0, toData = 1.0, extraArgs = [
            ...
            self.tsTop[1],
            Vec4(0, 0, 0, 0),
            Vec4(1, 1, 1, 1)]), LerpColorScaleInterval(self.sides, duration, settingsB[0][3]), LerpColorScaleInterval(self.top, duration, settingsB[1][3]), LerpColorScaleInterval(self.clouds, duration, settingsB[2]), LerpColorScaleInterval(self.horizon, duration, settingsB[3]))
        return ival
