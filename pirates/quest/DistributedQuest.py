...
class DistributedQuest(DistributedObject.DistributedObject, QuestBase.QuestBase, Quest.Quest):
    ...
    def announceGenerate(self):
        ...
        self.setActive()
        messenger.send('localAvatarQuestAdded', [
            self])
    ...
    def delete(self):
        ...
        messenger.send('localAvatarQuestDeleted', [
            self])
        ...
        self.stopTimer()
        self.setInactive()
        self._DistributedQuest__cleanupDialog()
    ...
    def _DistributedQuest__cleanupDialog(self, value = None):
        if self.popupDialog:
    ...
    def setTaskStates(self, taskStates):
        ...
        if self.isGenerated():
            if self.isComplete():
                messenger.send('localAvatarQuestComplete', [
                    self])
                if self.getQuestId() == 'c2_visit_will_turner':
                ...
                messenger.send('localAvatarQuestUpdate', [
                    self])
    ...
    def amFinalized(self):
        messenger.send('localAvatarQuestFinalized', [
            self.doId])
        Quest.Quest.setFinalized(self, True)
        base.localAvatar.resetStoryQuest()
        if localAvatar.playRewardAnimation and localAvatar.getGameState() not in [
            'NPCInteract']:
            localAvatar.b_setGameState('OOBEmote', localArgs = [
                localAvatar.playRewardAnimation[0],
                localAvatar.playRewardAnimation[1]])
