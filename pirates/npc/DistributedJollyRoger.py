from panda3d.core import ColorBlendAttrib, GeomNode, NodePath, Plane, Point3, Texture, VBase3, VBase4, Vec3, Vec4
...
class DistributedJollyRoger(DistributedBattleNPC.DistributedBattleNPC, JollyRoger.JollyRoger, Boss):
    ...
    def announceGenerate(self):
        ...
        self.attackSounds = [
            ...
            loadSfx(SoundGlobals.SFX_MONSTER_JR_ATTACK_06),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_ATTACK_07)]
        self.damageSounds = [
            loadSfx(SoundGlobals.SFX_MONSTER_JR_DAMAGE_01),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_DAMAGE_03),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_DAMAGE_06),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_DAMAGE_07),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_DAMAGE_10),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_DAMAGE_13),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_DAMAGE_14)]
        self.taunts = [
            loadSfx(SoundGlobals.SFX_MONSTER_JR_SMELL),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_REALLY_IS_A_LAUGH),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_FEAR_ME),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_NEED_TO_FIND_SOMEONE_ELSE),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_SUBMIT),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_CHILDS_PLAY),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_SUBMIT_TO_ME),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_SO_MUCH_FUN),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_FIGHT_BEFORE_MEAL),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_WEAPON_NUISANCE),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_BRING_OPPONENT),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_DEAD),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_ALL_YOU_GOT),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_LAUGH_01),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_MEET_YOUR_MAKER),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_HOW_ITS_DONE),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_SOULS_TO_DEVOUR),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_EVERYONE_WILL_PAY),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_QUIP_03),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_QUIP_04),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_LAUGH_02),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_YOU_CALL),
            loadSfx(SoundGlobals.SFX_MONSTER_JR_ENJOYING),
    ...
    def playOuch(self, skillId, ammoSkillId, targetEffects, attacker, pos, itemEffects = [], multihit = 0, targetBonus = 0, skillResult = 0):
        if not self.recentlyPlayedDamage:
    ...
    def useTargetedSkill(self, skillId, ammoSkillId, skillResult, targetId, areaIdList, attackerEffects, targetEffects, areaIdEffects, itemEffects, timestamp, pos, charge = 0, localSignal = 0):
        if not self.recentlyPlayedAttack:
