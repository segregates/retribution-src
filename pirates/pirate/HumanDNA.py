from panda3d.core import Datagram, DatagramIterator, Texture, VBase4
import random
...
class PirateHair:
    ...
    def __str__(self):
        string = 'style %s, beard %s, mustache %s, color %s, highlight %s' % (
            self.hair, self.beard, self.mustache, self.color, self.highlight)
        return string
...
class PirateHead:
    ...
    def __str__(self):
        string = '--shape--\n\t' + 'headWidth %s, headHeight %s, headRoundness %s\n\t' % (
            self.headWidth, self.headHeight,
            self.headRoundness) + 'jawWidth %s, jawChinSize %s, jawAngle %s, jawLength %s\n\t' % (
            self.jawWidth, self.jawChinSize, self.jawAngle,
            self.jawLength) + 'face %s\n\t' % self.texture + '--mouth--\n\t' + 'mouthWidth %s, mouthLipThickness %s, \n\t' % (
            self.mouthWidth,
            self.mouthLipThickness) + 'cheekFat %s, teeth %s\n\t' % (
        self.cheekFat,
        self.teeth) + '--eyes--\n\t' + 'browProtruding %s\n\t' % self.browProtruding + 'eyeCorner %s, eyeOpeningSize  %s, eyeSpace %s\n\t' % (
            self.eyeCorner, self.eyeOpeningSize,
            self.eyeBulge) + 'eye %s\n\t' % self.eyes + '--nose--\n\t' + 'noseBridgeWidth %s, noseNostrilWidth %s, noseLength %s, noseBump %s\n\t' % (
            self.noseBridgeWidth, self.noseNostrilWidth, self.noseLength,
            self.noseBump) + 'noseNostrilHeight %s, noseNostrilAngle %s, noseNostrilIndent %s\n\t' % (
            self.noseNostrilHeight, self.noseNostrilAngle,
            self.noseNostrilIndent) + 'noseBridgeBroke %s, noseNostrilBroke %s\n\t' % (
            self.noseBridgeBroke,
            self.noseNostrilBroke) + '--ear--\n\t' + 'earScale %s, earFlapAngle %s, earPosition %s\n\t' % (
            self.earScale, self.earFlapAngle,
            self.earPosition) + '--hair--\n\t' + '%s\n\t' % self.hair
        return string
...
class PirateBody:
    ...
    def __str__(self):
        string = 'shape %s, height %s, headSize %s, skintone %s, skinTexture %s' % (
            self.shape, self.height, self.headSize, self.color, self.skin)
        return string
...
class PirateClothes:
    ...
    def __str__(self):
        string = 'shirt %s:%s:%s, vest %s:%s:%s, coat %s:%s:%s, pant %s:%s:%s,%s, belt %s:%s:%s, hat %s:%s:%s, sock %s:%s, shoe %s:%s' % (
            self.shirt, self.shirtTexture, self.shirtColor, self.vest,
            self.vestTexture, self.vestColor, self.coat, self.coatTexture,
            self.coatColor, self.pant, self.pantTexture, self.pantColor,
            self.shoeColor, self.belt, self.beltTexture, self.sashColor,
            self.hat,
            self.hatTexture, self.hatColor, self.sock, self.sockTexture,
            self.shoe,
            self.shoeTexture)
        return string
...
class HumanDNA(object):
    ...
    def saveAsNPCDict(self):
        ...
        d[HumanDNA.setClothesShirt] = (
            self.clothes.shirt, self.clothes.shirtTexture)
        d[HumanDNA.setClothesPant] = (
            self.clothes.pant, self.clothes.pantTexture)
        d[HumanDNA.setClothesSock] = (
            self.clothes.sock, self.clothes.sockTexture)
        d[HumanDNA.setClothesShoe] = (
            self.clothes.shoe, self.clothes.shoeTexture)
        d[HumanDNA.setClothesVest] = (
            self.clothes.vest, self.clothes.vestTexture)
        d[HumanDNA.setClothesCoat] = (
            self.clothes.coat, self.clothes.coatTexture)
        d[HumanDNA.setClothesBelt] = (
            self.clothes.belt, self.clothes.beltTexture)
        d[HumanDNA.setClothesTopColor] = (
            self.clothes.shirtColor, self.clothes.vestColor,
            self.clothes.coatColor)
        d[HumanDNA.setClothesBotColor] = (
    ...
    def verifyClothing(self, type, modelId, texId):
        if (
                self.gender, type, modelId,
                texId) in ClothingGlobals.quickConfirmSet:
            return [
                modelId,
                texId]
    ...
    def getClothesShirt(self):
        return self.verifyClothing('SHIRT', self.clothes.shirt,
                                   self.clothes.shirtTexture)


    def getClothesPant(self):
        return self.verifyClothing('PANT', self.clothes.pant,
                                   self.clothes.pantTexture)


    def getClothesHat(self):
        return self.verifyClothing('HAT', self.clothes.hat,
    ...
    def getClothesShoe(self):
        return self.verifyClothing('SHOE', self.clothes.shoe,
                                   self.clothes.shoeTexture)


    def getClothesVest(self):
        return self.verifyClothing('VEST', self.clothes.vest,
                                   self.clothes.vestTexture)


    def getClothesCoat(self):
        return self.verifyClothing('COAT', self.clothes.coat,
                                   self.clothes.coatTexture)


    def getClothesBelt(self):
        return self.verifyClothing('BELT', self.clothes.belt,
    ...
    def makeFromNetString(self, netString):
        ...
        self.setTattooZone2(dgi.getUint8(), dgi.getInt16() / 100.0, dgi.getInt16() / 100.0,
                            dgi.getUint8(), dgi.getUint8(), dgi.getUint8())
        self.setTattooZone3(dgi.getUint8(), dgi.getInt16() / 100.0, dgi.getInt16() / 100.0,
                            dgi.getUint8(), dgi.getUint8(), dgi.getUint8())
        self.setTattooZone4(dgi.getUint8(), dgi.getInt16() / 100.0, dgi.getInt16() / 100.0,
                            dgi.getUint8(), dgi.getUint8(), dgi.getUint8())
        self.setTattooZone5(dgi.getUint8(), dgi.getInt16() / 100.0, dgi.getInt16() / 100.0,
                            dgi.getUint8(), dgi.getUint8(), dgi.getUint8())
        self.setTattooZone6(dgi.getUint8(), dgi.getInt16() / 100.0, dgi.getInt16() / 100.0,
                            dgi.getUint8(), dgi.getUint8(), dgi.getUint8())
        self.setTattooZone7(dgi.getUint8(), dgi.getInt16() / 100.0, dgi.getInt16() / 100.0,
                            dgi.getUint8(), dgi.getUint8(), dgi.getUint8())
        self.setTattooZone8(dgi.getUint8(), dgi.getInt16() / 100.0, dgi.getInt16() / 100.0,
                            dgi.getUint8(), dgi.getUint8(), dgi.getUint8())
