...
class DistributedWreck(DistributedNode.DistributedNode):
    ...
    def sink(self):
        sinking = Sequence(LerpPosInterval(self, duration = 10, startPos = self.getPos(), pos = self.getPos() - Point3(0, 0, 70), blendType = 'easeIn', name = 'SunkSinkLerp'), Func(self.hide), Func(self.remove_node)).start()
