from panda3d.core import ColorBlendAttrib, NodePath, Vec3
# File: C (Python 2.4)
...
class CandleFlame(EffectController, NodePath):
    ...
    def __init__(self, newParent = render, billboardOffset = 1.0):
        ...
        self.glowHalo.setPos(0, 0, 0.149)
        self.scaleIval = None
        self.haloIval = None


    def createTrack(self, lod = Options.SpecialEffectsHigh):
        baseScale = Vec3(1.10, 1.10, 1.10)
        endScale = Vec3(1.0, 1.0, 1.39)
        self.scaleIval = Sequence()
        numIvals = min(2, lod + 1)
        for i in xrange(numIvals):
            randomness = random.random() / 10
            self.scaleIval.append(self.glow.scaleInterval(0.100 + randomness, endScale, startScale = baseScale, blendType = 'easeInOut'))
            self.scaleIval.append(self.glow.scaleInterval(0.100 + randomness, baseScale, startScale = endScale, blendType = 'easeInOut'))

        randomness = random.random() / 20
        scaleUpHalo = self.glowHalo.scaleInterval(0.100 + randomness, 2.0, startScale = 2.2, blendType = 'easeInOut')
        scaleDownHalo = self.glowHalo.scaleInterval(0.100 + randomness, 2.2, startScale = 2.0, blendType = 'easeInOut')
