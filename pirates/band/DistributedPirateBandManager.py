...
class DistributedPirateBandManager(DistributedObject):
    ...
    def d_requestRejoin(self, avatarId, isManager):
        self.sendUpdate('requestRejoin', [
            avatarId,
            isManager])

    def d_requestCancel(self, avatarId):
        self.sendUpdate('requestCancel', [
            avatarId])
    ...
    def requestOutCome(self, avatarId, avatarName, responce):
        if responce == 0:
            ...
            messenger.send('BandAdded-%s' % (avatarId,), [
                avatarId])
        elif responce != BandConstance.outcome_already_invited:
    ...
    def rejoinFrom(self, avatarId, isManager, version):
        messenger.send(BandConstance.BandRejoinEvent, [
            avatarId,
            isManager,
            version])
    ...
    def d_invitationResponce(self, avatarId, avatarName, responce):
        ...
        self.sendUpdate('invitationResponce', [
            avatarId,
            avatarName,
            responce])
    ...
    def d_rejoinResponce(self, avatarId, isManager, responce):
        ...
        self.sendUpdate('rejoinResponce', [
            avatarId,
            isManager,
            responce])

    def d_requestBoot(self, avatarId):
        self.sendUpdate('requestBoot', [
            avatarId])
    ...
    def d_requestRejoinCheck(self, avatarId):
        self.sendUpdate('requestRejoinCheck', [
            avatarId])

    def d_requestCrewIconUpdate(self, iconKey):
        self.sendUpdate('requestCrewIconUpdate', [
            iconKey])
    ...
    def receiveBootTeleport(self, avId, avName):
        ...
        base.cr.teleportMgr.requestTeleportToIsland(base.localAvatar.getReturnLocation())
