from panda3d.core import Camera, DisplayRegion, Lens, Mat4, NodePath, Point3, TextNode, TransformState, Vec3, Vec4, lookAt
...
class AccessoriesStoreTab(LeftTab):
    ...
    def __init__(self, tabBar, name, **kw):
        optiondefs = (('modelName', 'general_frame_d', None), ('borderScale', 0.38, None), ('bgBuffer', 0.15, None))
...
class AccessoriesStoreCartList(DirectScrolledFrame):
    ...
    def addPanel(self, data, mode, repack = 1):
        ...
        panel = DirectButton(parent = self, relief = None, text = text[:maxLength], text_fg = self.itemColor, text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleMed, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.06, 0.0), command = self.removePanel, extraArgs = [
            data,
            mode])
        panel.costLabel = DirectLabel(parent = panel, relief = None, text = strCost, text_fg = self.itemColor, text_align = TextNode.ARight, text_scale = PiratesGuiGlobals.TextScaleMed, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.45, 0.0), image = self.parent.CoinImage, image_scale = 0.15, image_pos = (0.48, 0.0, 0.014))
        panel.bind(DGG.ENTER, self.highlightStart, extraArgs = [
            panel])
        panel.bind(DGG.EXIT, self.highlightStop, extraArgs = [
            panel])
        panel.data = data
    ...
    def removePanel(self, data, mode, repack = 1):
        for panel in self.panels:
            if (mode == 0 or panel.data[:3] == data[:3] or mode == 1) and panel.data == data:
    ...
    def hasPanel(self, data, mode):
        for panel in self.panels:
            if panel.data[0] == data[0] and panel.data[1] == data[1] and panel.data[2] == data[2]:
                if mode == 0 or panel.data[3] == data[3]:
                    return True
...
class AccessoriesStoreGUI(DirectFrame):
    ...
    def __init__(self, npc, shopId, **kw):
        ...
        self.nextPageButton = DirectButton(parent = self.panel, relief = None, state = DGG.DISABLED, image = triangle, image_scale = 0.065, pos = (0.53, 0.0, 0.175), rolloverSound = None, command = self.nextPage)
        self.prevPageButton = DirectButton(parent = self.panel, relief = None, state = DGG.DISABLED, image = triangle, image_scale = -0.065, pos = (0.17, 0.0, 0.175), rolloverSound = None, command = self.previousPage)
    ...
    def showColorFrame(self, button = None, type = None, id = None, tex = None, cost = None, uid = None):
        if not self.colorFrame:
            ...
            self.colorFrame = DirectFrame(parent = aspect2dp, relief = None, image = topImage, image_scale = (0.24, 0.24, 0.24), pos = (0.1, 0.0, -0.2))
        ...
        def selectColor(id, colorFrame):
            ...
            if button:
                for idx in xrange(len(self.buttons)):
        ...
        self.cancelColorButton = GuiButton.GuiButton(parent = self.colorFrame, text = PLocalizer.lCancel, pos = (0.65, 0.0, -0.025), command = exitFrame)
        ...
        charGui = loader.loadModel('models/gui/char_gui')
        for idx in xrange(0, len(colorSet)):
            color = colorSet[idx]
            if idx in colorsOwned:
                state = DGG.DISABLED
            else:
                state = DGG.NORMAL
            colorButton = GuiButton.GuiButton(parent = self.colorFrame, pos = (offsetx, 0.0, offsety), command = selectColor, image = (charGui.find('**/chargui_frame04'), charGui.find('**/chargui_frame04_down'), charGui.find('**/chargui_frame04_over')), image_scale = (0.16, 0.0, 0.12), helpText = PLocalizer.TailorColorStrings.get(color), helpDelay = 0)
            colorFrame = DirectFrame(parent = colorButton, frameSize = (-0.0325, 0.0325, -0.0325, 0.0325))
    ...
    def hideDisplayRegions(self):
        for id in xrange(len(self.clothRenders)):
    ...
    def aspectRatioChange(self):
        ...
        for id in xrange(len(self.buttons)):
    ...
    def destroy(self):
        ...
        if self.CoinImage:
            self.CoinImage.remove_node()
            self.CoinImage = None

        if self.ParchmentIcon:
            self.ParchmentIcon.remove_node()
            self.ParchmentIcon = None

        if self.TailorIcons:
            self.TailorIcons.remove_node()
    ...
    def setPage(self, pageName, startIndex = 0, refreshWardrobe = True):
        self.tabBar.unstash()
        self.titleLabel['text'] = '\x01smallCaps\x01' + self.rootTitle + ' - ' + self.categoryText[pageName][1]
        ...
        if self.equipRequests[ClothingGlobals.VEST] is None:
            currentVest = [
                ...
                localAvatar.style.clothes.vest,
                localAvatar.style.clothes.vestTexture,
                topColor[1]]
        else:
            currentVest = [
                ClothingGlobals.VEST,
        ...
        if self.equipRequests[ClothingGlobals.COAT] is None:
            currentCoat = [
                ...
                localAvatar.style.clothes.coat,
                localAvatar.style.clothes.coatTexture,
                topColor[2]]
        else:
            currentCoat = [
                ClothingGlobals.COAT,
        ...
        if self.equipRequests[ClothingGlobals.PANT] is None:
            currentPant = [
                ...
                localAvatar.style.clothes.pant,
                localAvatar.style.clothes.pantTexture,
                botColor[0]]
        else:
            currentPant = [
                ClothingGlobals.PANT,
        ...
        if self.equipRequests[ClothingGlobals.SHOE] is None:
            currentShoe = [
                ...
                localAvatar.style.clothes.shoe,
                localAvatar.style.clothes.shoeTexture,
                botColor[2]]
        else:
            currentShoe = [
                ClothingGlobals.SHOE,
        ...
        if self.equipRequests[ClothingGlobals.BELT] is None:
            currentBelt = [
                ...
                localAvatar.style.clothes.belt,
                localAvatar.style.clothes.beltTexture,
                botColor[1]]
        else:
            currentBelt = [
                ClothingGlobals.BELT,
        ...
        if self.mode == BUYING:
            ...
            for clothingId in clothingIds:
                if ItemGlobals.getType(clothingId) == clothingTypeId and ItemGlobals.getClass(clothingId) == InventoryType.ItemTypeClothing:
                    ...
                    if id != -1:
                        if not holiday or holiday in AccessoriesStoreGUI.holidayIdList:
                            clothes.append([
                                clothingId,
                                color,
                                0,
            ...
            if self.currentWardrobe:
                for item in self.currentWardrobe:
                    ...
                    if type == clothingTypeId:
                        ...
                        if id != -1:
                            if location in xrange(Locations.RANGE_EQUIP_CLOTHES[0], Locations.RANGE_EQUIP_CLOTHES[1]):
                                equipped = True
        ...
        for cloth in clothes:
            ...
            if numButtons < self.buttonsPerPage and self.clothingAmount >= startIndex:
                ...
                clothButton.addToCart = GuiButton.GuiButton(command = self.addToCart, parent = clothButton, text_fg = PiratesGuiGlobals.TextFG2, text_pos = (0.0, -0.01), text_scale = PiratesGuiGlobals.TextScaleLarge, text_align = TextNode.ACenter, text_shadow = PiratesGuiGlobals.TextShadow, image_color = buttonColorB, image_scale = (0.19, 0.22, 0.22), pos = (0.18, 0.0, 0.055), helpText = PLocalizer.ShopAddToCart, helpDelay = 0)
                clothButton.colorPicker = GuiButton.GuiButton(command = self.showColorFrame, parent = clothButton, text_fg = PiratesGuiGlobals.TextFG2, text_pos = (0.0, -0.01), text_scale = PiratesGuiGlobals.TextScaleLarge, text_align = TextNode.ACenter, text_shadow = PiratesGuiGlobals.TextShadow, image_color = buttonColorB, image_scale = (0.08, 0.22, 0.22), geom = self.ColorPickerIcon, geom_scale = 0.4, geom_color = Vec4(0.8, 0.8, 0.8, 1), pos = (0.045, 0.0, 0.055), helpText = PLocalizer.ShopSelectColor, helpPos = (-0.28, 0, 0.08), helpDelay = 0, helpOpaque = 1)
                if self.mode == BUYING:
                ...
                clothButton.bind(DGG.ENTER, self.highlightClothStart, extraArgs = [
                    self.clothingAmount])
                clothButton.bind(DGG.EXIT, self.highlightClothStop, extraArgs = [
                    self.clothingAmount])
                if self.mode == BUYING and not owned:
                    if clothCost == 0:
                ...
                if self.mode == BUYING and owned:
                    ...
                    if not not (self.pvpMode):
                        ...
                        if self.mode == BUYING:
                            ...
                            if self.pvpMode:
                                if landInfamyRequired or seaInfamyRequired:
                                    clothButton.addToCart.hide()
                ...
                regionData.append([
                    ClothingGlobals.CLOTHING_STRING[type],
                    id,
                    tex,
                    clothColorId])
    ...
    def createDisplayRegions(self):
        ...
        for x in xrange(self.buttonsPerPage):
    ...
    def setupDisplayRegions(self, regionData, pageName):
        ...
        source.update()
        x = 0
        for x in xrange(len(regionData)):
            if pageName == ClothingGlobals.HAT:
                ...
                offsetH = 200
            elif pageName == ClothingGlobals.COAT:
        ...
        if x < self.buttonsPerPage - 1:
            for y in xrange(self.buttonsPerPage - 1 - x):
