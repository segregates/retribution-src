...
def _parseChildren(nodePath, parent, elements = { }, elementTypes = None, **kw):
    for i in xrange(nodePath.getNumChildren()):
...
def _parseNodePath(nodePath, parent, elements = { }, elementTypes = None, **kw):
    ...
    elif prefix in PREFIX_TO_CREATE_CALL:
        pass
...
def _createButton(nodePath, parent, **kw):
    ...
    if textLocator.isEmpty():
        ...
        textPos = (textLocator.getX(), textLocator.getZ())
    args = { }
    args['parent'] = parent
...
PREFIX_TO_CREATE_CALL = {
    BUTTON_PREFIX: _parseButton,
    STATIC_PREFIX: _parseStatic,
    SPRITE_PREFIX: _parseStatic,
    SEQUENCE_PREFIX: _parseStatic,
    TEXT_PREFIX: _parseText,
    LOCATOR_PREFIX: _parseStatic }
