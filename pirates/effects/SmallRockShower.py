...
class SmallRockShower(PooledEffect, EffectController):
    cardScale = 128.0
    ...
    def __init__(self):
        ...
        if not SmallRockShower.particleDummy:
            SmallRockShower.particleDummy = render.attachNewNode(ModelNode('SmallRockShowerParticleDummy'))
    ...
    def createTrack(self):
        self.track = Sequence(Func(self.p0.setBirthRate, 0.0250), Func(self.p0.clearToInitial), Func(self.f.start, self, self.particleDummy), Func(self.f.reparentTo, self), Wait(0.299), Func(self.p0.setBirthRate, 100), Wait(3.0), Func(self.cleanUpEffect))
