...
class StatsHolder:
    ...
    def createNewItem(self, item, parent, itemType = None, columnWidths = [], color = None):
        if itemType == PiratesGuiGlobals.UIListItemType_ColumHeadings:
            newItem = StatRowHeadingGui(item, self.game.getColumnLabels(), parent, textScale = 0.06, itemHeight = PiratesGuiGlobals.TMCompletePageHeight / 10, itemWidths = columnWidths, frameColor = (0.0, 0.0, 0.0, 0.0))
...
class DistributedPVPInstance(DistributedInstanceWorld, FSM):
    ...
    def setGameStart(self, timestamp):
        if not localAvatar.doId in self.teams:
            self.performAILeave()
            return
    ...
    def getTeamStats(self, team):
        ...
        if team:
            ...
            for (playerName, playerStats, color) in teamStats:
                for i in xrange(len(playerStats)):
