...
from panda3d.core import ColorBlendAttrib, Light, ModelNode, Point3, VBase3, Vec3, Vec4
# File: L (Python 2.4)
...
class LightFire(PooledEffect, EffectController):
    cardScale = 64.0
    ...
    def __init__(self, effectParent = None):
        ...
        if not LightFire.particleDummy:
            ...
            LightFire.particleDummy.setDepthWrite(0)
            LightFire.particleDummy.setFogOff()
            LightFire.particleDummy.setLightOff()
            LightFire.particleDummy.setColorScaleOff()
            LightFire.particleDummy.setBin('fixed', 60)

        self.f = ParticleEffect.ParticleEffect('LightFire')
    ...
    def setEffectScale(self, scale):
        ...
        self.p0.emitter.setAmplitude(-0.800000 * scale)
