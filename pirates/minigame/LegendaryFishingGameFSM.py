...
class LegendaryFishingGameFSM(FSM.FSM):
    ...
    def enterReelingFish(self):
        ...
        if random.random() < lFish.myData['chanceItWillMakeABreakForIt']:
            taskMgr.doMethodLater(delay / 2.0, self.requestNext, name = self.gameObject.distributedFishingSpot.uniqueName(taskName), extraArgs = [
                'Transition',
                'CatchIt'])
        else:
            taskMgr.doMethodLater(delay, self.requestNext, name = self.gameObject.distributedFishingSpot.uniqueName(taskName), extraArgs = [
                'Transition',
                'Struggle'])
