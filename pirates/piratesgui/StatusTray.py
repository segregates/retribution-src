...
class StatusTray(GuiTray.GuiTray):
    ...
    def __init__(self, parent, showSkills = 0, **kw):
        ...
        if showSkills:
            ...
            self.activeName = DirectLabel(parent = self.reloadFrame, state = DGG.DISABLED, relief = None, text = PLocalizer.UsingSkill, text_align = TextNode.ARight, text_scale = 0.06, pos = (-0.15, 0, 0.01), text_fg = PiratesGuiGlobals.TextFG11, text_shadow = PiratesGuiGlobals.TextShadow, text_font = PiratesGlobals.getPirateOutlineFont())
    ...
    def updateName(self, name, level, doId):
        ...
        if avType.isA(AvatarTypes.JollyRoger):
            color = '\x01red\x01'
            text = '%s  \x01smallCaps\x01%s\x02%s\x02%s\x03' % (name, color, PLocalizer.Lv, PLocalizer.InvasionLv)
        elif target.isInInvasion():
            text = '%s' % self.name
        elif color:
            text = '%s  \x01smallCaps\x01%s\x02%s\x02%s\x03' % (name, color, PLocalizer.Lv, self.level)
        else:
            text = '%s' % self.name
