from panda3d.core import Buffer, Camera, CompassEffect, DisplayRegion, Lens, NodePath, OrthographicLens, TexGenAttrib, Texture, TextureStage, VBase4
...
class AvatarShadowCaster(DirectObject):
    ...
    def turnOnShadows(self):
        ...
        self._AvatarShadowCaster__createBuffer()
        self.accept('close_main_window', self._AvatarShadowCaster__destroyBuffer)
        self.accept('open_main_window', self._AvatarShadowCaster__createBuffer)
        self.accept('texture_state_changed', self._AvatarShadowCaster__createBuffer)


    def turnOffShadows(self):
        render.clearTexProjector(self.shadowStage)
        self._AvatarShadowCaster__destroyBuffer()
    ...
    def disable(self):
        ...
        taskName = 'shadowCamCompass'
        taskMgr.remove(taskName)
        self.shadowCamArm.remove_node()
        self.shadowCam.remove_node()
        if base.main_rtt:
    ...
    def _AvatarShadowCaster__createBuffer(self):
        if self.shadowsHidden or not (self.shadowsEnabled):
            return None
        ...
        self.setupTask = taskMgr.doMethodLater(0, self._AvatarShadowCaster__setupCamera, 'setupCamera')


    def _AvatarShadowCaster__setupCamera(self, task):
    ...
    def _AvatarShadowCaster__destroyBuffer(self):
