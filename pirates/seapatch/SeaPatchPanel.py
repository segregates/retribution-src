from panda3d.core import VBase2, Vec2, Vec4
...
class SeaPatchPanel(AppShell):
    ...
    def createInterface(self):
        ...
        geomFloaterDefs = (('Geom', 'Speed', 'Global control of wave speed', self.setOverallSpeed, 0.0, 0.0001), ('Geom', 'Radius', 'Outer Boundary at which wave will completely fade out in amplitude', self.setRadius, 0.0, 0.100), ('Geom', 'Threshold', 'Inner Boundary at which wave will start to fade out its amplitude', self.setThreshold, 0.0, 0.100), ('Geom', 'Height Damping', 'Damp the Bobbing for all floating objects', self.setHDamping, 0.01, 0.01), ('Geom', 'Normal Damping', 'Damp the Rocking for all floating objects', self.setNDamping, 0.01, 0.01))
        shadingFloaterDefs = (('Shading', 'Red High', 'Red Value of the High color', self.setHighColorR, 0.0, 0.100), ('Shading', 'Blue High', 'Blue Value of the High color', self.setHighColorB, 0.0, 0.100), ('Shading', 'Green High', 'Green Value of the High color', self.setHighColorG, 0.0, 0.100), ('Shading', 'Alpha High', 'Alpha Value of the High color', self.setHighColorA, 0.0, 0.100), ('Shading', 'Red Low', 'Red Value of the Low color', self.setLowColorR, 0.0, 0.100), ('Shading', 'Blue Low', 'Blue Value of the Low color', self.setLowColorB, 0.0, 0.100), ('Shading', 'Green Low', 'Green Value of the Low color', self.setLowColorG, 0.0, 0.100), ('Shading', 'Alpha Low', 'Alpha Value of the Low color', self.setLowColorA, 0.0, 0.100))
        UVFloaterDefs = (('UV', 'UScale', 'Set the U divisor for UV computation', self.setUScale, 0.100, 0.100), ('UV', 'VScale', 'Set the V divisor for UV computation', self.setVScale, 0.100, 0.100), ('UV', 'USpeed', 'The rate at which the texture slides at rest', self.setUSpeed, -100.0, 0.0001), ('UV', 'VSpeed', 'The rate at which the texture slides at rest', self.setVSpeed, -100.0, 0.0001))
        multiwaveFloaterDefs = (('MWave', 'Amplitude', 'Amplitude of the waves', self.setMAmplitude, 0.0, 0.01), ('MWave', 'Wavelength', 'Wavelength', self.setMWavelength, -1000.0, 0.0001), ('MWave', 'Speed', 'Speed', self.setMSpeed, 0.01, 0.01), ('MWave', 'WaveDirX', 'Wave X Direction', self.setWaveXDir, -100, 0.01), ('MWave', 'WaveDirY', 'Wave Y Direction', self.setWaveYDir, -100, 0.01), ('MWave', 'Choppiness', 'Wave Choppy Factor', self.setChoppyK, 0.01, 0.01))
        ...
        self.createOptionMenu(multiwavePage, 'Multiwave', 'Wave Number', 'Select wave number', ('1', '2', '3', '4', '5', '6'), self.selectWave)
        self.createCheckbutton(multiwavePage, 'Multiwave', 'Enable', 'Turn On-Off this particular wave', self.switchWaves, 1)
        self.createOptionMenu(multiwavePage, 'Multiwave', 'Wave Target', "Specify the wave's target", ('Height (Z)', 'U', 'V'), self.selectWaveTarget)
        self.createOptionMenu(multiwavePage, 'Multiwave', 'Wave Func', "Specify the wave's function", ('Sin', 'Noise'), self.selectWaveFunc)
    ...
    def loadWave(self):
        fileName = askopenfilename(filetypes = [
            ('SPF', 'spf')], title = 'Open SeaPatch File')
        if not fileName:
            return
    ...
    def saveWave(self):
        fileName = asksaveasfilename(filetypes = [
            ('SPF', 'spf')], title = 'Save SeaPatch File')
        if not fileName:
            return
