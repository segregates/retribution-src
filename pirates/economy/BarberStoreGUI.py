...
class BarberStoreGUI(DirectFrame):
    ...
    def __init__(self, npc, shopId, **kw):
        ...
        self.nextPageButton = DirectButton(parent = self.cartFrame, relief = None, state = DGG.DISABLED, image = triangle, image_scale = 0.065, pos = (0.18, 0.0, 0.1), rolloverSound = None, command = self.nextPage)
        ...
        self.pageNumber = DirectFrame(parent = self.cartFrame, relief = None, text = '', text_fg = PiratesGuiGlobals.TextFG2, text_align = TextNode.ACenter, text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (0.0, 0.0), text_shadow = PiratesGuiGlobals.TextShadow, pos = (0, 0, 0.09))
    ...
    def destroy(self):
        ...
        if self.barberIconsA:
            self.barberIconsA.remove_node()
            self.barberIconsA = None

        if self.barberIconsB:
            self.barberIconsB.remove_node()
    ...
    def createTabs(self):
        ...
        for id in BarberGlobals.barberTypes:
            if gender == 'f' and id in [
                BarberGlobals.BEARD,
                BarberGlobals.MUSTACHE]:
                continue
    ...
    def setPage(self, pageName, startIndex = 0):
        self.tabBar.unstash()
        self.titleLabel['text'] = '\x01smallCaps\x01' + self.rootTitle + ' - ' + PLocalizer.barberNames.get(pageName)
        ...
        for index in xrange(startRange, startRange + 9999):
            if index in set:
        ...
        for choice in choices:
            ...
            if self.itemAmount - startIndex < self.buttonsPerPage and self.itemAmount >= startIndex:
                ...
                itemButton.buy = GuiButton.GuiButton(command = self.buyItem, parent = itemButton, text = PLocalizer.PurchaseCommit, text_fg = PiratesGuiGlobals.TextFG2, text_pos = (0.0, -0.01), text_scale = PiratesGuiGlobals.TextScaleLarge, text_align = TextNode.ACenter, text_shadow = PiratesGuiGlobals.TextShadow, pos = (0.25, 0.0, 0.09), extraArgs = [
                    uid,
                    itemButton])
                ...
                if itemId != 0 or GENDER == 'FEMALE':
                    ...
                    for i in choices:
                        ...
                        button = DirectButton(parent = itemButton, relief = DGG.RAISED, pos = (xOffset, 0, yOffset), frameSize = (-0.024, 0.024, -0.025, 0.025), borderWidth = (0.004, 0.004), frameColor = hairTone, command = self.handleSetBaseColor, extraArgs = [
                            self.itemAmount,
                            type,
                            i,
