from panda3d.core import TextNode, VBase4, Vec3, Vec4
...
class ReputationMeterDial(DirectFrame):
    ...
    def __init__(self, category, width = 0.4):
        ...
        self.valueLabel = DirectLabel(parent = self, state = DGG.DISABLED, relief = None, text = '', text_scale = PiratesGuiGlobals.TextScaleLarge, text_align = TextNode.ACenter, text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, text_font = PiratesGlobals.getInterfaceFont(), pos = (0.0, 0, -0.2), textMayChange = 1)
        self.levelCapScroll = DirectFrame(parent = self, relief = None, image = guiModel.find('**/main_gui_quest_scroll'), image_scale = (0.11, 1.0, 0.08), pos = (0.01, 0.0, -0.175))
        self.levelCapScroll.hide()
        self.levelCapIcon = DirectFrame(parent = self, relief = None, image = logoModel.find('**/skull'), image_pos = (0.0, 0.0, 0.0), image_scale = 0.75, pos = (-0.0275, 0.0, -0.105))
    ...
    def update(self, value, updateLocal = 0):
        ...
        if level == levelCap:
            ...
            self.valueLabel['text_scale'] = 0.037
            self.valueLabel['text'] = PLocalizer.RepCapText_Overall % level
            self.valueLabel.setZ(0.13)
