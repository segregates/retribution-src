from panda3d.core import Buffer, Lens, NodePath, PandaNode, Point3, TextNode, TextProperties, TextPropertiesManager, TransparencyAttrib, VBase4, Vec3, Vec4, WindowProperties, rad2Deg
# File: G (Python 2.4)
...
class GuiManager(FSM.FSM):
    ...
    def __init__(self, av):
        ...
        self.warningMsg.text.setBin('gui-fixed', 2)
        ...
        self._GuiManager__repValues = { }
        self.repHandlers = []
        self.chestTray = ChestTray.ChestTray(parent = NodePath(), parentMgr = self, pos = (-0.131, 0, 0.02), sortOrder = 1)

        if config.GetBool('want-dummy-profile-page', False):
            self.profilePage = DummyProfileInfo()

        else:
            self.profilePage = PirateProfilePage.PirateProfilePage()

        self.chestPanel = ChestPanel.ChestPanel(parent = self.chestTray.chestButton)
        self.chestPanel.setPos(-.55, 0, .8)
        scale = 1.0
        if base.options:
        ...
        self.bossMeter.hide()
        self.targetStatusTray = StatusTray.StatusTray(None, pos = (-0.225, 0, -0.25), showSkills = 1)
        self.targetStatusTray.hpMeter.setPos(-0.19, 0, -0.192)
        self.targetStatusTray.hpMeter.setScale(1.01)
        self.targetStatusTray.voodooMeter.setPos(-0.19, 0, -0.220)
        self.targetStatusTray.voodooMeter.setScale(0.925, 1, 0.325)
        self.targetStatusTray.meterChangeOffset = (-0.3875, 0, -0.192)
        self.targetStatusTray.nameLabel.setPos(-0.185, 0, -0.145)
        self.targetStatusTray.reloadFrame.setPos(-0.1055, 0, -0.2688)
        self.shipIcons = loader.loadModel('models/gui/ship_battle')
        self.targetStatusTray.targetFrame2 = DirectFrame(relief = None, parent = self.targetStatusTray.hpMeter, image = self.shipIcons.find('**/ship_battle_speed_bar*'), image_scale = (0.4, 1.0, 0.7), pos = (0.244, 0.0, 0.002), scale = (0.925, 1.0, 0.325))
        self.targetStatusTray.targetFrame = DirectFrame(relief = None, parent = self.targetStatusTray.hpMeter, image = self.shipIcons.find('**/ship_battle_speed_bar*'), image_scale = (0.43, 1.0, 0.7), pos = (0.244, 0.0, 0.016))
        ...
        self.accept(PiratesGlobals.TreasureHotkey, self.toggleBossMeter)
        # For Chest Tray buttons
        ...
        # For Chest Tray hot keys
        self.accept(PiratesGlobals.WorldMapHotKey, self.showMapPage)
        self.accept(PiratesGlobals.BagButtonHotKey, self.showInventoryBagPanel)
        self.accept(PiratesGlobals.WeaponHotKey, self.showWeaponPanel)
        self.accept(PiratesGlobals.SkillLevelHotKey, self.showSkillPage)
        if self.WantTitlesPage:
            self.accept(PiratesGlobals.TitlesHotKey, self.showTitlesPanel)
        self.accept(PiratesGlobals.ShipHotKey, self.showShipPanel)
        ...
        self.accept(PiratesGlobals.QuestJournalHotKey, self.showQuestPanel)
        self.accept(PiratesGlobals.LookoutHotKey, self.showLookoutPanel)
        self.accept('guiMgrToggleMainMenu', self.toggleMainMenu)
        ...
        self.instructionMessageBoxText = BorderFrame(parent = base.a2dBottomCenter, frameSize = iMBframeSize, modelName = iMBmodelName, imageColorScale = iMBimageColorScale, pos = (self.iMBsizeX * -0.5, 0, 0.3), textMayChange = 1, text = '', text_scale = self.iMBtextScale, text_align = TextNode.ACenter, text_pos = (self.iMBsizeX * 0.5, 0.09), text_fg = PiratesGuiGlobals.TextFG1, text_shadow = (0, 0, 0, 1), text_wordwrap = self.iMBsizeX * 0.9 / self.iMBtextScale, text_font = PiratesGlobals.getInterfaceFont())
        self.instructionMessageBoxText.hide()
        self.instructionMessageText = DirectLabel(parent = base.a2dBottomCenter, pos = (self.iMBsizeX * -0.5, 0, 0.23), textMayChange = 1, text = '', text_scale = self.iMBtextScale, text_align = TextNode.ACenter, text_pos = (self.iMBsizeX * 0.5, 0.09), text_fg = PiratesGuiGlobals.TextFG1, text_shadow = (0, 0, 0, 1), text_wordwrap = self.iMBsizeX * 0.9 / self.iMBtextScale, text_font = PiratesGlobals.getInterfaceFont())
    ...
    def delete(self):
        ...
        if self.injuredEffects:
            ...
            self.injuredBlackoutGeom.remove_node()
    ...
    def updateReputation(self, category, value):
        ...
        self._GuiManager__repValues[category] = value
        if inv:
    ...
    def refreshInventoryWeapons(self, newWeaponId = None):
        ...
        if self.combatTray.slotDisplay:
            for i in xrange(0, len(self.combatTray.slotDisplay.cellList)):
    ...
    def createLevelUpText(self):
        ...
        self.levelUpCategoryLabel = DirectLabel(parent = self.levelUpText, relief = None, text = '', text_font = PiratesGlobals.getPirateOutlineFont(), text_fg = (0.10, 0.7, 0.1, 1), scale = 0.125, pos = (0, 0, -0.125))
    ...
    def showLevelUpText(self, category, level):
        ...
        elif category == InventoryType.DefenseCannonRep:
            tpMgr = TextPropertiesManager.getGlobalPtr()
    ...
    def showTrays(self):
        ...
        if self.tutorialStatus >= PiratesGlobals.TUT_GOT_COMPASS:
            self.accept('c', self.radarGui.toggle)
            self.radarGui.show()
    ...
    def toggleMainMenu(self, args = None):
        ...
        elif self.av.gameFSM.state not in ('Cutscene', 'Dialog', 'NPCInteract', 'DinghyInteract', 'MakeAPirate'):
            self.chatPanel.show()
            self.av.chatMgr.start()
            self.av.startChat()
            self.setSeaChestAllowed(True)
    ...
    def setMinimap(self, minimap):
        ...
        if self.minimap is not minimap:
            ...
            self.newMap = minimap
            messenger.send('transferMinimapObjects', [
                self])
            self.oldMap = None
            self.newMap = None
    ...
    def hideSmokePanel(self):
        ...
        fadeOut = LerpFunctionInterval(self.smokePanel.setAlphaScale, fromData = self.smokePanel.getColorScale()[3], toData = 0, duration = 0.23)
    ...
    def addCheckPortraitButton(self):
        if not self.checkPortraitButton:
            self.checkPortraitButton = GuiButton(parent = base.a2dTopRight, text = PLocalizer.CheckPortrait, text_align = TextNode.ACenter, text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (0.0, -0.01), text_fg = PiratesGuiGlobals.TextFG1, text_shadow = PiratesGuiGlobals.TextShadow, text_wordwrap = 40, textMayChange = 1, image_scale = (0.45, 1, 0.25), command = self.checkPortrait, pos = (-0.65, 0, -0.23))
    ...
    def systemWarning(self, warningText = 'Bad things'):
        from pirates.chat.ChatWarningBox import ChatWarningBox
