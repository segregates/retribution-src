...
class InventoryUIManager(DirectFrame):
    ...
    def setWithinBag(self, bag, clear = 0, task = None):
        if bag == self.withInBag and clear:
            ...
            if self.itemPickup == PICKUP_DRAG:
                taskMgr.doMethodLater(0.3, bag.bar.openBag, 'InventoryUIManagerOpenBag', [
                    bag])
    ...
    def findOpenSlotForItemType(self, type, runningList = []):
        ...
        if slotRange:
            ...
            for rangeIndex in xrange(len(slotRange) - 1, -1, -1):
                atomicRange = slotRange[rangeIndex]
                for slotIndex in xrange(atomicRange[0], atomicRange[1]):
    ...
    def handleSlotUpdate(self, slot):
        if slot in self.slotToCellMap:
            ...
            if not slotCell:
                if slot >= Locations.RANGE_OVERFLOW[0] and slot <= Locations.RANGE_OVERFLOW[1]:
                    messenger.send('overflowChanged', [])
    ...
    def getGivingItems(self, items):
        ...
        if inventory:
            ...
            for i in xrange(0, invalidLocations):
                extraItems.append(weapons[i][1])

            for i in xrange(invalidLocations, len(weapons)):
            ...
            for i in xrange(invalidLocations, len(clothes)):
            ...
            for i in xrange(invalidLocations, len(consumables)):
    ...
    def startCellItemDetails(self, cell, detailsPos, detailsHeight, detailsDelay, event = None):
        ...
        if cell.inventoryItem:
            taskMgr.doMethodLater(detailsDelay, self.showDetails, 'inventoryUIDetailTask', extraArgs = [
                cell.inventoryItem,
                cell,
                detailsPos,
                detailsHeight])
