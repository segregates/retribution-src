...
class ShipRepairSpotMgr(ShipRepairSpotMgrBase):
    ...
    def _onShipReady(self):
        ...
        self._statePushes.extend([
            FunctionCall(self._evalNeedModels, self._state.validShipClass, self._state.hasTeam).pushCurrentState(),
            FunctionCall(self._evalNeedHoles, self._state.fullHealth, self._state.needModels).pushCurrentState(),
            FunctionCall(self._needModelsChanged, self._state.needModels).pushCurrentState(),
            FunctionCall(self._needHolesChanged, self._state.needHoles).pushCurrentState(),
    ...
    def _evalNeedModels(self, validShipClass, hasTeam):
        if config.GetBool('want-repair-game', 0):
            ...
            pass
        self._state.needModels.set(hasTeam)


    def _evalNeedHoles(self, fullHealth, needModels):
        if needModels:
            pass
        self._state.needHoles.set(not fullHealth)
