...
class LegendaryFish(Fish):
    ...
    def initFishStaminaBar(self):
        ...
        self.iconBaseFrame = DirectFrame(relief = None, state = DGG.DISABLED, pos = (0, 0, 0), sortOrder = 30, image = self.legendaryGui.find('**/pir_t_gui_fsh_fishPortraitFrame'), image_scale = 0.179, image_pos = (0, 0, 0))
        self.iconBaseFrame.setTransparency(TransparencyAttrib.MAlpha)
        self.fishUINodePath = NodePath(self.iconBaseFrame)
        self.fishUINodePath.setPos(-0.299, 0.0, 0.826)
        self.fishUINodePath.reparentTo(hidden)
        self.iconCard = loader.loadModel('models/gui/treasure_gui')
        self.iconBaseFrame.iconImage = OnscreenImage(parent = self.iconBaseFrame, image = self.iconCard.find('**/%s*' % CollectionMap.Assets[self.myData['id']]), scale = 0.348, hpr = (0, 0, 0), pos = (0.0, 0, 0.0))
        ...
        self.fishNameLabelNodePath.setPos(0.299, 0, 0.0400)
        self.fishNameLabelNodePath.setScale(0.0448)
        self.fishNameLabelNodePath.reparentTo(self.iconBaseFrame)
        self.fishStaminaBar = DirectWaitBar(parent = self.iconBaseFrame, relief = DGG.FLAT, state = DGG.DISABLED, range = 100, value = 0, sortOrder = 20, frameColor = (0, 0, 0, 1.0), pos = (0.070, 0.0, -0.0149), hpr = (0, 0, 0), frameSize = (0, 0.71, 0, 0.0280))
        ...
        self.fishStaminaValueLabelNodePath.setPos(0.66000, 0, -0.0598)
        self.fishStaminaValueLabelNodePath.setScale(0.0448)
        self.fishStaminaValueLabelNodePath.reparentTo(self.iconBaseFrame)
        self.fishStaminaBarFrame = DirectLabel(parent = self.iconBaseFrame, relief = None, state = DGG.DISABLED, frameColor = (1, 1, 1, 0.100), pos = (0.44, 0.0, 0.0), hpr = (0, 0, 0), sortOrder = 25, image = self.legendaryGui.find('**/pir_t_gui_fsh_staminaBarForeground'), image_scale = (1.0, 0.0, 0.050000), image_pos = (0.0, 0.0, 0.0), image_hpr = (0.0, 0.0, 0.0))
