from panda3d.core import Point3, TextProperties, TextPropertiesManager, Vec3, Vec4
...
class QuestRewardGUI(QuestDetailBase):
    ...
    def __init__(self, quest, rewards):
        ...
        self.questComplete = DirectLabel(parent = self, relief = None, text = PLocalizer.QuestCompleted, text_fg = PiratesGuiGlobals.TextFG4, text_font = PiratesGlobals.getPirateOutlineFont(), text_shadow = (0, 0, 0, 1), pos = (-1.32, 0, 0.13), scale = PiratesGuiGlobals.TextScaleTitleMed)
    ...
    def setQuestInfoFromQuest(self, quest):
        ...
        text = PLocalizer.QuestItemGuiFormat % {
            'title': title }
        if gold and self.reputation or self.items:
        ...
        if self.reputation:
            text += PLocalizer.QuestItemGuiAddRep % {
                'rep': self.reputation }

        if gold:
            text += PLocalizer.QuestItemGuiAddGold % {
                'gold': gold }
